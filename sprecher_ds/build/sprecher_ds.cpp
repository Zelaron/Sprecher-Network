// sprecher_ds.cpp
// Nintendo DS homebrew: MNIST digit classification with Sprecher Networks (SNDS v3)
// Build environment: devkitPro (libnds + libfat)
//
// This implementation follows the paper and the Python reference:
//  - Vector lambda (size d_in) -- NOT a d_inxd_out matrix
//  - Memory-efficient evaluation: compute outputs sequentially in q
//  - Shared splines phi and Phi
//  - Optional cyclic lateral mixing (cyclic or bidirectional)
//  - Optional node-style residuals that preserve O(N) memory
//
// Fixed-point: Q16.16 throughout.

#include <nds.h>
#include <fat.h>

#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <stdint.h>

// ============================================================
// CRITICAL: VBlank interrupt handler prevents white screen freeze
// ============================================================
static void VblankHandler() {}

// ============================================================
// Fixed-point (Q16.16)
// ============================================================

typedef int32_t fix16;
static const int FIX_SHIFT = 16;
static const fix16 FIX_ONE = (fix16)(1 << FIX_SHIFT);

static inline fix16 sat_i64(int64_t v) {
    if (v > (int64_t)INT32_MAX) return (fix16)INT32_MAX;
    if (v < (int64_t)INT32_MIN) return (fix16)INT32_MIN;
    return (fix16)v;
}

static inline fix16 fix_add(fix16 a, fix16 b) { return sat_i64((int64_t)a + (int64_t)b); }
static inline fix16 fix_sub(fix16 a, fix16 b) { return sat_i64((int64_t)a - (int64_t)b); }

static inline fix16 fix_mul(fix16 a, fix16 b) {
    // Saturating multiply (Q16.16 * Q16.16 -> Q16.16)
    int64_t prod = (int64_t)a * (int64_t)b;
    return sat_i64(prod >> FIX_SHIFT);
}

static inline fix16 fix_div(fix16 a, fix16 b) {
    // Saturating divide (Q16.16 / Q16.16 -> Q16.16)
    if (b == 0) return (a >= 0) ? (fix16)INT32_MAX : (fix16)INT32_MIN;
    int64_t num = ((int64_t)a << FIX_SHIFT);
    return sat_i64(num / (int64_t)b);
}

// 64-bit multiply-and-shift without per-term saturation (closer to float reference).
static inline int64_t fix_mul_to_i64_q16(fix16 a, fix16 b) {
    return ((int64_t)a * (int64_t)b) >> FIX_SHIFT;
}

// Print Q16.16 value as signed decimal with 3 fractional digits (no float printf needed).
static void iprint_fix3(fix16 v) {
    if (v < 0) {
        iprintf("-");
        v = (fix16)(-v);
    }
    int32_t whole = (int32_t)(v >> FIX_SHIFT);
    // round to nearest 0.001
    uint32_t frac = (uint32_t)(v & 0xFFFF);
    uint32_t frac3 = (frac * 1000u + 0x8000u) >> 16;
    if (frac3 >= 1000u) { whole += 1; frac3 -= 1000u; }
    iprintf("%ld.%03lu", (long)whole, (unsigned long)frac3);
}

// ============================================================
// Piecewise-linear spline (matches SimpleSpline linear path)
// ============================================================

typedef struct {
    uint32_t n_knots;
    fix16* knots;   // length n_knots
    fix16* coeffs;  // length n_knots (effective values at knots)
    bool monotonic; // phi is monotonic (clamp outside to [0,1]); Phi is not (linear extension)

    // precomputed
    fix16 x0, xN;     // first/last knot
    fix16 dx, inv_dx; // only valid if uniform==true
    fix16 slope0, slopeN; // boundary slopes for linear extension (Phi)
    bool uniform;
} Spline;

static void prepare_spline(Spline* sp) {
    if (!sp || sp->n_knots < 2 || !sp->knots || !sp->coeffs) return;

    sp->x0 = sp->knots[0];
    sp->xN = sp->knots[sp->n_knots - 1];

    // Boundary slopes (always computed, even if non-uniform)
    fix16 denom0 = fix_sub(sp->knots[1], sp->knots[0]);
    if (denom0 != 0) sp->slope0 = fix_div(fix_sub(sp->coeffs[1], sp->coeffs[0]), denom0);
    else sp->slope0 = 0;

    fix16 denomN = fix_sub(sp->knots[sp->n_knots - 1], sp->knots[sp->n_knots - 2]);
    if (denomN != 0) sp->slopeN = fix_div(fix_sub(sp->coeffs[sp->n_knots - 1], sp->coeffs[sp->n_knots - 2]), denomN);
    else sp->slopeN = 0;

    // Detect true uniform knot spacing AFTER Q16.16 quantization
    sp->uniform = false;
    sp->dx = 0;
    sp->inv_dx = 0;

    fix16 dx0 = denom0;
    if (dx0 != 0) {
        bool is_uniform = true;
        for (uint32_t k = 2; k < sp->n_knots; k++) {
            fix16 d = fix_sub(sp->knots[k], sp->knots[k - 1]);
            if (d != dx0) { is_uniform = false; break; }
        }
        if (is_uniform) {
            sp->uniform = true;
            sp->dx = dx0;
            sp->inv_dx = fix_div(FIX_ONE, dx0);
        }
    }
}

static fix16 spline_eval(const Spline* sp, fix16 x) {
    if (!sp || sp->n_knots < 2) return 0;

    // Outside domain handling matches Python SimpleSpline.forward() in linear mode:
    //  - monotonic phi: below->0, above->1
    //  - non-monotonic Phi: linear extension using boundary slopes
    if (x <= sp->x0) {
        if (sp->monotonic) return 0;
        return fix_add(sp->coeffs[0], fix_mul(sp->slope0, fix_sub(x, sp->x0)));
    }
    if (x >= sp->xN) {
        if (sp->monotonic) return FIX_ONE;
        return fix_add(sp->coeffs[sp->n_knots - 1], fix_mul(sp->slopeN, fix_sub(x, sp->xN)));
    }

    if (sp->uniform) {
        // Fast index: idx = floor((x-x0)/dx)
        fix16 t_all = fix_mul(fix_sub(x, sp->x0), sp->inv_dx);
        int idx = (int)(t_all >> FIX_SHIFT);
        if (idx < 0) idx = 0;
        if (idx >= (int)sp->n_knots - 1) idx = (int)sp->n_knots - 2;

        int64_t x_base = (int64_t)sp->x0 + (int64_t)idx * (int64_t)sp->dx;
        fix16 local_t = fix_mul(fix_sub(x, (fix16)x_base), sp->inv_dx);
        fix16 y0 = sp->coeffs[idx];
        fix16 y1 = sp->coeffs[idx + 1];
        return fix_add(y0, fix_mul(local_t, fix_sub(y1, y0)));
    }

    // General case: binary search then per-segment t=(x-x_k)/(x_{k+1}-x_k)
    int lo = 0;
    int hi = (int)sp->n_knots - 1;
    while (hi - lo > 1) {
        int mid = (lo + hi) >> 1;
        if (x < sp->knots[mid]) hi = mid;
        else lo = mid;
    }
    fix16 denom = fix_sub(sp->knots[hi], sp->knots[lo]);
    fix16 t = (denom != 0) ? fix_div(fix_sub(x, sp->knots[lo]), denom) : 0;
    return fix_add(sp->coeffs[lo], fix_mul(t, fix_sub(sp->coeffs[hi], sp->coeffs[lo])));
}

// ============================================================
// Sprecher Network structures (SNDS v3)
// ============================================================

typedef enum { RES_NONE=0, RES_SCALAR=1, RES_POOL=2, RES_BROADCAST=3, RES_PROJ=4 } ResidualType;

typedef struct {
    uint32_t d_in, d_out;
    fix16 eta;
    Spline phi, Phi;
    fix16* lambdas; // vector length d_in

    // Lateral mixing
    bool has_lateral;
    fix16 lateral_scale;
    fix16* lateral_w_fwd; // length d_out
    fix16* lateral_w_bwd; // length d_out (only if bidirectional)

    // Residuals (node-style)
    ResidualType residual_type;
    fix16 residual_scalar;   // scalar weight (when d_in==d_out)
    fix16* residual_pool;    // length d_in
    fix16* residual_bcast;   // length d_out

    // Norm (BatchNorm folded to affine)
    uint32_t norm_dim;       // 0 means none
    fix16* norm_scale;       // length norm_dim
    fix16* norm_bias;        // length norm_dim
} Block;

typedef struct {
    uint32_t version, flags, num_blocks, input_dim, output_dim, total_params, max_dim;
    fix16 q_factor;

    // global feature flags
    bool lateral_bidir;
    bool norm_before;
    uint32_t norm_type;
    bool norm_skip_first;

    Block* blocks;

    fix16 output_scale, output_bias;

    // single contiguous allocation: [all weights][work_a][work_b][scratch]
    fix16* all_memory;
    fix16* work_a;
    fix16* work_b;
    fix16* scratch;
} SprecherNet;

// Global flags (match exporter)
static const uint32_t FLAG_LATERAL_BIDIR   = 1u << 1;
static const uint32_t FLAG_NORM_BEFORE     = 1u << 6;
static const uint32_t FLAG_NORM_SKIP_FIRST = 1u << 7;

// Per-block flags
static const uint32_t BF_HAS_LATERAL = 1u << 0;

// ============================================================
// File IO helpers
// ============================================================

static bool read_u32(FILE* f, uint32_t* out) { return fread(out, 4, 1, f) == 1; }
static bool read_i32(FILE* f, int32_t* out) { return fread(out, 4, 1, f) == 1; }

static void free_net(SprecherNet* net) {
    if (!net) return;
    if (net->blocks) free(net->blocks);
    if (net->all_memory) free(net->all_memory);
    memset(net, 0, sizeof(SprecherNet));
}

// ============================================================
// Normalization (affine only; BN already folded in exporter)
// ============================================================

static void apply_norm_inplace(const Block* blk, fix16* vec) {
    if (!blk || blk->norm_dim == 0 || !blk->norm_scale || !blk->norm_bias) return;
    for (uint32_t i = 0; i < blk->norm_dim; i++) {
        vec[i] = fix_add(fix_mul(vec[i], blk->norm_scale[i]), blk->norm_bias[i]);
    }
}

// ============================================================
// Forward pass for one block (memory-efficient)
// ============================================================

static void forward_block(const SprecherNet* net, const Block* blk, const fix16* x, fix16* y) {
    const uint32_t d_in = blk->d_in;
    const uint32_t d_out = blk->d_out;
    fix16* scratch = net->scratch;

    // 1) s_q = sum_i lambda_i * phi(x_i + eta*q) + q_factor*q
    for (uint32_t q = 0; q < d_out; q++) {
        fix16 q_fix = (fix16)((int32_t)q << FIX_SHIFT);
        fix16 eta_q = fix_mul(blk->eta, q_fix);

        int64_t acc = 0;
        for (uint32_t i = 0; i < d_in; i++) {
            fix16 phi_val = spline_eval(&blk->phi, fix_add(x[i], eta_q));
            acc += fix_mul_to_i64_q16(blk->lambdas[i], phi_val);
        }
        acc += fix_mul_to_i64_q16(net->q_factor, q_fix);
        y[q] = sat_i64(acc);
    }

    // 2) Lateral mixing on s (cyclic neighbor indices)
    if (blk->has_lateral && d_out > 1) {
        if (net->lateral_bidir) {
            for (uint32_t q = 0; q < d_out; q++) {
                uint32_t q_next = (q + 1u == d_out) ? 0u : (q + 1u);
                uint32_t q_prev = (q == 0u) ? (d_out - 1u) : (q - 1u);
                fix16 sum = fix_add(fix_mul(blk->lateral_w_fwd[q], y[q_next]),
                                    fix_mul(blk->lateral_w_bwd[q], y[q_prev]));
                scratch[q] = fix_add(y[q], fix_mul(blk->lateral_scale, sum));
            }
        } else {
            // cyclic (single direction): each q mixes from (q+1)%d_out
            for (uint32_t q = 0; q < d_out; q++) {
                uint32_t q_next = (q + 1u == d_out) ? 0u : (q + 1u);
                scratch[q] = fix_add(y[q], fix_mul(blk->lateral_scale,
                                                   fix_mul(blk->lateral_w_fwd[q], y[q_next])));
            }
        }
        memcpy(y, scratch, d_out * sizeof(fix16));
    }

    // 3) Phi spline
    for (uint32_t q = 0; q < d_out; q++) {
        y[q] = spline_eval(&blk->Phi, y[q]);
    }

    // 4) Node-style residuals (preserve O(N))
    switch (blk->residual_type) {
        case RES_SCALAR:
            // only used when d_in==d_out
            for (uint32_t q = 0; q < d_out; q++)
                y[q] = fix_add(y[q], fix_mul(blk->residual_scalar, x[q]));
            break;

        case RES_POOL:
            // d_in > d_out: scatter-add by i % d_out
            for (uint32_t i = 0; i < d_in; i++) {
                uint32_t q = (d_out != 0) ? (i % d_out) : 0;
                y[q] = fix_add(y[q], fix_mul(blk->residual_pool[i], x[i]));
            }
            break;

        case RES_BROADCAST:
            // d_in < d_out: gather from q % d_in
            for (uint32_t q = 0; q < d_out; q++) {
                uint32_t src = (d_in != 0) ? (q % d_in) : 0;
                y[q] = fix_add(y[q], fix_mul(blk->residual_bcast[q], x[src]));
            }
            break;

        default:
            break;
    }
}

static void net_forward(const SprecherNet* net, const fix16* input, fix16* output, bool show_progress) {
    if (!net || !input || !output) return;

    fix16* a = net->work_a;
    fix16* b = net->work_b;

    uint32_t cur_dim = net->input_dim;
    memcpy(a, input, cur_dim * sizeof(fix16));

    for (uint32_t bi = 0; bi < net->num_blocks; bi++) {
        const Block* blk = &net->blocks[bi];
        if (blk->d_in != cur_dim) break;

        if (show_progress) {
            iprintf("\x1b[17;0H Layer %lu/%lu     ",
                    (unsigned long)(bi + 1), (unsigned long)net->num_blocks);
        }

        if (net->norm_before && blk->norm_dim > 0) apply_norm_inplace(blk, a);
        forward_block(net, blk, a, b);
        if (!net->norm_before && blk->norm_dim > 0) apply_norm_inplace(blk, b);

        // swap
        fix16* tmp = a; a = b; b = tmp;
        cur_dim = blk->d_out;
    }

    // final affine
    for (uint32_t i = 0; i < net->output_dim; i++) {
        output[i] = fix_add(fix_mul(a[i], net->output_scale), net->output_bias);
    }
}

// ============================================================
// Loading SNDS v3 weights (binary format defined by sn_mnist.py export_nds)
// ============================================================

static bool load_net_v3(SprecherNet* net, const char* filename, char* err, size_t errlen) {
    FILE* f = fopen(filename, "rb");
    if (!f) { snprintf(err, errlen, "Cannot open %s", filename); return false; }

    char magic[4];
    if (fread(magic, 1, 4, f) != 4 || memcmp(magic, "SNDS", 4) != 0) {
        snprintf(err, errlen, "Bad magic"); fclose(f); return false;
    }

    uint32_t version = 0, fix_shift = 0;
    if (!read_u32(f, &version) || !read_u32(f, &fix_shift) || version != 3 || fix_shift != 16) {
        snprintf(err, errlen, "Bad version/shift"); fclose(f); return false;
    }

    uint32_t flags=0, num_blocks=0, input_dim=0, output_dim=0, total_params=0, max_dim=0;
    int32_t q_factor_i32=0;
    if (!read_u32(f, &flags) || !read_u32(f, &num_blocks) || !read_u32(f, &input_dim) ||
        !read_u32(f, &output_dim) || !read_u32(f, &total_params) || !read_u32(f, &max_dim) ||
        !read_i32(f, &q_factor_i32)) {
        snprintf(err, errlen, "Header read failed"); fclose(f); return false;
    }

    uint32_t norm_type = (flags >> 4) & 3u;
    bool lateral_bidir = (flags & FLAG_LATERAL_BIDIR) != 0;
    bool norm_before = (flags & FLAG_NORM_BEFORE) != 0;
    bool norm_skip_first = (flags & FLAG_NORM_SKIP_FIRST) != 0;

    if (num_blocks == 0 || num_blocks > 128) {
        snprintf(err, errlen, "Bad num_blocks"); fclose(f); return false;
    }
    if (max_dim == 0 || max_dim > 4096) {
        snprintf(err, errlen, "Bad max_dim"); fclose(f); return false;
    }

    net->blocks = (Block*)calloc(num_blocks, sizeof(Block));
    if (!net->blocks) { snprintf(err, errlen, "OOM blocks"); fclose(f); return false; }

    // First pass: count how many fix16 values we need for all weights
    long blocks_start = ftell(f);
    if (blocks_start < 0) blocks_start = 0;

    uint32_t weight_fix16s = 0;

    for (uint32_t bi = 0; bi < num_blocks; bi++) {
        uint32_t d_in=0, d_out=0, phi_nk=0, Phi_nk=0, block_flags=0, residual_type=0, norm_dim=0;
        int32_t eta_i32=0;
        if (!read_u32(f, &d_in) || !read_u32(f, &d_out) || !read_u32(f, &phi_nk) || !read_u32(f, &Phi_nk) ||
            !read_u32(f, &block_flags) || !read_u32(f, &residual_type) || !read_u32(f, &norm_dim) || !read_i32(f, &eta_i32)) {
            snprintf(err, errlen, "Block header EOF"); fclose(f); return false;
        }

        if (phi_nk < 2 || Phi_nk < 2) { snprintf(err, errlen, "Bad knot count"); fclose(f); return false; }
        if (d_in == 0 || d_out == 0) { snprintf(err, errlen, "Bad dims"); fclose(f); return false; }
        if (residual_type == RES_PROJ) { snprintf(err, errlen, "Residual projection not supported"); fclose(f); return false; }
        if (residual_type > RES_PROJ) { snprintf(err, errlen, "Bad residual type"); fclose(f); return false; }

        net->blocks[bi].d_in = d_in;
        net->blocks[bi].d_out = d_out;
        net->blocks[bi].has_lateral = (block_flags & BF_HAS_LATERAL) != 0;
        net->blocks[bi].residual_type = (ResidualType)residual_type;
        net->blocks[bi].norm_dim = norm_dim;

        // base: phi knots+coeffs, Phi knots+coeffs, lambda vector
        uint32_t skip_fix16 = phi_nk * 2 + Phi_nk * 2 + d_in;
        weight_fix16s += skip_fix16;

        // lateral
        if (net->blocks[bi].has_lateral) {
            uint32_t lat = 1 + (lateral_bidir ? (d_out * 2) : d_out);
            skip_fix16 += lat;
            weight_fix16s += lat;
        }

        // residual
        if (residual_type == RES_SCALAR) { skip_fix16 += 1; weight_fix16s += 1; }
        else if (residual_type == RES_POOL) { skip_fix16 += d_in; weight_fix16s += d_in; }
        else if (residual_type == RES_BROADCAST) { skip_fix16 += d_out; weight_fix16s += d_out; }

        // norm
        if (norm_dim > 0) { skip_fix16 += norm_dim * 2; weight_fix16s += norm_dim * 2; }

        // Skip arrays (each is int32 -> 4 bytes)
        if (fseek(f, (long)skip_fix16 * 4L, SEEK_CUR) != 0) {
            snprintf(err, errlen, "Seek failed"); fclose(f); return false;
        }
    }

    // Allocate one contiguous block: weights + work_a + work_b + scratch
    uint32_t total_fix16s = weight_fix16s + 3 * max_dim;
    net->all_memory = (fix16*)malloc((size_t)total_fix16s * sizeof(fix16));
    if (!net->all_memory) { snprintf(err, errlen, "OOM weights"); fclose(f); return false; }

    // Second pass: actually load the weights into all_memory
    if (fseek(f, blocks_start, SEEK_SET) != 0) {
        snprintf(err, errlen, "Seek blocks start failed"); fclose(f); return false;
    }

    fix16* mem = net->all_memory;

    for (uint32_t bi = 0; bi < num_blocks; bi++) {
        uint32_t d_in=0, d_out=0, phi_nk=0, Phi_nk=0, block_flags=0, res_type=0, norm_dim=0;
        int32_t eta_i32=0;
        if (!read_u32(f, &d_in) || !read_u32(f, &d_out) || !read_u32(f, &phi_nk) || !read_u32(f, &Phi_nk) ||
            !read_u32(f, &block_flags) || !read_u32(f, &res_type) || !read_u32(f, &norm_dim) || !read_i32(f, &eta_i32)) {
            snprintf(err, errlen, "Block header EOF"); fclose(f); return false;
        }
        if (res_type == RES_PROJ) { snprintf(err, errlen, "Residual projection not supported"); fclose(f); return false; }

        Block* blk = &net->blocks[bi];
        blk->d_in = d_in;
        blk->d_out = d_out;
        blk->eta = (fix16)eta_i32;
        blk->has_lateral = (block_flags & BF_HAS_LATERAL) != 0;
        blk->residual_type = (ResidualType)res_type;
        blk->norm_dim = norm_dim;

        // phi (monotonic)
        blk->phi.n_knots = phi_nk;
        blk->phi.monotonic = true;
        blk->phi.knots = mem;
        for (uint32_t i = 0; i < phi_nk; i++) { int32_t v; if (!read_i32(f, &v)) { snprintf(err, errlen, "EOF phi knots"); fclose(f); return false; } *mem++ = (fix16)v; }
        blk->phi.coeffs = mem;
        for (uint32_t i = 0; i < phi_nk; i++) { int32_t v; if (!read_i32(f, &v)) { snprintf(err, errlen, "EOF phi coeffs"); fclose(f); return false; } *mem++ = (fix16)v; }
        prepare_spline(&blk->phi);

        // Phi (non-monotonic)
        blk->Phi.n_knots = Phi_nk;
        blk->Phi.monotonic = false;
        blk->Phi.knots = mem;
        for (uint32_t i = 0; i < Phi_nk; i++) { int32_t v; if (!read_i32(f, &v)) { snprintf(err, errlen, "EOF Phi knots"); fclose(f); return false; } *mem++ = (fix16)v; }
        blk->Phi.coeffs = mem;
        for (uint32_t i = 0; i < Phi_nk; i++) { int32_t v; if (!read_i32(f, &v)) { snprintf(err, errlen, "EOF Phi coeffs"); fclose(f); return false; } *mem++ = (fix16)v; }
        prepare_spline(&blk->Phi);

        // lambda vector
        blk->lambdas = mem;
        for (uint32_t i = 0; i < d_in; i++) { int32_t v; if (!read_i32(f, &v)) { snprintf(err, errlen, "EOF lambdas"); fclose(f); return false; } *mem++ = (fix16)v; }

        // Lateral params
        blk->lateral_w_fwd = NULL;
        blk->lateral_w_bwd = NULL;
        blk->lateral_scale = 0;
        if (blk->has_lateral) {
            int32_t v;
            if (!read_i32(f, &v)) { snprintf(err, errlen, "EOF lateral_scale"); fclose(f); return false; }
            blk->lateral_scale = (fix16)v;

            blk->lateral_w_fwd = mem;
            for (uint32_t i = 0; i < d_out; i++) { if (!read_i32(f, &v)) { snprintf(err, errlen, "EOF lat_w"); fclose(f); return false; } *mem++ = (fix16)v; }

            if (lateral_bidir) {
                blk->lateral_w_bwd = mem;
                for (uint32_t i = 0; i < d_out; i++) { if (!read_i32(f, &v)) { snprintf(err, errlen, "EOF lat_wb"); fclose(f); return false; } *mem++ = (fix16)v; }
            }
        }

        // Residual params
        blk->residual_scalar = 0;
        blk->residual_pool = NULL;
        blk->residual_bcast = NULL;

        if (blk->residual_type == RES_SCALAR) {
            int32_t v;
            if (!read_i32(f, &v)) { snprintf(err, errlen, "EOF residual scalar"); fclose(f); return false; }
            blk->residual_scalar = (fix16)v;
        } else if (blk->residual_type == RES_POOL) {
            blk->residual_pool = mem;
            for (uint32_t i = 0; i < d_in; i++) { int32_t v; if (!read_i32(f, &v)) { snprintf(err, errlen, "EOF residual pool"); fclose(f); return false; } *mem++ = (fix16)v; }
        } else if (blk->residual_type == RES_BROADCAST) {
            blk->residual_bcast = mem;
            for (uint32_t i = 0; i < d_out; i++) { int32_t v; if (!read_i32(f, &v)) { snprintf(err, errlen, "EOF residual bcast"); fclose(f); return false; } *mem++ = (fix16)v; }
        }

        // Norm params (folded affine)
        blk->norm_scale = NULL;
        blk->norm_bias = NULL;
        if (norm_dim > 0) {
            blk->norm_scale = mem;
            for (uint32_t i = 0; i < norm_dim; i++) { int32_t v; if (!read_i32(f, &v)) { snprintf(err, errlen, "EOF norm scale"); fclose(f); return false; } *mem++ = (fix16)v; }
            blk->norm_bias = mem;
            for (uint32_t i = 0; i < norm_dim; i++) { int32_t v; if (!read_i32(f, &v)) { snprintf(err, errlen, "EOF norm bias"); fclose(f); return false; } *mem++ = (fix16)v; }
        }
    }

    int32_t out_scale_i32=0, out_bias_i32=0;
    if (!read_i32(f, &out_scale_i32) || !read_i32(f, &out_bias_i32)) {
        snprintf(err, errlen, "EOF footer"); fclose(f); return false;
    }
    fclose(f);

    // Fill net header
    net->version = version;
    net->flags = flags;
    net->num_blocks = num_blocks;
    net->input_dim = input_dim;
    net->output_dim = output_dim;
    net->total_params = total_params;
    net->max_dim = max_dim;
    net->q_factor = (fix16)q_factor_i32;

    net->lateral_bidir = lateral_bidir;
    net->norm_before = norm_before;
    net->norm_type = norm_type;
    net->norm_skip_first = norm_skip_first;

    net->output_scale = (fix16)out_scale_i32;
    net->output_bias = (fix16)out_bias_i32;

    // Work buffers
    net->work_a = mem; mem += max_dim;
    net->work_b = mem; mem += max_dim;
    net->scratch = mem; // max_dim

    return true;
}

static bool load_net(SprecherNet* net, const char* filename, char* err, size_t errlen) {
    memset(net, 0, sizeof(SprecherNet));

    FILE* f = fopen(filename, "rb");
    if (!f) { snprintf(err, errlen, "Cannot open %s", filename); return false; }

    char magic[4];
    uint32_t version = 0;
    bool ok = fread(magic, 1, 4, f) == 4 && memcmp(magic, "SNDS", 4) == 0 && read_u32(f, &version);
    fclose(f);

    if (!ok) { snprintf(err, errlen, "Invalid file"); return false; }
    if (version == 3) return load_net_v3(net, filename, err, errlen);

    snprintf(err, errlen, "Need v3, got v%lu", (unsigned long)version);
    return false;
}

// ============================================================
// Drawing canvas (bottom screen)
// ============================================================

static const int CANVAS_W = 256;
static const int CANVAS_H = 192;
static uint8_t g_canvas[CANVAS_W * CANVAS_H];

static inline uint16_t gray555(uint8_t v) {
    int c = v >> 3;
    return (uint16_t)(RGB15(c, c, c) | BIT(15));
}

static void clear_canvas(u16* gfx) {
    memset(g_canvas, 0, sizeof(g_canvas));
    for (int i = 0; i < 256 * 192; i++) gfx[i] = gray555(0);
}

static void draw_dot(u16* gfx, int x, int y, int radius, uint8_t intensity) {
    for (int dy = -radius; dy <= radius; dy++) {
        for (int dx = -radius; dx <= radius; dx++) {
            int xx = x + dx, yy = y + dy;
            if (xx >= 0 && xx < CANVAS_W && yy >= 0 && yy < CANVAS_H && dx*dx + dy*dy <= radius*radius) {
                g_canvas[yy * CANVAS_W + xx] = intensity;
                gfx[yy * 256 + xx] = gray555(intensity);
            }
        }
    }
}

static void draw_line(u16* gfx, int x0, int y0, int x1, int y1, int radius, uint8_t intensity) {
    int dx = abs(x1 - x0), sx = x0 < x1 ? 1 : -1;
    int dy = -abs(y1 - y0), sy = y0 < y1 ? 1 : -1;
    int err = dx + dy;
    for (;;) {
        draw_dot(gfx, x0, y0, radius, intensity);
        if (x0 == x1 && y0 == y1) break;
        int e2 = 2 * err;
        if (e2 >= dy) { err += dy; x0 += sx; }
        if (e2 <= dx) { err += dx; y0 += sy; }
    }
}

// ============================================================
// Downsample to 28x28 (simple average pooling)
// ============================================================

static void downsample_28x28(uint8_t out[28*28]) {
    // Use center 192x192 ROI to map to 28x28
    const int ROI_X0 = 32;
    const int ROI_Y0 = 0;
    const int ROI_SIZE = 192;

    for (int y = 0; y < 28; y++) {
        int y0 = ROI_Y0 + (y * ROI_SIZE) / 28;
        int y1 = ROI_Y0 + ((y + 1) * ROI_SIZE) / 28;
        for (int x = 0; x < 28; x++) {
            int x0 = ROI_X0 + (x * ROI_SIZE) / 28;
            int x1 = ROI_X0 + ((x + 1) * ROI_SIZE) / 28;
            int sum = 0, count = 0;
            for (int yy = y0; yy < y1; yy++) {
                for (int xx = x0; xx < x1; xx++) {
                    sum += g_canvas[yy * CANVAS_W + xx];
                    count++;
                }
            }
            out[y * 28 + x] = (count > 0) ? (uint8_t)(sum / count) : 0;
        }
    }
}

static void pixels_to_input(const uint8_t pix[28*28], fix16 out[28*28]) {
    for (int i = 0; i < 28*28; i++) {
        // Map [0,255] -> [0,1] in Q16.16
        out[i] = (fix16)(((uint32_t)pix[i] << FIX_SHIFT) / 255u);
    }
}

// ============================================================
// Main
// ============================================================

int main(void) {
    // VBlank handler is required on some emulators/hardware.
    irqSet(IRQ_VBLANK, VblankHandler);
    irqEnable(IRQ_VBLANK);

    // Console on top screen
    consoleDemoInit();

    // Bottom screen: bitmap framebuffer for drawing
    videoSetModeSub(MODE_5_2D);
    vramSetBankC(VRAM_C_SUB_BG);
    int bg = bgInitSub(3, BgType_Bmp16, BgSize_B16_256x256, 0, 0);
    u16* gfx = (u16*)bgGetGfxPtr(bg);

    clear_canvas(gfx);

    iprintf("\n Sprecher Network on Nintendo DS\n");
    iprintf(" ================================\n\n");
    iprintf(" Stylus: draw on bottom screen\n");
    iprintf(" A: classify digit\n");
    iprintf(" B: clear canvas\n");
    iprintf(" START: quit\n\n");

    // Init FAT for SD card access
    iprintf(" Initializing FAT...\n");
    if (!fatInitDefault()) {
        iprintf("\x1b[31m FAT init FAILED!\x1b[39m\n");
        iprintf(" Check DLDI / SD card setup.\n");
        iprintf("\n Press START to exit.\n");
        while (1) {
            swiWaitForVBlank();
            scanKeys();
            if (keysDown() & KEY_START) return 0;
        }
    }
    iprintf("\x1b[32m FAT OK.\x1b[39m\n");

    // Load weights (try both long name and 8.3 name)
    SprecherNet net;
    char err[128];
    bool net_ok = load_net(&net, "sn_weights.bin", err, sizeof(err));
    if (!net_ok) net_ok = load_net(&net, "SN_WEIGH.BIN", err, sizeof(err));

    fix16* logits_buf = NULL;

    if (!net_ok) {
        iprintf("\x1b[31m Load FAILED:\x1b[39m %s\n", err);
        iprintf("\n Put weights file on SD root.\n");
    } else {
        // MLP equivalent params for reference (purely informational)
        uint64_t mlp_params = 0;
        for (uint32_t bi = 0; bi < net.num_blocks; bi++) {
            mlp_params += (uint64_t)net.blocks[bi].d_in * (uint64_t)net.blocks[bi].d_out + (uint64_t)net.blocks[bi].d_out;
        }

        iprintf("\n\x1b[32m Loaded SNDS v%lu\x1b[39m\n", (unsigned long)net.version);
        iprintf(" Blocks: %lu\n", (unsigned long)net.num_blocks);
        iprintf(" Arch: %lu", (unsigned long)net.input_dim);
        for (uint32_t bi = 0; bi < net.num_blocks; bi++) iprintf("->%lu", (unsigned long)net.blocks[bi].d_out);
        iprintf("\n");
        iprintf(" SN params:  %lu\n", (unsigned long)net.total_params);
        iprintf(" MLP equiv:  %llu\n", (unsigned long long)mlp_params);

        iprintf("\n Ready! Draw a digit, press A.\n");

        if (net.output_dim >= 1 && net.output_dim <= 64) {
            logits_buf = (fix16*)malloc(net.output_dim * sizeof(fix16));
        }
    }

    touchPosition touch;
    bool drawing = false;
    int last_x = 0, last_y = 0;

    while (1) {
        swiWaitForVBlank();
        scanKeys();
        int down = keysDown();
        int held = keysHeld();

        if (down & KEY_START) break;

        if (down & KEY_B) {
            clear_canvas(gfx);
            iprintf("\x1b[18;0H Canvas cleared.             \n");
        }

        if (held & KEY_TOUCH) {
            touchRead(&touch);
            int x = touch.px, y = touch.py;
            if (!drawing) { drawing = true; last_x = x; last_y = y; }
            draw_line(gfx, last_x, last_y, x, y, 6, 255);
            last_x = x; last_y = y;
        } else {
            drawing = false;
        }

        if ((down & KEY_A) && net_ok && logits_buf) {
            iprintf("\x1b[18;0H Running inference...        \n");

            uint8_t pix28[28*28];
            fix16 input[28*28];

            downsample_28x28(pix28);
            pixels_to_input(pix28, input);

            memset(logits_buf, 0, net.output_dim * sizeof(fix16));
            net_forward(&net, input, logits_buf, true);

            int pred = 0;
            fix16 best = logits_buf[0];
            for (uint32_t i = 1; i < net.output_dim; i++) {
                if (logits_buf[i] > best) { best = logits_buf[i]; pred = (int)i; }
            }

            iprintf("\x1b[18;0H                             \n");
            iprintf("\x1b[19;0H >>> PREDICTED: %d <<<       \n", pred);

            if (net.output_dim <= 10) {
                for (uint32_t i = 0; i < net.output_dim; i++) {
                    iprintf("\x1b[%d;0H  %lu: ", 21 + (int)i, (unsigned long)i);
                    iprint_fix3(logits_buf[i]);
                    iprintf(" %s\n", (i == (uint32_t)pred) ? "<" : " ");
                }
            }
        } else if ((down & KEY_A) && !net_ok) {
            iprintf("\x1b[18;0H No weights loaded!          \n");
        }
    }

    if (logits_buf) free(logits_buf);
    free_net(&net);
    return 0;
}
