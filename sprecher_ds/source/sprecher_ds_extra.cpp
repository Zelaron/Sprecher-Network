...
static inline fix16 fix_mul(fix16 a, fix16 b) {
    // Q16.16 multiply with rounding-to-nearest (ties away from zero).
    int64_t prod = (int64_t)a * (int64_t)b;
    // Add +/- 0.5 ulp before shifting.
    prod += ((prod >> 63) | 1LL) * (1LL << (FIX_SHIFT - 1));
    return sat_i64(prod >> FIX_SHIFT);
}

static inline fix16 fix_div(fix16 a, fix16 b) {
    if (b == 0) return (a >= 0) ? INT32_MAX : INT32_MIN;
    // Q16.16 divide with rounding-to-nearest (ties away from zero).
    ...
    int64_t q = abs_num / abs_den;
    int64_t r = abs_num % abs_den;
...
static void iprint_fix16_4(fix16 v) {
    // Print Q16.16 with 4 fractional decimals.
    bool neg = v < 0;
    int64_t absv = neg ? -(int64_t)v : (int64_t)v;
    int32_t ip = (int32_t)(absv >> FIX_SHIFT);
    int32_t frac = (int32_t)((((absv & 0xFFFF) * 10000LL) + (1LL << (FIX_SHIFT - 1))) >> FIX_SHIFT);

    if (frac >= 10000) { ip += 1; frac -= 10000; }

    iprintf(neg ? "-" : " ");
    iprintf("%ld.%04ld", (long)ip, (long)frac);
}

static void console_clear_line(int row) {
    // Clear a single 32-char console line.
    iprintf("\x1b[%d;0H                                ", row);
}

static void console_clear_area(int row0, int row1) {
    for (int r = row0; r <= row1; r++) console_clear_line(r);
}

static void print_logits_two_column(const fix16* logits, uint32_t n_logits) {
    // Fits the DS top-screen text console (32 cols). We show at most 10 outputs.
    uint32_t n = (n_logits > 10U) ? 10U : n_logits;
    uint32_t half = (n + 1U) / 2U;  // left column size
    uint32_t rows = half;

    const int start_row = 18;
    const int col1 = 2;
    const int col2 = 18;

    console_clear_area(start_row, start_row + (int)rows - 1);
    ...
    for (uint32_t r = 0; r < rows; ++r) {
        ...
        if (li < n) {
            iprintf("\x1b[%d;%dH%lu: ", start_row + (int)r, col1, (unsigned long)li);
            iprint_fix16_4(logits[li]);
        }

        if (ri < n) {
            iprintf("\x1b[%d;%dH%lu: ", start_row + (int)r, col2, (unsigned long)ri);
            iprint_fix16_4(logits[ri]);
        }
    }
}
...
static bool net_forward(const SprecherNet* net, const fix16* input, fix16* output, bool show_progress) {
    if (!net || !net->blocks || !net->work_a || !net->work_b || !net->scratch || !input || !output) return false;
    ...
    for (uint32_t bi = 0; bi < net->num_blocks; bi++) {
        ...
        if (show_progress) iprintf("\x1b[16;0H Layer %lu/%lu     ", (unsigned long)(bi+1), (unsigned long)net->num_blocks);
        if (blk->d_in != cur_dim) return false;
    ...
    if (cur_dim != net->output_dim) return false;
...
static void draw_dot(u16* gfx, int x, int y, int radius, uint8_t intensity) {
    ...
    for (int dy = -radius; dy <= radius; ++dy) {
        int yy = y + dy;
        if ((unsigned)yy >= CANVAS_H) continue;
        for (int dx = -radius; dx <= radius; ++dx) {
            int xx = x + dx;
            if ((unsigned)xx >= CANVAS_W) continue;
            const int d2 = dx * dx + dy * dy;
            if (d2 > r2) continue;

            // Soft brush: linear falloff to avoid "blocky" downsampled digits.
            // Keep the max intensity written to each pixel (prevents erasing).
            int val = (intensity * (r2 - d2) + (r2 / 2)) / r2;  // 0..intensity
            uint8_t* p = &g_canvas[yy * CANVAS_W + xx];
            if (val > (int)(*p)) {
                *p = (uint8_t)val;
                gfx[yy * 256 + xx] = gray555((uint8_t)val);
            }
        }
    }
}
...
// ============================================================
// Downsample the drawing canvas to MNIST-like 28x28
// ============================================================
//
// The original MNIST digits are roughly centered and have some margin.
// Freehand drawing on the DS screen is *not*.
// To improve accuracy without touching the network math, we:
//   1) find the bounding box of non-black pixels on the full 256x192 canvas
//   2) expand to a square region with ~20% margin (so the digit occupies ~20x20 in 28x28)
//   3) area-average downsample to 28x28 (anti-aliased)
//   4) normalize contrast (stretch so max==255)
//   5) apply a light 3x3 Gaussian-ish blur (makes strokes look closer to MNIST grayscale)
//
// All of this runs only when you press A (not per-frame), so it's cheap.

static bool canvas_find_bbox(int* out_min_x, int* out_min_y, int* out_max_x, int* out_max_y, uint8_t thresh) {
    int min_x = CANVAS_W, min_y = CANVAS_H;
    int max_x = -1, max_y = -1;

    for (int y = 0; y < CANVAS_H; y++) {
        const uint8_t* row = &g_canvas[y * CANVAS_W];
...
static void shift_to_center_of_mass_28x28(uint8_t img[28 * 28]) {
    uint64_t sum = 0;
    uint64_t sum_x = 0;
    uint64_t sum_y = 0;
    ...
    for (int y = 0; y < 28; ++y) {
        for (int x = 0; x < 28; ++x) {
            uint32_t v = img[y * 28 + x];
            sum += v;
            sum_x += (uint64_t)v * (uint64_t)x;
            sum_y += (uint64_t)v * (uint64_t)y;
        }
    }

    if (sum == 0) return;

    // Integer pixel coordinates of the center of mass.
    int cx = (int)((sum_x + (sum / 2)) / sum);
    int cy = (int)((sum_y + (sum / 2)) / sum);

    // Shift so COM is near the center (13.5, 13.5). Using 14 is a good integer target.
    int dx = 14 - cx;
    int dy = 14 - cy;
...
static void downsample_28x28(uint8_t out[28 * 28]) {
    ...
    // Find bounding box of "ink" in the full canvas.
    int min_x, min_y, max_x, max_y;
    if (!canvas_find_bbox(&min_x, &min_y, &max_x, &max_y, 8)) {
        return;
    }

    // Expand bbox a bit to include some margin around the digit.
    int w = max_x - min_x + 1;
    int h = max_y - min_y + 1;
    int max_dim = (w > h) ? w : h;
    int margin = (max_dim / 8) + 2;
    ...
    // Resize to fit into a 20x20 box (MNIST-style), preserving aspect ratio.
    int new_w, new_h;
    if (w >= h) {
        new_w = 20;
        new_h = (h * 20 + (w / 2)) / w;
    ...
    // Resample bbox -> tmp (area averaging, partitions bbox).
    uint8_t tmp[20 * 20];
    memset(tmp, 0, sizeof(tmp));

    for (int oy = 0; oy < new_h; ++oy) {
        int y0 = min_y + (oy * h) / new_h;
        int y1 = min_y + ((oy + 1) * h) / new_h;
        if (y1 <= y0) y1 = y0 + 1;
        if (y1 > min_y + h) y1 = min_y + h;

        for (int ox = 0; ox < new_w; ++ox) {
            int x0 = min_x + (ox * w) / new_w;
            int x1 = min_x + ((ox + 1) * w) / new_w;
            if (x1 <= x0) x1 = x0 + 1;
            if (x1 > min_x + w) x1 = min_x + w;

            uint32_t sum = 0;
            uint32_t cnt = 0;

            for (int sy = y0; sy < y1; ++sy) {
                const uint8_t* row = &g_canvas[sy * CANVAS_W];
            ...
            tmp[oy * 20 + ox] = (cnt ? (uint8_t)(sum / cnt) : 0);
        }
    }

    // Paste into the center of 28x28.
    const int off_x = (28 - new_w) / 2;
    const int off_y = (28 - new_h) / 2;

    for (int oy = 0; oy < new_h; ++oy) {
        memcpy(&out[(oy + off_y) * 28 + off_x], &tmp[oy * 20], (size_t)new_w);
    }
}

static void normalize_contrast_28x28(uint8_t img[28 * 28]) {
    uint8_t maxv = 0;
    for (int i = 0; i < 28 * 28; i++) if (img[i] > maxv) maxv = img[i];
    if (maxv == 0 || maxv == 255) return;

    for (int i = 0; i < 28 * 28; i++) {
        img[i] = (uint8_t)(((int)img[i] * 255 + (maxv / 2)) / maxv);
    }
}

static void blur_28x28(uint8_t img[28 * 28]) {
    ...
    for (int y = 0; y < 28; y++) {
        for (int x = 0; x < 28; x++) {
            ...
            if (x > 0)       sum += 2 * img[y * 28 + (x - 1)];
            if (x < 27)      sum += 2 * img[y * 28 + (x + 1)];
            if (y > 0)       sum += 2 * img[(y - 1) * 28 + x];
            if (y < 27)      sum += 2 * img[(y + 1) * 28 + x];

            // diagonal neighbors (weight 1)
            if (x > 0 && y > 0)          sum += img[(y - 1) * 28 + (x - 1)];
            if (x < 27 && y > 0)         sum += img[(y - 1) * 28 + (x + 1)];
            if (x > 0 && y < 27)         sum += img[(y + 1) * 28 + (x - 1)];
            if (x < 27 && y < 27)        sum += img[(y + 1) * 28 + (x + 1)];

            tmp[y * 28 + x] = (uint8_t)((sum + 8) >> 4); // /16 with rounding
        }
    }
    memcpy(img, tmp, 28 * 28);
}
...
int main(void) {
    ...
    if (!fatInitDefault()) {
        iprintf("\x1b[31mFAT init FAILED!\x1b[39m\n");
    ...
    // Scratch buffers for the 28x28 input (allocated once to avoid repeated stack use)
    uint8_t pix28[28*28];
    fix16   input[28*28];
    ...
    while (1) {
        ...
        if (down & KEY_B) {
            clear_canvas(gfx);
            iprintf("\x1b[18;0HCanvas cleared.                 ");
        }
        ...
        if ((down & KEY_A) && net_ok && logits_buf) {
            // Convert the 192x192 drawing ROI to a 28x28 grayscale image, then apply a simple
            // MNIST-style centering/scaling pass. This greatly improves accuracy for freehand digits.
            downsample_28x28(pix28);
            normalize_contrast_28x28(pix28);
            blur_28x28(pix28);
            shift_to_center_of_mass_28x28(pix28);
            pixels_to_input(pix28, input);

            // Clear the results area and run inference.
            console_clear_area(16, 23);
            iprintf("\x1b[17;0H Running inference...              ");

            memset(logits_buf, 0, net.output_dim * sizeof(fix16));
            bool ok = net_forward(&net, input, logits_buf, true);
            
console_clear_line(16); // clear progress line

if (!ok) {
    iprintf("\x1b[17;0H  Inference failed.                 ");
} else {
    // Argmax over the *final* logits (matches Python inference).
    int pred = 0;
    ...
    // For display: undo the final output affine so numbers stay in a "normal" logit range.
    fix16 disp[64];
    fix16 scale = net.output_scale;
    fix16 bias  = net.output_bias;
    if (scale != 0) {
        for (uint32_t i = 0; i < net.output_dim && i < 64; ++i) {
            disp[i] = fix_div(fix_sub(logits_buf[i], bias), scale);
        }
    } else {
        for (uint32_t i = 0; i < net.output_dim && i < 64; ++i) {
            disp[i] = logits_buf[i];
    ...
    iprintf("\x1b[16;0H OutScale:");
    iprint_fix16_4(net.output_scale);
    iprintf("             ");

    iprintf("\x1b[17;0H  >>> PREDICTED: %d <<<            ", pred);
    print_logits_two_column(disp, net.output_dim);
    iprintf("\x1b[23;0H Ready! Draw a digit, press A.      ");
}
        }
        else if ((down & KEY_A) && !net_ok) {
            iprintf("\x1b[18;0HNo weights loaded!              ");
        }
    }
