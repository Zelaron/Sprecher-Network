...
def plot_network_structure_ax(ax, layers, input_dim):
    ...
    This version covers the cases of one or two hidden layers.
    """
    # Determine number of hidden layers (excluding final output layer, if present)
    if len(layers) == 0:
        num_hidden = 0
    elif layers[-1].is_final:
        num_hidden = len(layers) - 1
    else:
        num_hidden = len(layers)
    
    # Set x positions based on number of hidden layers.
    if num_hidden == 1:
        layer_x = [0.2, 0.5, 0.8]  # input, hidden, output
    elif num_hidden == 2:
        layer_x = [0.2, 0.4, 0.6, 0.8]  # input, hidden1, hidden2, output
    else:
        total_layers = num_hidden + 2
        layer_x = np.linspace(0.2, 0.8, total_layers).tolist()
    ...
    for i, y in enumerate(input_y):
        ...
        # For the first hidden layer, display lambda (from first column) above the input nodes.
        if num_hidden >= 1:
            lambda_val = layers[0].lambdas[i, 0].item() if i < layers[0].lambdas.shape[0] else 0.0
            ax.text(layer_x[0] + 0.05, y + 0.0125, f'$\\lambda_{{{i+1}}}={lambda_val:.3f}$', ha='center', va='bottom', fontsize=10)
    
    # Plot hidden layers and output.
    if num_hidden == 1:
        # One hidden layer: first hidden block is layers[0], final block is layers[1]
        hidden_nodes = layers[0].d_out
        hidden_y = np.linspace(0.2, 0.8, hidden_nodes)
        for i, y in enumerate(hidden_y):
            ax.scatter(layer_x[1], y, c='blue', s=100)
            ax.text(layer_x[1] + 0.05, y, f'$q_{{{i}}}$', ha='left', fontsize=10)
            for in_y in input_y:
                ax.plot([layer_x[0], layer_x[1]], [in_y, y], 'k-', alpha=0.5)
        # Output node.
        ax.scatter(layer_x[2], 0.5, c='red', s=100)
        ax.text(layer_x[2] + 0.05, 0.5, 'output', ha='left', fontsize=12)
        for y in hidden_y:
            ax.plot([layer_x[1], layer_x[2]], [y, 0.5], 'k-', alpha=0.5)
        mid_x1 = (layer_x[0] + layer_x[1]) / 2
        mid_x2 = (layer_x[1] + layer_x[2]) / 2
        ax.text(mid_x1, 0.15, '$\\varphi$', ha='center', fontsize=14)
        ax.text(mid_x2, 0.15, '$\\Phi$', ha='center', fontsize=14)
        eta_val = layers[0].eta.item()
        ax.text(layer_x[2] + 0.05, 0.15, f'$\\eta={eta_val:.3e}$', ha='left', fontsize=10)
    elif num_hidden == 2:
        # Two hidden layers: layers[0] and layers[1] are hidden; final output is layers[2]
        hidden1_nodes = layers[0].d_out
        hidden1_y = np.linspace(0.2, 0.8, hidden1_nodes)
        for i, y in enumerate(hidden1_y):
            ax.scatter(layer_x[1], y, c='blue', s=100)
            ax.text(layer_x[1] + 0.02, y, f'$q^1_{{{i}}}$', ha='left', fontsize=10)
            for in_y in input_y:
                ax.plot([layer_x[0], layer_x[1]], [in_y, y], 'k-', alpha=0.5)
        hidden2_nodes = layers[1].d_out
        hidden2_y = np.linspace(0.2, 0.8, hidden2_nodes)
        for i, y in enumerate(hidden2_y):
            ax.scatter(layer_x[2], y, c='blue', s=100)
            ax.text(layer_x[2] + 0.02, y, f'$q^2_{{{i}}}$', ha='left', fontsize=10)
            for h1_y in hidden1_y:
                ax.plot([layer_x[1], layer_x[2]], [h1_y, y], 'k-', alpha=0.5)
        # Output node.
        ax.scatter(layer_x[3], 0.5, c='red', s=100)
        ax.text(layer_x[3] + 0.02, 0.5, 'output', ha='left', fontsize=12)
        for y in hidden2_y:
            ax.plot([layer_x[2], layer_x[3]], [y, 0.5], 'k-', alpha=0.5)
        mid_x1 = (layer_x[0] + layer_x[1]) / 2
        mid_x2 = (layer_x[1] + layer_x[2]) / 2
        mid_x3 = (layer_x[2] + layer_x[3]) / 2
        ax.text(mid_x1, 0.1, '$\\varphi_1$', ha='center', fontsize=14)
        ax.text(mid_x2, 0.1, '$\\varphi_2$', ha='center', fontsize=14)
        ax.text(mid_x3, 0.1, '$\\Phi_2$', ha='center', fontsize=14)
        eta_val = layers[1].eta.item()
        ax.text(layer_x[3] + 0.02, 0.1, f'$\\eta={eta_val:.3e}$', ha='left', fontsize=10)
    else:
        # Fallback for no hidden layers or other cases.
        ax.scatter(layer_x[-1], 0.5, c='red', s=100)
        ax.text(layer_x[-1] + 0.05, 0.5, 'output', ha='left', fontsize=12)
        for in_y in input_y:
            ax.plot([layer_x[0], layer_x[-1]], [in_y, 0.5], 'k-', alpha=0.5)
        mid_x = (layer_x[0] + layer_x[-1]) / 2
        ax.text(mid_x, 0.15, '$f$', ha='center', fontsize=14)
...
def plot_results(model, layers):
    ...
    # Top row, first column: Updated Network Structure with Parameters
