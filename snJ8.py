...
# Set HIDDEN_NODES to a specific integer to override the default (2 * input_dim + 1).
# If set to None, the default behavior (2n+1, where n is the number of input nodes) is used.
HIDDEN_NODES = None
...
class SprecherLayer(nn.Module):
    def __init__(self, input_dim=2, hidden_dim=None, in_range=(0, 1), out_range=(0, 1)):
        ...
        # Reduced the number of knots to avoid overly jagged behavior
        # Keep phi monotonic
        self.phi = SimpleSpline(num_knots=300, in_range=in_range, out_range=in_range, monotonic=True)
        # Reduced the number of knots for Phi as well
...
def train_network_iterative(target_function, n_layers=3, total_epochs=100000, print_every=10000, device="cpu"):
    ...
    for r in range(n_layers):
        ...
        # Pass the global HIDDEN_NODES variable into the layer.
        layer = SprecherLayer(input_dim=input_dim, hidden_dim=HIDDEN_NODES).to(device)
        
        # Lower learning rate for smoother training
        optimizer = torch.optim.Adam(layer.parameters(), lr=0.0003, weight_decay=1e-7)
        ...
        for epoch in pbar:
            ...
            # Avoid NaN by checking if lambdas has more than one element
            if layer.lambdas.numel() > 1:
                lambda_reg = torch.mean((layer.lambdas[1:] - layer.lambdas[:-1]) ** 2)
            else:
                lambda_reg = 0.0
            
            # Increased flatness penalties to encourage smoother splines
            ...
            loss = (
                mse_loss
                + 0.01 * lambda_reg
                + 0.01 * flatness_penalty_phi
                + 0.1 * flatness_penalty_Phi
            )
            loss.backward()
            optimizer.step()
...
def plot_results(model, layers):
    ...
    for i, y in enumerate(input_y):
        ...
        plt.text(layer_x[0] - 0.05, y, f'x_{i+1}', ha='right')
        # Show lambda values slightly above nodes but not too high
        lambda_val = layers[0].lambdas[i].item()
        plt.text(layer_x[0] + 0.05, y + 0.0125, f'λ_{i+1}={lambda_val:.3f}', ha='center', va='bottom')

    # Plot hidden nodes with q values
    hidden_y = np.linspace(0.2, 0.8, layers[0].hidden_dim)
    for i, y in enumerate(hidden_y):
        plt.scatter([layer_x[1]], [y], c='blue', s=100)
        plt.text(layer_x[1] + 0.05, y, f'q={i}', ha='left')
        # Draw connections from inputs to hidden node
        for j, in_y in enumerate(input_y):
            plt.plot([layer_x[0], layer_x[1]], [in_y, y], 'k-', alpha=0.5)

    # Plot output node
    plt.scatter([layer_x[2]], [0.5], c='red', s=100)
    plt.text(layer_x[2] + 0.05, 0.5, 'output', ha='left')
    
    # Draw connections to output
    for h_y in hidden_y:
        plt.plot([layer_x[1], layer_x[2]], [h_y, 0.5], 'k-', alpha=0.5)

    # Show function labels at proper horizontal centers
    mid_x1 = (layer_x[0] + layer_x[1]) / 2
    mid_x2 = (layer_x[1] + layer_x[2]) / 2
    # φ for input->hidden edges
    plt.text(mid_x1, 0.15, 'φ', ha='center')
    # Φ for hidden->output edges
    plt.text(mid_x2, 0.15, 'Φ', ha='center')

    # Show η value at the same height as φ/Φ, right-aligned with 'output' text
    eta_val = layers[0].eta.item()
    plt.text(layer_x[2] + 0.05, 0.15, f'η={eta_val:.3e}', ha='left')
...
combined_forward, all_losses, layers = train_network_iterative(
    target_function, n_layers=1, total_epochs=100000, print_every=10000, device=device
)
