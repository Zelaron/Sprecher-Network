...
This recovers the structure of Sprecher's formula when $L=1$, but generalized to multiple layers for $L>1$. If we introduce the operator
$$T^{(\ell)}: \mathbb{R}^{d_{\ell-1}} \to \mathbb{R}^{d_\ell},\quad \text{defined by}\quad \bigl(T^{(\ell)}(z)\bigr)_q=\Phi^{(\ell)}\Bigl(\sum_{i=1}^{d_{\ell-1}}\lambda^{(\ell)}_{i,q}\,\phi^{(\ell)}\bigl(z_i+\eta^{(\ell)}\,q\bigr)+q\Bigr),$$
with the convention $\mathbf{h}^{(0)}=\mathbf{x}$, then we can write
\begin{equation}
...
\item \textbf{2D case:} A surface such as $f(x,y)=\frac{1}{7}(\exp(\sin(\pi x)+y^2) - 1)$. Once trained for sufficient epochs, good approximation is generally observed after $O(10^5)$ gradient updates. Layerwise spline plots offer an interpretable view of how the network transforms inputs.
\end{itemize}
