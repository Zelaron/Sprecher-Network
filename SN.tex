...
\begin{itemize}
    \item \textbf{Parameters:} Each block includes the learnable scalar shift $\eta^{(\ell)} > 0$ and the learnable mixing weight matrix $\lambda^{(\ell)} \in \mathbb{R}^{d_{\ell-1} \times d_\ell}$. The shift parameter $\eta^{(\ell)}$ is theoretically required to be positive ($\eta > 0$) in Sprecher's original construction. In practice, it is typically initialized to a positive value (e.g., $1/(d_{\text{out}} + 10)$) and often remains positive throughout training due to appropriate initialization and learning rates, though explicit enforcement through transformations like softplus or absolute value could ensure theoretical consistency.
...
Summing over $L$ (or $L+1$) blocks, plus output scaling parameters and potential residual connection weights, the total parameter count scales approximately as:
        $$ O(LN^2 + LG + L + m).$$
        where the $L$ term accounts for shift parameters $\eta^{(\ell)}$, the $m$ term represents output scaling parameters (2 for scalar output, potentially $2m$ for vector output if applied per dimension), and residual connection parameters (when used) are absorbed into the $LN^2$ term when dimensions differ or add at most $L$ parameters when dimensions match. In practice, these additional terms are negligible compared to the dominant $LN^2$ and $LG$ terms.
\end{itemize}
...
We present some illustrative calculations, based on examples in \cite{liu2024kan}, to highlight the potential parameter efficiency of SNs. Note that achieving comparable performance may require different architectural choices, as the optimal depth-width tradeoffs for SNs may differ from those of KANs due to the shared spline structure.
