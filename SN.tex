...
$$ (x_i)_{i=1}^{d_{\mathrm{in}}} \;\mapsto\; \Bigl[\Phi\Bigl(\sum_{i=1}^{d_{\mathrm{in}}}\lambda_{i}\,\phi(x_i+\eta\,q)+\alpha q\Bigr)\Bigr]_{q=0}^{d_{\mathrm{out}}-1}.$$
...
When batch normalization is applied, it can be positioned either before or after each Sprecher block:
\begin{itemize}
    \item \textbf{After block} (default): Normalization is applied to $\mathbf{h}^{(\ell)} \in \mathbb{R}^{d_\ell}$. For a final block whose outputs are summed to produce a scalar, the normalization effectively applies to a single value (using $\mathrm{num_features}=1$).
    \item \textbf{Before block}: Normalization is applied to the input $\mathbf{h}^{(\ell-1)} \in \mathbb{R}^{d_{\ell-1}}$ before it enters the block.
\end{itemize}
...
While universality guarantees that an approximation exists, it doesn't quantify how the error behaves as the approximating functions (splines) become more refined. The following lemmas and theorem address the approximation rate achievable by replacing the ideal continuous functions in the Sprecher structure with finite-resolution piecewise-linear splines, assuming such an ideal network exists for the target function $f$.
...
$$T(\mathbf{z})_q = \Phi\left(\sum_{i=1}^{d_{in}} \lambda_{i} \phi(z_i + \eta q) + \alpha q + \tau \sum_{j \in \mathcal{N}(q)} \omega_{q,j} s_j\right), \quad q = 0, \ldots, d_{out}-1$$
where $\phi \in C^{2}(\mathbb{R})$ is monotonic, $\Phi \in C^{2}(\mathbb{R})$, $|\lambda_{i}| \leq \Lambda$, $\eta \in \mathbb{R}$, and $\tau, \omega_{q,j}$ are the lateral mixing parameters.
...
\item $I_\Phi := [-R_{out}, R_{out}]$ where $R_{out} := (1 + |\tau| \|\omega\|_\infty |\mathcal{N}|_{max})(\Lambda d_{in} \|\phi\|_{L^\infty(I_\phi)} + \alpha d_{out})$ (domain accounting for lateral mixing)
\end{itemize}

Let $\hat{\phi}$ be a monotonic piecewise-linear spline with $G_\phi$ uniformly spaced knots on $I_\phi$, and $\hat{\Phi}$ be a piecewise-linear spline with $G_\Phi$ uniformly spaced knots on $I_\Phi$. If
$$\|\phi - \hat{\phi}\|_{L^\infty(I_\phi)} \leq \frac{\|\phi^{(2)}\|_{L^\infty(I_\phi)}}{8} \left(\frac{|I_\phi|}{G_\phi}\right)^{2}$$
$$\|\Phi - \hat{\Phi}\|_{L^\infty(I_\Phi)} \leq \frac{\|\Phi^{(2)}\|_{L^\infty(I_\Phi)}}{8} \left(\frac{|I_\Phi|}{G_\Phi}\right)^{2}$$
then the approximate block $\hat{T}$ using $\hat{\phi}, \hat{\Phi}$ satisfies
$$\sup_{\mathbf{z} \in \mathcal{B}_{in}} \|T(\mathbf{z}) - \hat{T}(\mathbf{z})\|_\infty \leq K_T \max\left\{\left(\frac{|I_\phi|}{G_\phi}\right)^{2}, \left(\frac{|I_\Phi|}{G_\Phi}\right)^{2}\right\}$$
where 
$$K_T := \frac{L_\Phi \Lambda d_{in} (1 + |\tau| \|\omega\|_\infty |\mathcal{N}|_{max}) \|\phi^{(2)}\|_{L^\infty(I_\phi)} + \|\Phi^{(2)}\|_{L^\infty(I_\Phi)}}{8}$$
and $L_\Phi$ is the Lipschitz constant of $\Phi$ on $I_\Phi$.
\end{lemma}

\begin{proof}
Fix $\mathbf{z} \in \mathcal{B}_{in}$ and $q \in \{0, \ldots, d_{out}-1\}$. Define:
$$s_q := \sum_{i=1}^{d_{in}} \lambda_{i} \phi(z_i + \eta q) + \alpha q, \quad \hat{s}_q := \sum_{i=1}^{d_{in}} \lambda_{i} \hat{\phi}(z_i + \eta q) + \alpha q$$

Since $z_i \in [-B_{in}, B_{in}]$ and $\eta q \in [-|\eta|(d_{out}-1), |\eta|(d_{out}-1)]$, we have $z_i + \eta q \in I_\phi$. 

For piecewise-linear spline approximation of a $C^2$ function on an interval $I$ with $G$ uniformly spaced knots, the standard approximation bound gives:
$$\|\phi - \hat{\phi}\|_{L^\infty(I_\phi)} \leq \frac{\|\phi^{(2)}\|_{L^\infty(I_\phi)}}{8} \left(\frac{|I_\phi|}{G_\phi}\right)^{2}$$

This follows from the fact that on each subinterval of length $h = |I_\phi|/G_\phi$, the error of linear interpolation for a $C^2$ function is bounded by $\frac{1}{8}h^2 \max|\phi''|$.
...
&\leq \Lambda d_{in} \cdot \frac{\|\phi^{(2)}\|_{L^\infty(I_\phi)}}{8} \left(\frac{|I_\phi|}{G_\phi}\right)^{2}
\end{align*}

Moreover, $|s_q| \leq \Lambda d_{in} \|\phi\|_{L^\infty(I_\phi)} + \alpha |q| \leq \Lambda d_{in} \|\phi\|_{L^\infty(I_\phi)} + \alpha d_{out}$. 

When lateral mixing is present with parameters $\tau, \omega$, the mixed value $\tilde{s}_q$ satisfies:
$$|\tilde{s}_q| \leq |s_q| + |\tau| \|\omega\|_\infty |\mathcal{N}|_{max} \max_j |s_j| \leq (1 + |\tau| \|\omega\|_\infty |\mathcal{N}|_{max})(\Lambda d_{in} \|\phi\|_{L^\infty(I_\phi)} + \alpha d_{out}) = R_{out}$$

Thus $\tilde{s}_q \in I_\Phi = [-R_{out}, R_{out}]$. Similarly, since $\hat{\phi}$ is bounded on $I_\phi$ (as a continuous function on a compact set), $\hat{\tilde{s}}_q \in I_\Phi$ for sufficiently large $G_\phi$.

Now, for the outer spline:
\begin{align*}
|T(\mathbf{z})_q - \hat{T}(\mathbf{z})_q| &= |\Phi(\tilde{s}_q) - \hat{\Phi}(\hat{\tilde{s}}_q)|\\
&\leq |\Phi(\tilde{s}_q) - \Phi(\hat{\tilde{s}}_q)| + |\Phi(\hat{\tilde{s}}_q) - \hat{\Phi}(\hat{\tilde{s}}_q)|\\
&\leq L_\Phi |\tilde{s}_q - \hat{\tilde{s}}_q| + \|\Phi - \hat{\Phi}\|_{L^\infty(I_\Phi)}
\end{align*}

The lateral mixing preserves the error bound up to the factor $(1 + |\tau| \|\omega\|_\infty |\mathcal{N}|_{max})$:
$$|\tilde{s}_q - \hat{\tilde{s}}_q| \leq (1 + |\tau| \|\omega\|_\infty |\mathcal{N}|_{max}) |s_q - \hat{s}_q|$$

Combining these bounds:
\begin{align*}
|T(\mathbf{z})_q - \hat{T}(\mathbf{z})_q| &\leq L_\Phi (1 + |\tau| \|\omega\|_\infty |\mathcal{N}|_{max}) \Lambda d_{in} \frac{\|\phi^{(2)}\|_{L^\infty(I_\phi)}}{8} \left(\frac{|I_\phi|}{G_\phi}\right)^{2} \\
&\quad + \frac{\|\Phi^{(2)}\|_{L^\infty(I_\Phi)}}{8} \left(\frac{|I_\Phi|}{G_\Phi}\right)^{2}\\
&\leq K_T \max\left\{\left(\frac{|I_\phi|}{G_\phi}\right)^{2}, \left(\frac{|I_\Phi|}{G_\Phi}\right)^{2}\right\}
\end{align*}

where $K_T = \frac{L_\Phi \Lambda d_{in} (1 + |\tau| \|\omega\|_\infty |\mathcal{N}|_{max}) \|\phi^{(2)}\|_{L^\infty(I_\phi)} + \|\Phi^{(2)}\|_{L^\infty(I_\Phi)}}{8}$.
...
\begin{theorem}[Spline approximation rate for Sprecher Networks with piecewise-linear splines]\label{thm:sprecher_rate}
Let $f:[0,1]^n \to \mathbb{R}$ be realized by an ideal $L$-block Sprecher network with scalar output. Assume:
\begin{enumerate}[label=\textup{(\roman*)}]
\item Each $\phi^{(\ell)}, \Phi^{(\ell)} \in C^{2}$ on their respective domains.
\item Each block satisfies $|\lambda_{i}^{(\ell)}| \leq \Lambda$, has shift parameter $\eta^{(\ell)} \in \mathbb{R}$, and if lateral mixing is enabled, $|\tau^{(\ell)}| \leq T$ and $\|\omega^{(\ell)}\|_\infty \leq W$.
\item The network propagates through bounded sets: $\mathbf{h}^{(\ell)}(\mathbf{x}) \in [-B_\ell, B_\ell]^{d_\ell}$ for all $\mathbf{x} \in [0,1]^n$.
\end{enumerate}

For each block $\ell$, approximate $\phi^{(\ell)}$ and $\Phi^{(\ell)}$ with piecewise-linear splines using $G_\phi^{(\ell)}$ and $G_\Phi^{(\ell)}$ knots respectively. Then:
...
If all blocks use the same number of knots $G$ (i.e., $G_\phi^{(\ell)}=G_\Phi^{(\ell)}=G$ for all $\ell$), the error scales as $O(G^{-2})$ with constants that may grow exponentially with depth $L$.
\end{theorem}
...
The approximation rates in Lemma \ref{lem:single_block} assume we can achieve standard spline approximation rates while maintaining monotonicity for $\phi^{(\ell)}$. Practical implementations use piecewise linear splines with a log-space parameterization that maintains monotonicity by construction.
\end{remark}
...
\item \textbf{Out-of-domain extension:} When inputs fall outside the spline's domain:
   \begin{itemize}
   \item $\phi$ (monotonic): Uses constant extension (outputs stay at 0 or 1)
   \item $\Phi$ (general): Uses linear extrapolation with boundary slopes, applied consistently when trainable codomains are active
   \end{itemize}
\end{enumerate}
\end{lemma}
...
\begin{algorithmic}[1]
    ...
    \For{each block $\ell = 1, \ldots, L$}
        ...
        \If{per-dimension intervals available}
            \State Compute per-$q$ bounds using Corollary
        \Else
        ...
        \EndIf
        \If{normalization is applied after block $\ell$}
            \State Apply normalization bounds (training: conservative $[-4,4]$; eval: use running stats)
        \EndIf
    \EndFor
\end{algorithmic}
\end{algorithm}
