...
\begin{remark}[Biological analogy for $\Phi$-shapes]
Several learned $\Phi^{(\ell)}$ splines exhibit a sharp central peak flanked by dips, e.g. as seen for $\Phi^{(1)}$ from Block~1 in Figure~\ref{fig:twovarsprecher_revised}. This shape resembles biphasic neural waveforms, such as extracellular spike templates or synaptic kernels. In neurophysiology, similar features arise from \emph{inhibitory post-synaptic potentials (IPSPs)} before a spike and \emph{afterhyperpolarizations (AHPs)} following it. Related motifs also appear in early sensory systems, for example, biphasic temporal filters and center-surround receptive fields emphasize changes and contrast. The recurrence of such structures in learned splines suggests that Sprecher Networks may naturally discover biologically relevant signal-processing patterns when approximating structured functions.
\end{remark}
