...
allows for the exact computation of output ranges by simply finding the extrema of the spline's values at its knots. Importantly, this computation is performed not just at initialization but at every training iteration as the parameters $\lambda^{(\ell)}$ and $\eta^{(\ell)}$ evolve, ensuring that splines always operate on their natural domains:
...
The shortcomings of naive rescaling motivate a more sophisticated approach: a \textbf{function-preserving resampling} strategy that is applied differently to the two types of splines, reflecting their distinct roles:
...
\section{Conclusion}
We have introduced Sprecher Networks (SNs), a trainable architecture built by re-imagining the components of David Sprecher's 1965 constructive proof as the building blocks for a modern deep learning model. By composing functional blocks that utilize shared monotonic and general splines, learnable mixing weights, and explicit shifts, SNs offer a distinct approach to function approximation that differs fundamentally from MLPs, KANs, and other existing architectures. Our central goal has not been to formally generalize Sprecher's theorem, but to investigate the empirical consequences of extending its shallow, highly-structured formula into a deep, compositional framework.

The key contribution of this work is this methodological exploration. While Sprecher's theorem guarantees universality for a shallow network, the practical power of deep learning has always stemmed from the empirical benefits of depth. Our findings suggest that these benefits can extend even to this highly constrained, theorem-inspired architecture. Deep compositions of Sprecher blocks can be surprisingly effective, and they come with a remarkable parameter efficiency that scales as $O(LN + LG)$ rather than the $O(LN^2)$ of MLPs or $O(LN^2G)$ of KANs. This efficiency is a direct result of adhering to Sprecher's use of shared splines and weight vectors rather than matrices.
