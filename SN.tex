...
The composition of these blocks and the final output generation depend on the desired final output dimension $m=d_{\mathrm{out}}$. Additionally, the network may include learnable output scaling parameters $\gamma$ (scale) and $\beta$ (bias) applied to the final output, such that the actual network output is $\gamma \cdot f(\mathbf{x}) + \beta$, where $f(\mathbf{x})$ is the output from the block composition described below. These parameters can be particularly useful for optimization, with appropriate initialization strategies (e.g., initializing $\beta$ to match target statistics and $\gamma$ to control initial output magnitudes):
...
\medskip
In summary: for an architecture with $L$ hidden layers, a scalar-output SN uses $L$ blocks and $2L$ shared splines (plus 2 output scaling parameters). A vector-output SN (with $m>1$) uses $L+1$ blocks and $2(L+1)$ shared splines (plus $2m$ output scaling parameters when applied per dimension, or 2 when applied globally). This structure provides a natural extension of Sprecher's original scalar formula to the vector-valued setting.
...
\begin{itemize}
    ...
    \item \textbf{Loss function:} Typically Mean Squared Error (MSE) for regression tasks, combined with regularization terms:
        ...
        where $w$ are weights for flatness penalty, sparsity penalty on $\lambda$ (e.g., L1 + entropy), and potentially a variance regularization term to match output variance to target variance. The variance term $\mathcal{L}_{\text{variance}} = (\sigma_{\text{output}} - \sigma_{\text{target}})^2$ encourages the network output to have similar variability to the target function.
\end{itemize}
...
Summing over $L$ (or $L+1$) blocks, plus output scaling parameters and potential residual connection weights, the total parameter count scales approximately as:
        $$ O(LN^2 + LG + L + m).$$
        where the $L$ term accounts for shift parameters $\eta^{(\ell)}$, the $m$ term represents output scaling parameters (2 for scalar output, potentially $2m$ for vector output if applied per dimension, or 2 when applied globally), and residual connection parameters (when used) are absorbed into the $LN^2$ term when dimensions differ or add at most $L$ parameters when dimensions match. In practice, these additional terms are negligible compared to the dominant $LN^2$ and $LG$ terms.
\end{itemize}
