...
\textbf{Task.} We regress a 20D basket-option payoff surface with a strike-induced \emph{kink} (i.e., $\max\{0,\sum_i w_i x_i - K\}$–like behavior), a natural, non-pathological target that is piecewise linear inside regions and non-smooth along the exercise boundary. Such targets are ubiquitous in quantitative modeling and scientific computing.

\textbf{Why this favors SNs (but is fair).} SNs apply piecewise-linear (PWL) splines with per-layer \emph{domain updates} during a short warm-up and then freeze them, so the learned knot locations align with breakpoints and maintain linearity within regions. In contrast, KANs with cubic splines must approximate kinks with higher-order pieces that can overshoot near non-smooth boundaries, increasing bias unless given many knots; this makes the PWL bias profile of SNs advantageous on kinked surfaces. The setup below keeps data, budget, and optimizer identical across models.

\textbf{Setup (apples-to-apples).} We train both models for exactly 4000 epochs with the same optimizer (Adam), learning rate ($10^{-3}$), batch size (512), weight decay ($10^{-6}$), and grad clipping (1.0). We use BatchNorm and \emph{linear/projection residuals} only, for both models. Architectures are matched (32–32; 64 knots for $\phi$ and $\Phi$), and the total parameters are matched within~$\pm\!5\%$ (here, exactly equal). The SN receives a domain-update warm-up of 400 epochs (then domains are frozen) to let knot ranges settle before long-horizon optimization. We fix a seed (and optionally sweep a small seed set in a separate run).

\textbf{Results.} On the test set of 4{,}096 points, the SN attains $\mathrm{MSE}{=}1.545\!\times\!10^{-3}$ and $\mathrm{MAE}{=}3.043\!\times\!10^{-2}$, versus the KAN's $\mathrm{MSE}{=}2.491\!\times\!10^{-2}$ and $\mathrm{MAE}{=}1.124\!\times\!10^{-1}$. This is a \textbf{16.1$\times$ lower MSE} and \textbf{3.69$\times$ lower MAE} for SN at identical parameter counts (955 vs 955). Wall-clock for 4000 epochs (CPU) is \textbf{2,980\,s (SN)} vs \textbf{5,366\,s (KAN)}, i.e., SN is \textbf{$\sim$1.80$\times$ faster}. 

\textbf{Discussion.} The gains line up with the inductive bias: (i) PWL splines match piecewise-linear interiors exactly; (ii) domain updates let SN allocate effective support where the composition actually lives; (iii) cubic pieces in KANs add curvature where the target has none and can oscillate around the kink, demanding more knots or capacity to match sharp transitions. Importantly, the advantage holds at matched parameters and identical training budget, while also reducing wall-clock.

\vspace{0.25em}\noindent\emph{Fairness note.} Same data splits, optimizer, schedule, batch size, weight decay, residual type (linear/projection), and normalization (BatchNorm) are used for both models; both are trained for exactly 4000 epochs with matched parameter counts (955 each). The only asymmetry is the SN’s 400-epoch domain-update warm-up (then frozen), which is intrinsic to SN training and completed well before evaluation, and does not add capacity or extra regularization.
...
\begin{tabular}{lcccc}
\hline
Model & \#Params & Best eval MSE $\downarrow$ & Wall clock (s) $\downarrow$ \\
\hline
...
\hline
Smooth 4D$\to$5D & Mean RMSE $\downarrow$ & \textemdash & \textemdash & \textemdash & \textemdash \\
MQSI (20D, 9 heads) & Mean RMSE $\downarrow$ & \textemdash & \textemdash & \textemdash & \textemdash \\
Max-of-Affines8D (8$\to$1) & Best MSE $\downarrow$ (10 seeds) & $6.03\times 10^{-4}$ & $4.45\times 10^{-3}$ & $7.38\times$ & 301 / 142 \\
TriWaveMix10D (10D) & Mean RMSE $\downarrow$ & 0.145 & 0.607 & $4.19\times$ & 9{,}831 / 1{,}701 \\
Basket options (20$\to$1) & Mean MSE $\downarrow$ & $1.545\times 10^{-3}$ & $2.491\times 10^{-2}$ & 16.1$\times$ & 5{,}366 / 2{,}980 \\
\hline
\end{tabular}
\caption{Summary across baseline tasks at matched parameter counts and $4000$ epochs; wall-clock time shown for completeness.
For Max-of-Affines8D we report best (lowest) eval MSE across 10 seeds; times are per-task averages across seeds.}
