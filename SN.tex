...
If all blocks use the same number of knots $G$, the error scales as $O(G^{-(k+1)})$ with a constant that may grow exponentially in $L$.
\end{theorem}

\begin{proof}
By Lemma \ref{lem:single_block}, each block $\ell$ can be approximated with error bounded by $K_{T^{(\ell)}} \varepsilon_\ell$. By Lemma \ref{lem:error_composition}, the error after $L$ blocks is bounded by
$$E_L \leq \sum_{j=1}^{L} \left(\prod_{m=j+1}^{L} \tilde{L}_m\right) K_{T^{(j)}} \varepsilon_j$$
