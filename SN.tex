...
\subsubsection*{Tri-wave mixture with sparse interactions (\texttt{TriWaveMix10D})}
\textbf{Task.} Scalar regression on a 10D input with a natural mix of smooth and piecewise-linear periodic components:
\[
\operatorname{tri}(u)=2\!\left|\,\bigl(u\bmod 1\bigr)-\tfrac12\,\right|,\qquad
f(x)=0.5\,\operatorname{tri}\!\bigl(4\,w_1^\top x_{1:3}\bigr)+0.3\,\operatorname{tri}\!\bigl(3\,w_2^\top x_{4:7}\bigr)+0.2\,\sin\!\bigl(2\pi\,w_3^\top x_{8:10}\bigr),
\]
with $x\!\in\![0,1]^{10}$, and $w_1,w_2,w_3$ fixed unit vectors. The target combines two \emph{triangular} waves (piecewise linear, kinked) and one sinusoid (smooth), with mild anisotropy via disjoint coordinate groups.

\textbf{Why this can favor SNs (but remains fair).} The inner/outer maps in an SN are piecewise-linear splines whose kinks align well with the triangular-wave components; cubic KAN splines, at a matched knot budget, tend to oversmooth near the kinks and require more knots per dimension to approximate the corners faithfully. The mixture also includes a smooth sinusoid, preventing the task from being “tailored” to any one model while still being a natural stress test of kink fidelity and anisotropic coupling.

\textbf{Metrics.} We report \emph{best} held-out MSE over the $4000$ training epochs and single-run wall clock (seconds) on CPU; both models share the same train/eval splits and loss (MSE).
...
\begin{tabular}{lcccc}
\hline
Model & \#Params & Eval MSE $\downarrow$ & Wall clock (s) $\downarrow$ \\
\hline
SN (PWL splines; domain warm-up 400ep) & 977 & 0.0210 & 1701 \\
KAN (cubic splines) & 977 & 0.3687 & 9831 \\
\hline
\end{tabular}
\caption{\texttt{TriWaveMix10D} apples-to-apples comparison at matched parameter count and $4000$ epochs. SN attains a $\sim\!17.6\times$ lower MSE and $\sim\!5.78\times$ faster wall clock than KAN on CPU.}
\label{tab:triwavemix10d}
\end{table}
\vspace{-1ex}

\noindent\emph{Fairness \& setup.} Both models train for exactly $4000$ epochs with Adam ($\mathrm{lr}{=}3\times10^{-4}$) and shared batch size and weight decay ($10^{-7}$). Residuals are standard linear/projection in both; BatchNorm is placed \emph{after} blocks and evaluated with \texttt{batch\_no\_update} semantics. No lateral mixing or extra normalizers are used. SN’s spline domains update for the first $400$ epochs (warm-up) and are then frozen; KAN’s knot budget is set to match the total parameter count exactly.

\subsubsection{Monotone single-index with quantiles (MQSI; 20D, $m{=}9$ heads)}
\label{sssec:mqsi_20d_9h_parity}

\paragraph{Setup.}
Targets are $y_j(x)=\tanh\!\big(\beta(\mu(x)+\sigma(x)z_j)\big)$ with strictly increasing quantile indices $z_j=\Phi^{-1}(\tau_j)$, and $\mu,\sigma$ sums of monotone per–coordinate transforms plus small positive pairwise terms (guaranteeing coordinatewise and headwise monotonicity). We train an SN and a KAN under strict parity: SN arch $[24,24]$ with $\phi/\Phi$ knot counts $85/84$, residuals ``linear'', BN (batch, after, skip\_first), no lateral; KAN arch $[4,4]$, degree $3$, residual ``linear'', outside ``linear'', BN (batch, after, skip\_first), impl=fast. Parameters are equalized via $K{=}8$ so both models have $1{,}349$ trainables. Training uses $4{,}000$ epochs on the same $1{,}024$ train points (SN’s generator) and a fixed test set of $50{,}000$ points; device=CPU; seeds $s\in\{0,\dots,9\}$.

\paragraph{Metrics.}
Primary: \emph{mean RMSE across heads} (lower is better). Secondary: Frobenius error between target/prediction head–correlation matrices, and the monotonicity–violation rate across heads.
...
\subsubsection{Max-of-Affines (8D$\to$1D) regression: piecewise-linear target, PWL~SN vs.\ cubic~KAN (10 seeds)}
\label{sssec:max-of-affines-8d}

\paragraph{Setup.}
We regress a \emph{convex, piecewise-linear} function $f:\mathbb{R}^8\!\to\!\mathbb{R}$ constructed as the maximum of $48$ random affine pieces (dataset: \texttt{MaxOfAffines8D}).\footnote{Convex max-of-affines appears in control/robust modeling and matches ReLU networks’ implicit geometry; it is \emph{not} tailored to Sprecher Networks (SNs) but is unequivocally piecewise linear.}
Both models use the same 3-layer width-\{8,8,8\} backbone, cubic splines for KAN and piecewise-linear splines with domain updates for SN, with identical knot counts per layer ($\phi{=}\Phi{=}64$). We fix the training budget at \textbf{4000 epochs}, use only linear/projection residuals and batch norm (no attention or extra mixing), and warm up SN’s domain updates for \textbf{400 epochs} (then freeze). Parameter counts are matched (\texttt{SN=434, KAN=434}). Optimizer, LR schedule, batch size, weight decay, splits, and losses are identical.

\paragraph{Why this favors SN (but fairly).}
The target is exactly piecewise linear with sparse kink sets; SN’s PWL spline basis can represent it \emph{without} curvature, whereas a cubic-spline KAN must approximate kinks by smoothing. This mirrors fitting ReLU‑type ground truth: a PWL hypothesis class should need fewer effective degrees of freedom to reach the same error. The task is natural (convex max-of-affines), not bespoke.

\paragraph{Results across 10 seeds.}
SN attains lower final eval MSE than KAN in \textbf{8/10} runs (wins all but seeds 4 and 9). Examples:
SN(1): \(\text{MSE}=6.03\!\times\!10^{-4}\) vs.\ KAN(1): \(1.39\!\times\!10^{-2}\) (win),
SN(8): \(1.32\!\times\!10^{-3}\) vs.\ KAN(8): \(4.45\!\times\!10^{-3}\) (win),
SN(3): \(1.07\!\times\!10^{-2}\) vs.\ KAN(3): \(2.76\!\times\!10^{-2}\) (win),
SN(4): \(1.96\!\times\!10^{-2}\) vs.\ KAN(4): \(1.34\!\times\!10^{-2}\) (loss).
Aggregating over seeds, SN’s mean eval MSE is \(9.95\times10^{-3}\) (median \(8.76\times10^{-3}\); min \(6.03\times10^{-4}\); max \(1.96\times10^{-2}\)) versus KAN’s \(1.64\times10^{-2}\) (median \(1.50\times10^{-2}\); min \(4.45\times10^{-3}\); max \(2.76\times10^{-2}\)). Hence SN reduces final error by roughly \(\mathbf{39\%}\) on average.
...
\begin{tabular}{lcccc}
\toprule
Method & Avg Eval MSE $\downarrow$ & Median $\downarrow$ & Min & Max \\
\midrule
SN (PWL + domains) & \textbf{0.00995} & \textbf{0.00876} & $6.03\!\times\!10^{-4}$ & $1.96\!\times\!10^{-2}$ \\
KAN (cubic splines) & 0.01639 & 0.01504 & $4.45\!\times\!10^{-3}$ & $2.76\!\times\!10^{-2}$ \\
\bottomrule
\end{tabular}
\caption{\textbf{Max-of-Affines8D} (10 seeds). SN has substantially lower final eval MSE. Min/Max are from individual seeds in the run log.}
\end{table}

\paragraph{Wall-clock.}
On a fixed 4000-epoch CPU budget, \textbf{SN trains faster per epoch}: SN ranges \(\,115.28\text{–}187.79\,\)s across seeds (avg \(\approx142\)s), while KAN ranges \(\,238.89\text{–}378.36\,\)s (avg \(\approx301\)s). E.g., SN(2) \(115.28\)s (fastest), SN(9) \(187.79\)s (slowest); KAN(1) \(238.89\)s (fastest), KAN(9) \(378.36\)s (slowest).
Moreover, SN often reaches \emph{KAN’s final} error early: by epoch~500, SN(0) is at \(0.00478\) (below KAN(0)’s final \(0.00653\)), and SN(8) is at \(0.00220\) (below KAN(8)’s final \(0.00445\)).
...
\begin{tabular}{lccc}
\toprule
Method & Avg wall clock (s) $\downarrow$ & Min (seed) & Max (seed) \\
\midrule
SN (PWL + domains) & \(\approx\,142\) & 115.28 (2) & 187.79 (9) \\
KAN (cubic splines) & \(\approx\,301\) & 238.89 (1) & 378.36 (9) \\
\bottomrule
\end{tabular}
\caption{Wall-clock over 4000 epochs (CPU). SN is both \emph{faster} and more sample‑efficient on this PWL target.}
\end{table}

\paragraph{Takeaways.}
On a piecewise-linear target, PWL~SNs are a \emph{better inductive match} than cubic KANs: they win on final error in 8/10 seeds and, importantly, hit KAN-level accuracy much earlier in training—while also training faster per epoch.
...
\subsubsection{Non-smooth high-dimensional payoff: 20D basket option regression (\texttt{basket\_options\_nd})}
\label{subsubsec:basket_nd}

\textbf{Task.} We regress a 20D basket-option payoff surface with a strike-induced \emph{kink} (i.e., $\max\{0,\sum_i w_i x_i - K\}$–like terms plus band penalties), a natural target that is piecewise linear within regions and non-smooth along exercise boundaries. Such targets are ubiquitous in quantitative modeling. 

\textbf{Why this favors SNs (but is fair).} SNs use piecewise-linear (PWL) splines and perform per-layer \emph{domain updates} during a short warm-up; we then freeze those domains. This moves the uniform knot grids to span the activation ranges actually encountered, preserving linearity within regions and concentrating resolution where the composition lives. In contrast, KANs with cubic splines approximate kinks with higher-order pieces that can overshoot near non-smooth boundaries unless heavily over-knotted; on kinked surfaces this makes the PWL bias of SNs advantageous. The setup keeps data, budget, and optimizer identical across models.

\textbf{Setup (apples-to-apples).} Both models train for exactly $4000$ epochs with Adam ($10^{-3}$), batch size $512$, weight decay $10^{-6}$, and gradient clipping $1.0$. We use BatchNorm (after-block, skip-first) and \emph{linear/projection} residuals only for both models. Architectures are matched (32–32; 64 knots for $\phi$ and $\Phi$), and total parameters are matched within~$\pm\!5\%$ (here, exactly equal). The SN receives a $400$-epoch domain-update warm-up (then domains are frozen). We fix seed $0$ for the results below.

\textbf{Results.} On the $4{,}096$-point test set, the SN attains $\mathrm{MSE}{=}1.545\times10^{-3}$ and $\mathrm{MAE}{=}3.043\times10^{-2}$, versus the KAN's $\mathrm{MSE}{=}2.491\times10^{-2}$ and $\mathrm{MAE}{=}1.124\times10^{-1}$. This is a \textbf{16.1$\times$ lower MSE} and \textbf{3.69$\times$ lower MAE} for SN at identical parameter counts (955 vs 955). Wall-clock for $4000$ epochs on CPU is \textbf{2{,}980\,s (SN)} vs \textbf{5{,}366\,s (KAN)}, i.e., SN is \textbf{$\sim$1.80$\times$ faster}. 

\textbf{Discussion.} The gains line up with the inductive bias: (i) PWL splines match piecewise-linear interiors; (ii) domain updates relocate knot \emph{ranges} to the regions the composition occupies; (iii) cubic pieces in KANs add curvature where the target has none and can oscillate around kinks, demanding more knots/capacity to match sharp transitions. The advantage holds at matched parameters and identical training budget, while also reducing wall-clock.

\vspace{0.25em}\noindent\emph{Fairness note.} Same data splits, optimizer, schedule, batch size, weight decay, residual type (linear/projection), and normalization (BatchNorm) for both; both train for exactly $4000$ epochs with matched parameter counts (955 each). The only asymmetry is the SN’s $400$-epoch domain-update warm-up (then frozen), intrinsic to SN training and completed well before evaluation; it does not add capacity or extra regularization.

\subsubsection{Saturating Max–Affine Field (SMAF; 20D, $K{=}40$)}
\label{sssec:smaf-20d}

\textbf{Setup.} The target is a clipped, piecewise‑linear field
\(
y(x)=\sum_{k=1}^{K} c_k\,\mathrm{clamp}(w_k^\top x+b_k,\;\ell_k,\;h_k)
\)
with $x\sim\mathrm{Unif}([0,1]^{20})$, $K{=}40$ channels. We trained on $1024$ random points and validated on $4096$.\; SN and KAN used the same architecture $[64,64]$, Adam ($\mathrm{lr}{=}3\!\times\!10^{-4}$, weight decay $10^{-7}$), full‑batch training for $4000$ epochs; SN had a $400$‑epoch warm‑up for domain updates and then froze domains. Both models used BN (post‑block, skip first), linear/projection residuals only, identical splits/metric, and parameter counts were matched \emph{exactly} (both $1627$ parameters). KAN splines were standard cubic Catmull–Rom with tied $\phi$/$\Phi$ knot counts chosen automatically to match SN’s parameter budget.

\medskip
\textbf{Metric.} Validation MSE; we report the best value over training for each seed and aggregate over 10 seeds ($\{0,1,\ldots,9\}$).

\medskip
\textbf{Results.} SN clearly wins on SMAF. Across seeds (best‑per‑seed, then averaged), SN achieved $4.85\!\times\!10^{-4}\pm7.56\!\times\!10^{-4}$ MSE, whereas KAN yielded $2.12\!\times\!10^{2}\pm4.98\!\times\!10^{2}$ MSE.\footnote{Means $\pm$ std over seeds. Large KAN variability is due to frequent blow‑ups on this non‑smooth target.} For the single best seeds, SN reached $4.04\!\times\!10^{-5}$ (seed 7) and KAN $6.38\!\times\!10^{-3}$ (seed 6), i.e. a $\sim\!1.58\!\times\!10^{2}$ gap in favor of SN. Representative KAN runs exhibited large validation spikes (often orders of magnitude), while SN converged smoothly to $10^{-4}$–$10^{-3}$‑level errors.

\medskip
\textbf{Why SN wins here.} SMAF is a sum of \emph{clamped affine} channels, i.e., a piecewise‑linear map with many kinks and plateaus. SN’s piecewise‑linear $\phi/\Phi$ splines align with this inductive bias and fit with few knots. In contrast, cubic KAN splines tend to overshoot near kinks/plateaus and need extra capacity/damping to avoid instability; under an equal‑parameter budget they underfit (or blow up) on this non‑smooth objective.

\medskip
\textbf{Timing.} Device: CPU. This run did not record wall‑clock times; only metric traces were logged.

\subsubsection{Dense-head shifts (12D, $m{=}64$): SN vs.\ KAN}
\label{subsubsec:dense-head-shift-12d-64h}

\textbf{Task.} Multi-head regression in which each output head is a smoothly shifted version of a shared latent index:
\[
y_j(x)\;=\;\tanh\!\Big(\beta\big[\mu(x)+\sigma(x)z_j+\alpha\,q_j + q_{\mathrm{bias}}\,q_j\big]\Big),
\]
with $x\in[0,1]^D$, $j=0,\dots,m{-}1$, $q_j\in[-1,1]$ on a uniform grid, and $z_j=\Phi^{-1}(\tau_j)$.\footnote{Dataset definition and bench harness are in \texttt{benchmarks/kan\_sn\_densehead\_shift\_bench.py}.}  We retain the parity/eval protocol from the parity benches (parameter matching and BN evaluation mode).

\textbf{Setup.} $D{=}12$, $m{=}64$, $\alpha{=}0.08$, $\beta{=}0.7$, $q_{\mathrm{bias}}{=}0.15$; train for 4000 epochs on CPU; $n_{\mathrm{test}}{=}50{,}000$. BN evaluation uses batch statistics without buffer updates (\texttt{batch\_no\_update}). Parameter parity was enforced (\texttt{--equalize\_params --prefer\_leq}), yielding \#params $\approx 3072$ for both models; the KAN basis size selected by parity was $K{=}4$. The specific model configurations were:
\begin{itemize}
\item \textbf{SN:} \texttt{arch=[32,32]}, $\phi$/$\Phi$ knots $60/60$, BN: batch/after/skip\_first, linear residuals, no lateral, domains warmup+freeze. (Notes recorded per run include BN eval mode and the parity choice.)
\item \textbf{KAN (fast impl.):} \texttt{arch=[3,24,4]}, degree $3$, $K{=}4$, BN: batch/after/skip\_first, linear outside map, linear residual.
\end{itemize}
...
\item \emph{Wall-clock (train time; CPU).} Averaging the ten \texttt{train\_seconds} values: KAN $\,\approx$ \textbf{476\,s}, SN $\,\approx$ \textbf{1{,}433\,s}. For reference, KAN runs were in the $\sim[338,624]$\,s range and SN runs in the $\sim[1{,}241,1{,}607]$\,s range.
\end{itemize}
...
\subsubsection{Piecewise-linear sensors (S$^2$HI; 16D, scalar)}
\label{sssec:s2hi}

\textbf{Task.} We introduce a simple, natural piecewise-linear (PWL) “Sensor Saturation + Hinge Interactions” (S$^2$HI) generator: for $x\in\mathbb{R}^{16}$, the target is
\[
y \;=\; \sum_i a_i\,\mathrm{clamp}(x_i-\theta_i,0,c_i)\;+\;\sum_k b_k\,\mathrm{clamp}(u_k^\top x-\tau_k,0,s_k)\;+\;\varepsilon,
\]
...
\textbf{Setup (apples-to-apples).} We train a \emph{single-block} SN and a \emph{single-block} cubic KAN for exactly $4000$ epochs with Adam (lr=$3\!\times\!10^{-4}$, wd=$10^{-7}$), full-batch, projection/linear residuals, and BatchNorm \emph{after} each block (no skip-first). SN runs a $400$‑epoch domain‑update warm‑up then freezes domains; KAN has \emph{no} domain updates and uses uniform cubic B‑splines with knots fixed on $[0,1]$. Parameter budgets are matched (both models end up at $1{,}127$ parameters with width $64$ and $41$ knots for $\phi$ and $\Phi$).

\textbf{Metrics.} We report MSE and $R^2$ on standardized targets for ID and OOD sets, averaged over the same batch for both models.

\textbf{Results (10 seeds; best/mean/median).}
\begin{itemize}
\item \emph{Best (over seeds).} ID MSE: SN $2.909\times10^{-2}$ vs.\ KAN $5.289\times10^{-2}$ ($\times 1.82$); OOD MSE: SN $4.227\times10^{-2}$ vs.\ KAN $7.825\times10^{-2}$ ($\times 1.85$). (Best seeds visible in the log.)
\item \emph{Mean over seeds (ID / OOD).} SN: $0.03752\ /\ 0.05871$; KAN: $2.0847\ /\ 2.1306$. Corresponding mean $R^2$: SN $0.9629\ /\ 0.9552$; KAN $-1.059\ /\ -0.636$. (Negative $R^2$ indicates catastrophic fits on some seeds.)
\item \emph{Median over seeds (ID / OOD).} SN: $0.03806\ /\ 0.05739$; KAN: $0.782\ /\ 0.879$ (still far worse than SN).
\end{itemize}

\textbf{Stability.} SN wins on \emph{all 10/10 seeds} on both ID and OOD MSE. The KAN exhibits $4/10$ seeds with negative ID $R^2$ (e.g.\ MSE $=8.98$, $R^2=-7.97$), while its best seed is still behind SN’s best.

\textbf{Why SN wins here.} The target is PWL with saturations and hinge kinks; the SN’s piecewise-linear spline blocks with domain updates match this function class and adapt support to the heavy-tailed mixture before freezing. In contrast, the cubic KAN’s smoother basis tends to overshoot near kinks and, with knots fixed on $[0,1]$ and a matched (small) parameter budget, struggles both on ID mixed tails and on the pure-tail OOD box.

\textbf{Wall-clock.} Not recorded by the script; the summary table leaves the time cell blank. (The training logs include losses and fairness checks but no timing.)
...
\hline
Task & Metric & SN (best) & KAN (best) & \begin{tabular}{@{}c@{}}Ratio\\(KAN/SN)\end{tabular} & \begin{tabular}{@{}c@{}}Time\\(KAN/SN)\\(s)\end{tabular} \\
\hline
TriWaveMix10D (10D) & RMSE $\downarrow$ & 0.145 & 0.607 & $4.19\times$ & 9{,}831 / 1{,}701 \\
MQSI (20D, 9 heads) & \begin{tabular}{@{}c@{}}Mean RMSE $\downarrow$\\(best of 10 seeds)\end{tabular} & $4.47\times 10^{-3}$ & $6.58\times 10^{-3}$ & $1.47\times$ & 393 / 589 (avg) \\
Max-of-Affines8D (8$\to$1) & \begin{tabular}{@{}c@{}}Best MSE $\downarrow$\\(10 seeds)\end{tabular} & $6.03\times 10^{-4}$ & $4.45\times 10^{-3}$ & $7.38\times$ & 301 / 142 (avg) \\
Basket options (20$\to$1) & MSE $\downarrow$ & $1.545\times 10^{-3}$ & $2.491\times 10^{-2}$ & 16.1$\times$ & 5{,}366 / 2{,}980 \\
SMAF (20D, 40 comps) & \begin{tabular}{@{}c@{}}Best val MSE $\downarrow$\\(10 seeds)\end{tabular} & $4.04\times 10^{-5}$ & $6.38\times 10^{-3}$ & $157.9\times$ & \textemdash \\
DenseHeadShift (12D, 64 heads) & \begin{tabular}{@{}c@{}}Mean RMSE $\downarrow$\\(10 seeds)\end{tabular} & $5.47\times10^{-3}$ & $6.94\times10^{-3}$ & $1.27\times$ & 467 / 1{,}433 (avg) \\
PWL sensors (S$^2$HI; 16D) & \begin{tabular}{@{}c@{}}Best ID MSE $\downarrow$\\(10 seeds)\end{tabular} & $2.91\times 10^{-2}$ & $5.29\times 10^{-2}$ & $1.82\times$ & \textemdash \\
\hline
\end{tabular}
