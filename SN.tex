...
\begin{itemize}
    \item \textbf{Monotonic spline $\phi(\cdot)$:} An increasing piecewise-linear function mapping $\mathbb{R} \to [0,1]$. While we denote the domain as $\mathbb{R}$ for generality, in practice each $\phi^{(\ell)}$ operates on a bounded domain determined by the network's structure and current parameters, as detailed in Section~\ref{sec:theoretical_domains}. This function is shared across all input-output connections within the block and its coefficients are learnable. Strict monotonicity (strictly increasing) is enforced during training (see Section \ref{sec:implementation}).
...
for $q=0, \dots, d_{\mathrm{out}}-1$, where $\alpha$ is a scaling factor (set to 1 for the original Sprecher construction).
where $\alpha$ is a scaling factor (set to 1 for the original Sprecher construction). While $\alpha = 1$ maintains theoretical consistency with Sprecher's formula, alternative values may be explored to improve optimization dynamics in deeper networks. Note that $q$ serves dual roles here: as an output index ($q = 0, \ldots, d_{\mathrm{out}}-1$) and as an additive shift parameter within the formula.
...
\begin{itemize}
    \item \textbf{Residual connections:} When enabled, learnable parameters facilitate skip connections around each block. If input and output dimensions match ($d_{\mathrm{in}} = d_{\mathrm{out}}$), a scalar weight $w_{\text{res}}$ is used. When dimensions differ, a learnable linear projection matrix $W_{\text{res}} \in \mathbb{R}^{d_{\mathrm{out}} \times d_{\mathrm{in}}}$ (without bias) maps the input to the output dimension. The block output with residual connection becomes:
    $$h_q^{\text{final}} = h_q + \begin{cases}
    w_{\text{res}} \cdot x_q & \text{if } d_{\mathrm{in}} = d_{\mathrm{out}}\\
    (W_{\text{res}} \mathbf{x})_q & \text{if } d_{\mathrm{in}} \neq d_{\mathrm{out}}
    \end{cases}$$
...
%\begin{remark}[Regularization for smoother splines]
%While training with MSE alone is sufficient, the splines learned under pure MSE optimization can exhibit rapid oscillations or jagged behavior. To encourage smoother, more interpretable splines (as shown in Figures \ref{fig:onevar_example} and \ref{fig:twovarsprecher_revised}), additional regularization terms can be incorporated:
...
%where:
%\begin{itemize}
%    \item $\text{Penalty}(\Phi^{(\ell)})$ is a flatness penalty encouraging smoothness, calculated as the mean squared second difference of spline coefficients: $\frac{1}{K-2} \sum_{k=1}^{K-1} (c_{k+1} - 2c_k + c_{k-1})^2$
%    \item $\text{Sparsity}(\lambda^{(\ell)})$ encourages sparse mixing weights (e.g., using L1 norm combined with entropy)
%    \item $\mathcal{L}_{\text{variance}} = (\sigma_{\text{output}} - \sigma_{\text{target}})^2$ encourages the network output to match the target variance
%\end{itemize}
%These regularization terms are not necessary for successful training but can improve interpretability and generalization. The weights $w_{\text{flat}}$, $w_{\text{sparse}}$, and $w_{\text{var}}$ control the strength of each regularization component.
%\end{remark}

\subsection{Theoretical domain computation}
\label{sec:theoretical_domains}
One advantage of the structured Sprecher architecture is the ability to compute exact theoretical bounds for all intermediate values. Throughout this analysis, we assume inputs lie in $[0, 1]^n$, though the methodology extends naturally to any bounded domain. This assumption enables principled initialization and can aid in debugging and analysis.
...
\item If $\Phi$ has trainable codomain parameters $(c_c, c_r)$ defining codomain $[c_c - c_r, c_c + c_r]$, then the output range is determined by examining the actual spline coefficients. For piecewise linear splines, the extrema occur at knot points, so the actual output range is $[\min_j \Phi(t_j), \max_j \Phi(t_j)]$ where $t_j$ are the knot locations. This ensures accurate range propagation even for highly oscillatory splines.
\end{enumerate}
\end{lemma}
...
This lemma enables a forward propagation algorithm for computing all spline domains throughout the network. Importantly, this computation is performed not just at initialization but at every training iteration as the parameters $\lambda^{(\ell)}$ and $\eta^{(\ell)}$ evolve, ensuring that splines always operate on their natural domains:
...
\begin{algorithmic}[1]
    ...
    \For{each block $\ell = 1, \ldots, L$}
        ...
        \If{block $\ell$ has residual weight $w^{(\ell)}$}
            \State $\mathcal{R}_\ell \leftarrow \mathcal{R}_\ell \oplus w^{(\ell)} \cdot \mathcal{R}_{\ell-1}$ \Comment{Interval arithmetic}
        \EndIf
    \EndFor
\end{algorithmic}
\end{algorithm}

where $\oplus$ denotes interval addition: $[a_1, b_1] \oplus [a_2, b_2] = [a_1 + a_2, b_1 + b_2]$.

\begin{remark}[Horizontal rescaling during training]
When spline domains change during training due to evolving parameters, we preserve the learned function by horizontally rescaling it to the new domain. Specifically, if a spline's domain changes from $[a_{old}, b_{old}]$ to $[a_{new}, b_{new}]$, we remap the knot positions linearly while keeping the knot values (coefficients) unchanged. This maintains the functional relationship learned by the spline while adapting to the network's evolving internal representations. However, this approach has potential drawbacks that warrant further investigation. Horizontal rescaling changes the derivative at every point, which may disrupt gradient flow during optimization. For the highly oscillatory $\Phi$ splines, compression can amplify oscillations and cause numerical instability. Alternative approaches that preserve the learned function more faithfully, such as adaptive knot placement within a fixed domain or careful initialization to prevent excessive domain growth, may prove more effective in practice.
\end{remark}
...
\begin{remark}[Domain-aware initialization and dynamic updates]
The ability to compute exact domains enables the following strategy:
\begin{enumerate}
\item \textbf{Initialization:}
    \begin{itemize}
    \item Initialize all parameters ($\lambda^{(\ell)}, \eta^{(\ell)}$) with small random values
    \item Run forward domain propagation (Algorithm~\ref{alg:domain_prop}) to compute each spline's theoretical domain
    \item For each $\Phi^{(\ell)}$: set domain knots uniformly, initialize $(c_c, c_r)$ to match the domain if trainable, and set coefficients to create an approximately linear function
    \item For each $\phi^{(\ell)}$: set domain knots uniformly and initialize as an approximately linear increasing function mapping to $[0,1]$
    \end{itemize}
\item \textbf{Dynamic updates during training:}
    \begin{itemize}
    \item At each training iteration, as parameters $\lambda^{(\ell)}$ and $\eta^{(\ell)}$ evolve, recompute all domains via Algorithm~\ref{alg:domain_prop}
    \item Adjust spline knot positions to match new domains while preserving the learned function shapes (horizontal rescaling)
    \item This ensures splines always operate on their natural domains as the network evolves
    \end{itemize}
\end{enumerate}
\end{remark}
...
\begin{itemize}
    ...
    \item \textbf{SN (hypothetical):} Architecture $2 \to [10] \to 1$. $L=1$ hidden layer, scalar output means $L=1$ block total. Shared splines: $2 \times (G+k) \approx 2 \times 23 = 46$. Mixing weights: $d_0 = 2$ (vector weights). Shift $\eta^{(1)}$: $1$. Total SN params $\approx 46 + 2 + 1 = 49$.
    \item \textbf{Potential advantage:} For equivalent structure, theoretical reduction factor $\approx 14$x (49 vs 690). Actual performance requires empirical validation.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{SN (hypothetical):} Architecture $17 \to [1] \to 14$. $L=1$ hidden layer, vector output means $L+1=2$ blocks total. Shared splines: $2(L+1) \times (G+k) \approx 4 \times 6 = 24$. Mixing weights: $d_0 + d_1 = 17 + 1 = 18$ (vector weights). Shifts $\eta^{(1)}, \eta^{(2)}$: $2$. Total SN params $\approx 24 + 18 + 2 = 44$.
    \item \textbf{Potential advantage:} Theoretical reduction factor $\approx 4.2$x (44 vs 186). The narrow intermediate dimension $d_1=1$ might pose challenges for SN training.
\end{itemize}
...
Current implementations face several significant obstacles. Training speed and stability remain primary concerns, with SNs often requiring more computation time compared to comparable MLPs per epoch. The shared spline structure can lead to optimization difficulties where networks converge to suboptimal local minima with flat or overly simplistic splines. Escaping such minima likely requires advanced optimization techniques tailored specifically to the unique parameter structure of SNs.

The initialization of splines, shifts, and weights significantly impacts training success, yet optimal strategies beyond the domain-aware initialization discussed in Section \ref{sec:implementation} remain elusive. Similarly, architectural choices such as network depth and hidden layer widths have profound effects on performance, but principled methods for architecture selection are lacking. The number of knots for shared splines represents a critical hyperparameter that, unlike in KANs where complexity can vary per edge, affects the entire block's expressivity in SNs. Developing adaptive strategies for knot placement and automated methods for selecting appropriate spline resolution based on data complexity would be highly valuable. In particular, adaptive knot placement that maintains a fixed total knot count while concentrating resolution where the data lives could address the oscillatory spline behavior observed in practice without sacrificing the architecture's parameter efficiency. This could involve quantile-based knot spacing or other data-aware placement strategies that respect the theoretical domain bounds while improving function approximation quality.
