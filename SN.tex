...
\begin{itemize}
    \item \textbf{Monotonic spline $\phi(\cdot)$:} A non-decreasing spline mapping $\mathbb{R} \to [0,1]$ (piecewise-linear or $C^1$ piecewise-cubic Hermite/PCHIP; see \Cref{sec:implementation}) that is applied coordinate-wise. While we denote the domain as $\mathbb{R}$ for generality, in practice each $\phi^{(\ell)}$ operates on a bounded domain determined dynamically by the network's structure and current parameters, as detailed in Section~\ref{sec:theoretical_domains}. This function is shared across all input-output connections within the block and its coefficients are learnable. Monotonicity is enforced during training through appropriate parameterization (see Section \ref{sec:implementation}).
    \item \textbf{General spline $\Phi(\cdot)$:} A spline mapping $\mathbb{R} \to \mathbb{R}$ (piecewise-linear or $C^1$ piecewise-cubic Hermite/PCHIP; see \Cref{sec:implementation}, without monotonicity constraints) whose domain and codomain are determined by the network structure and can be either fixed or made trainable. If trainable codomains are used, they can be parameterized in various ways, such as by center and radius parameters, allowing the spline to adapt its output range during training. This function is also shared across the block and its coefficients are learnable.
...
where $s_q^{(\ell)} = \sum_{i=1}^{d_{\mathrm{in}}} \lambda_i^{(\ell)} \phi^{(\ell)}(x_i^{(\ell)} + \eta^{(\ell)} q) + \alpha q$ represents the pre-mixing activation, $\alpha$ is a fixed channel-spacing constant (we set $\alpha=1$ in all experiments unless explicitly stated otherwise), and $\mathcal{N}(q)$ denotes the neighborhood structure:
\begin{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Output scaling:} The network may include learnable output scaling parameters $\gamma$ (scale) and $b$ (bias) applied to the final network output: $f_{\text{final}}(\mathbf{x}) = \gamma \cdot f(\mathbf{x}) + b$. These parameters can improve optimization dynamics and are typically initialized with $\gamma = 0.1$ and $b$ set to match the mean of the target distribution when available.
\end{itemize}
...
\begin{table}[ht]
\centering
\caption{Cyclic residuals vs.\ linear residual projections on synthetic regression tasks. We report test RMSE after the stated number of epochs. Toy-2D refers to the scalar 2D regression benchmark defined above; Toy-$4\to5$ refers to the synthetic regression task defined in Section~\ref{sec:toy4to5}. ``Linear'' uses a learned scalar when $d_{\mathrm{in}}=d_{\mathrm{out}}$ and a learned matrix $W\in\mathbb{R}^{d_{\mathrm{in}}\times d_{\mathrm{out}}}$ otherwise; ``cyclic'' uses the pooling/broadcast construction when $d_{\mathrm{in}}\neq d_{\mathrm{out}}$ (and a learned scalar when $d_{\mathrm{in}}=d_{\mathrm{out}}$). The last columns count residual-path parameters only.}
\label{tab:cyclic-residual-ablation}
\small
\resizebox{\textwidth}{!}{%
\begin{tabular}{@{}llrrrrr@{}}
\toprule
Task & Architecture & Epochs & RMSE (linear) & RMSE (cyclic) & \#resid params (lin./cyc.) & Ratio \\
\midrule
Toy-2D & $2\rightarrow[10,10,10]\rightarrow1$ & 10{,}000 & $3.38\times10^{-3}$ & $2.87\times10^{-3}$ & $22/12$ & $1.83\times$ \\
Toy-2D & $2\rightarrow[10,11,10,11,10,11,10]\rightarrow1$ & 50{,}000 & $3.14\times10^{-3}$ & $3.88\times10^{-3}$ & $680/76$ & $8.95\times$ \\
Toy-2D & $2\rightarrow[10,11,12,13,14,15,16,17]\rightarrow1$ & 50{,}000 & $2.42\times10^{-3}$ & $1.89\times10^{-3}$ & $1322/108$ & $12.2\times$ \\
Toy-$4\to5$ & $4\rightarrow[10,11,12,13,14,15,16,17]\rightarrow5$ & 10{,}000 & $4.85\times10^{-3}$ & $4.26\times10^{-3}$ & $1427/125$ & $11.4\times$ \\
\bottomrule
\end{tabular}%
}
\end{table}
...
\begin{table}[t]
\centering
\caption{Ablation on Toy-2D. Mean test RMSE $\pm$ std over 10 seeds. We progressively add key architectural components, ending with the full SN configuration. Lower is better.}
...
Setting & Test RMSE \\
\midrule
Base SN (no residuals/mixing/domain tracking) & $0.438 \pm 0.061$ \\
+ cyclic residuals & $0.267 \pm 0.038$ \\
+ bidirectional mixing & $0.082 \pm 0.010$ \\
+ domain tracking & $0.045 \pm 0.008$ \\
+ resampling + domain tracking (full SN) & $\mathbf{0.028 \pm 0.006}$ \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[ht]
    ...
    \begin{tikzpicture}[scale=0.9, >={Latex[scale=0.8]}]
        ...
        \begin{scope}[xshift=0cm]
            ...
            \node[font=\footnotesize, align=center] at (1.25,-4.5) {$h_q \mathrel{+}= w \cdot x_q$\\[2pt]\footnotesize 1 parameter};
        \end{scope}
        ...
        \begin{scope}[xshift=5.5cm]
            ...
            \node[font=\footnotesize, align=center] at (1.25,-4.5) {$h_q \mathrel{+}= \sum_{i \bmod d_{\mathrm{out}} = q} w_i x_i$\\[2pt]\footnotesize $d_{\mathrm{in}}$ parameters};
        \end{scope}
    ...
    \begin{tikzpicture}
        ...
        \begin{scope}[xshift=0cm]
            ...
            \draw[thick, sprechergreen] 
                (0,0) -- (0.6,0.35) -- (1.2,0.8) -- (1.8,1.4) -- 
                (2.4,1.85) -- (3.0,2.2) -- (3.6,2.5);
            
            % Mark knots
            \foreach \x/\y in {0/0, 0.6/0.35, 1.2/0.8, 1.8/1.4, 2.4/1.85, 3.0/2.2, 3.6/2.5} {
                \fill[sprechergreen] (\x,\y) circle (2pt);
            }
            
            % Codomain annotation
            \draw[<->, gray] (-0.4,0) -- (-0.4,2.5);
            \node[left, gray, font=\footnotesize] at (-0.4,1.25) {$[0,1]$};
...
\end{align}
where the lateral mixing term is included when enabled, and $s_j^{(\ell)}$ denotes the pre-mixing activation for output $j$. When cyclic residual connections are enabled (Fig.~\ref{fig:cyclic_residuals}), the post-activation vector from \eqref{eq:SN} is further updated by adding the corresponding pooling/broadcast residual term. When BatchNorm is used, it is applied before or after each block as described in Section~\ref{sec:normalization}.
...
which yields strictly increasing knot values. Interpolating these knots with piecewise–linear segments or a shape–preserving monotone cubic (\textsc{PCHIP}) scheme which yields strictly increasing knot values and therefore a monotone non-decreasing spline on its domain (piecewise–linear or monotone $C^1$ cubic Hermite/PCHIP, depending on the interpolation). Outside the domain we use constant extension, so $\phi^{(\ell)}(x)=0$ for inputs below the leftmost knot and $\phi^{(\ell)}(x)=1$ above the rightmost knot. The increments are initialized nearly uniform so that $\phi^{(\ell)}$ starts close to linear.
...
In practice, this enables training of architectures that would otherwise be infeasible. For example, a network with layers of width 512 might exhaust 32GB of RAM in parallel computation but trains comfortably with sequential processing. Empirically, we observe substantial activation-memory reductions on wide architectures (often exceeding 40\%), while the runtime impact is workload-dependent and can range from modest overhead to slight speedups on very large inputs.
...
\item Without lateral mixing, monotonicity of $\phi$ implies $\phi(h_i+\eta q)\in[\phi(a+\eta q),\,\phi(b+\eta q)]$ for all $i$ and $q$. Let
...
(As a coarse bound, one may substitute $\phi(a+\eta q)=0$ and $\phi(b+\eta q)=1$.)
...
\end{aligned}$$
where $a^+ = \max(a,0)$ and $a^- = \min(a,0)$. For bidirectional mixing, add the analogous sign-split terms using $(q-1)\bmod d_{\mathrm{out}}$ and sum the contributions.

\item If $\Phi$ has trainable codomain parameters $(c_c, c_r)$ defining codomain $[c_c - c_r, c_c + c_r]$, then tight output bounds over an input interval require checking all candidate extrema of the spline on that interval. For piecewise linear splines, extrema occur at interval endpoints and knot points, so it suffices to evaluate $\Phi$ at those locations. For cubic (PCHIP) splines, extrema may also occur at interior critical points where the derivative is zero; thus one must additionally evaluate $\Phi$ at all such critical points within the interval. This yields correct range propagation even for oscillatory $\Phi$.
...
\item \textbf{Out-of-domain extension:} When inputs fall outside the spline's domain:
   ...
   \item $\Phi$ (general): Uses \emph{linear} extrapolation based on the boundary \emph{derivative} of the spline interpolation (piecewise-linear: slope of the first/last segment; cubic \textsc{PCHIP}: endpoint derivative from the \textsc{PCHIP} slope rule). If $\Phi$'s codomain is parameterized by $(c_c,c_r)$ with $\operatorname{cod}(\Phi)=[c_c-c_r,\,c_c+c_r]$, extrapolation is computed in the unnormalized coefficient space and then mapped by
   $$t \;\mapsto\; c_c - c_r \;+\; 2c_r \cdot \frac{t - t_{\min}}{t_{\max}-t_{\min}}\,,$$
   with the boundary derivatives scaled by the same factor $2c_r/(t_{\max}-t_{\min})$.
   \end{itemize}
\end{enumerate}
\end{lemma}
...
This lemma enables a forward propagation algorithm for computing all spline domains throughout the network. The algorithm can optionally apply a domain safety margin (default 0) to reduce edge hits during training. Crucially, the algorithm's efficiency relies on the fact that spline extrema over compact intervals can be found by evaluating a small, explicit candidate set: for piecewise-linear splines, the interval endpoints and any knots inside the interval; for piecewise-cubic Hermite/PCHIP splines, the same candidates plus any interior critical points where the derivative vanishes (one quadratic solve per segment).
...
\begin{figure}[ht]
    ...
    \begin{tikzpicture}[scale=0.95, >=latex]
        ...
        \begin{scope}[xshift=7.5cm]
            ...
            % 5. x=3.0 (Extrap): y ~ 0.80
            
            \draw[thick, sprechergreen] (0.6,1.17) -- (1.2,1.03) -- (1.8,1.47) -- (2.4,1.40) -- (3.0,0.80);
            
            % New Knots (5 count)
            \foreach \p in {(0.6,1.17), (1.2,1.03), (1.8,1.47), (2.4,1.40), (3.0,0.80)}
                \fill[sprechergreen] \p circle (2pt);
                
            % Label moved safely below the tail
            \node[font=\scriptsize, sprechergreen, anchor=north] at (3.0, 0.7) {resampled};
            
            % Success Text: Centered
            \node[anchor=north, align=left, font=\footnotesize, text=sprechergreen] at (2.4, -0.6) {
                \ding{51} Function shape preserved\\
                \ding{51} $C^0$ continuity maintained
            };
        \end{scope}
    \end{tikzpicture}
    \caption{Spline domain resampling during training (applied to the outer spline $\Phi$). As parameters $\lambda$ and $\eta$ evolve, induced spline input ranges shift dynamically. \textbf{Left:} Naive horizontal rescaling distorts the learned function by stretching and shifting the shape to fit the new domain. \textbf{Right:} Our resampling evaluates the previous spline at the new knot positions (preserving its values at those knots over the overlapping domain) and extrapolates linearly elsewhere.}
...
\paragraph{General Splines ($\Phi^{(\ell)}$):} When the domain of a $\Phi^{(\ell)}$ spline changes, new knot locations are established (typically uniformly spaced across the new computed domain). The original spline is treated as a continuous function and evaluated at these new knot locations to yield updated knot coefficients. This effectively ``resamples'' the learned shape onto the new domain: it preserves the function values at the new knots. For piecewise-linear $\Phi^{(\ell)}$, this procedure is exact whenever the new knot set contains the old knots; for cubic (PCHIP) $\Phi^{(\ell)}$, it matches the old spline at the resampled knots but can (slightly) change derivatives between knots, while still avoiding abrupt resets.
...
Remove output scaling $(c_c,c_r)$ & Forces $\Phi$ to learn both scale and shape, often slowing convergence and degrading final accuracy. \\
...
\bottomrule
\end{tabular}
\caption{Qualitative summary of component ablations observed across our experiments. These effects align with each component's motivation: domain management for numerical stability, normalization and output scaling for conditioning, residual connections for depth, and lateral mixing for breaking shared-weight symmetries.}
...
\textbf{Poisson PINN benchmark (manufactured solution).}
...
We train in \texttt{torch.float64} using Adam with $10^{-3}$ learning rate, cosine learning rate decay to $10^{-5}$, and gradient-norm clipping 1.0. We report two parameter budgets: an $\approx$1200-parameter model trained for 5{,}000 epochs (mean $\pm$ std over 5 seeds), and an $\approx$3000-parameter model trained for 20{,}000 epochs (1 seed). We evaluate both models on a dense grid and report: (i) PDE residual MSE, (ii) boundary MSE, and (iii) MSE against the manufactured solution.

\begin{table}[t]
  \centering
  \caption{\textbf{Poisson PINN results (manufactured solution).} We report PDE residual MSE on a dense interior grid, boundary MSE on a dense boundary grid, and MSE against the manufactured solution (all lower is better). For the $\approx$1200-parameter setting we report mean $\pm$ std over 5 seeds; the $\approx$3000-parameter setting is reported for 1 seed due to cost.}
  ...
  \begin{tabular}{lrrr}
    ...
    SN  & $1.4\times 10^{2} \pm 2.6$ & $7.8\times 10^{-2} \pm 2.5\times 10^{-2}$ & $\mathbf{4.1\times 10^{-2} \pm 1.3\times 10^{-2}}$ \\
    KAN & $1.5\times 10^{2} \pm 4.9$ & $\mathbf{1.4\times 10^{-2} \pm 4.0\times 10^{-3}}$ & $1.1\times 10^{-1} \pm 4.2\times 10^{-2}$ \\
    \midrule
    \multicolumn{4}{l}{\textbf{$\approx$3000 params, 20{,}000 epochs (1 seed)}} \\
    SN  & $\mathbf{3.4\times 10^{-2}}$ & $6.1\times 10^{-6}$ & $\mathbf{6.0\times 10^{-6}}$ \\
    KAN & $3.8\times 10^{-1}$ & $\mathbf{3.1\times 10^{-6}}$ & $1.6\times 10^{-4}$ \\
    \bottomrule
  \end{tabular}
\end{table}
...
Unless otherwise stated, each head-to-head uses exactly $4000$ training epochs for both architectures. Both models use a standard learned linear residual projection (a dense matrix when $d_{\mathrm{in}}\neq d_{\mathrm{out}}$, or a learned scalar weight when $d_{\mathrm{in}}=d_{\mathrm{out}}$). KANs use cubic ($\deg=3$) spline bases. SNs use domain warmup early in training (300–400 epochs depending on the task) and then freeze the learned domains; no lateral mixing or other SN-specific auxiliaries are enabled. We match parameter counts by choosing the KAN basis size $K$ to be as close as possible (preferring $\leq$) to the SN parameter count, and we keep batch-normalization (BN) semantics aligned at test time with the training semantics. Seeds, datasets, and train/test splits are identical across models. Training is full-batch (no mini-batching): each epoch uses the entire training set. Primary metrics are test set RMSE or MSE (reported per task below); for multi-head monotone problems we also report a monotonicity-violation rate and a correlation-structure error.
...
For each coordinate, define a strictly increasing feature
...
$$\sigma(\mathbf{x})=\max\!\Bigl(10^{-5},\,s_0+\sum_{i=1}^{D} v_i\,h_i(x_i)\Bigr).$$
To introduce controlled head correlations while preserving monotonicity, we add positive pairwise terms:
choose $n_{\mathrm{pairs}}=\max(1,\mathrm{round}(0.15D))$ random distinct pairs $(i,j)$ and set
...
\subsubsection{Dense-head shifts (12D, $m{=}64$): SN vs.\ KAN}
...
\]
with $x\in[0,1]^D$, $j=0,\dots,m{-}1$, $q_j\in[-1,1]$ on a uniform grid, and $z_j=\Phi_{\mathcal{N}}^{-1}(\tau_j)$. Here $\mu(\mathbf{x})$ and $\sigma(\mathbf{x})$ are constructed exactly as in the MQSI benchmark above (same $h_i$, weight normalization, and positive pairwise interactions), but with $D=12$ and $m=64$ heads. We use $\tau_j=\mathrm{linspace}(0.1,0.9)$ and $q_j$ as a uniform grid on $[-1,1]$. We retain the parity/eval protocol from the parity benches (parameter matching and BN evaluation mode).

\textbf{Setup.} $D{=}12$, $m{=}64$, $a_{\mathrm{shift}}{=}0.08$, $\beta{=}0.7$, $q_{\mathrm{bias}}{=}0.15$; train for 4000 epochs on CPU on a fixed i.i.d.\ training set of $n_{\mathrm{train}}{=}1024$ samples; $n_{\mathrm{test}}{=}50{,}000$. BN evaluation uses per-batch statistics while freezing the running-statistics buffers (i.e., no running-mean/variance updates during evaluation). Parameter parity was enforced by choosing the KAN basis size $K$ to be as close as possible to (and not exceeding) the SN parameter count, yielding \#params $\approx 3072$ for both models; the resulting KAN basis size was $K{=}4$. The specific model configurations were:
\begin{itemize}
...
\item \textbf{KAN baseline:} A lightweight edge-wise KAN with hidden widths $(3,24,4)$ (architecture $12\to3\to24\to4\to64$), degree-$3$ splines with $K{=}4$ basis functions per edge; BatchNorm after each layer except the first; linear residual connections; and linear extrapolation outside the knot interval.
\end{itemize}
...
\paragraph{Dataset.}\label{sec:toy4to5}
We consider a synthetic smooth regression problem with four inputs $\mathbf{x}=(x_1,x_2,x_3,x_4)\sim \mathrm{Unif}([0,1]^4)$ and five outputs $\mathbf{y}(\mathbf{x})=(y_1,\dots,y_5)\in\mathbb{R}^5$ defined by
\begin{align*}
y_1(\mathbf{x}) &= \sin(2\pi x_1)\cos(\pi x_2),\\
y_2(\mathbf{x}) &= \exp\!\big(-2(x_1^2+x_2^2)\big),\\
y_3(\mathbf{x}) &= x_3^3 - x_4^2 + \tfrac{1}{2}\sin(5x_3),\\
y_4(\mathbf{x}) &= \operatorname{sigmoid}\!\big(3(x_1+x_2-x_3+x_4)\big),\\
y_5(\mathbf{x}) &= \tfrac{1}{2}\sin(4\pi x_1 x_4) + \tfrac{1}{2}\cos(3\pi x_2 x_3),
\end{align*}
where $\operatorname{sigmoid}(t)=1/(1+e^{-t})$. We train on a fixed i.i.d.\ training set of size $n_{\mathrm{train}}=1024$ and report all metrics on a held-out i.i.d.\ test set of size $n_{\mathrm{test}}=50{,}000$ from the same distribution.
...
matching the residual design advocated for KANs,\footnote{KANs place learnable 1D functions on edges and (in many KAN implementations) add a small residual base activation; see, e.g., Eqs.~(2.10)--(2.12) and surrounding discussion in \cite{liu2024kan}.}
...
\begin{itemize}
  ...
  \item inputs are passed through unchanged (no per-edge input normalization or domain adaptation), so both models receive identical raw features.
\end{itemize}
These choices make our KAN baseline easy to audit and reproducible, but they are not necessarily optimal on several benchmarks.\footnote{Grid extension and other KAN implementation details (Sec.~2.4 of \cite{liu2024kan}) are known to improve accuracy/speed trade-offs on several benchmarks.}
...
(iv) Timing is mixed but similar (SN slower in Run 1, faster in Run 2) and reflects basic, non-optimized PyTorch code for both models; we did not employ KAN-specific speedups such as grid extension or pruning.

\medskip
\noindent\textbf{Reproducibility.} All numbers in Table~\ref{tab:sn-vs-kan-4d5} were produced using the same fixed train/test split (per seed) and identical optimization and evaluation settings for both models.
