...
\subsection{Synthetic multi-output regression: SN vs.\ KAN}

We report a controlled head-to-head comparison between Sprecher Networks (SNs) and Kolmogorov–Arnold Networks (KANs) on a smooth synthetic mapping
$f:\mathbb{R}^{4}\!\to\!\mathbb{R}^{5}$ (dataset \texttt{toy\_4d\_to\_5d}).
The task is multi-output regression with coupled targets; train and test samples are drawn i.i.d.\ from the same distribution, and performance is measured on a large held-out set (20{,}000 points).
Both models are trained for the same number of epochs under a single harness that ensures identical training inputs are used for SN and KAN.\footnote{Implementation details and the exact experiment harness are in \texttt{compare\_sn\_vs\_kan\_multioutput.py}. In particular, the KAN is trained on the exact same $(x_{\text{train}},y_{\text{train}})$ returned by the SN trainer, and the same evaluation utilities are used for both models. \,:contentReference[oaicite:0]{index=0}}
