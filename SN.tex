...
\hline
Task & Metric & SN (best) & KAN (best) & Ratio (KAN/SN) & Time SN (s) & Time KAN (s) & Speedup (KAN/SN)\\
\hline
Smooth 4D$\to$5D & Mean RMSE $\downarrow$ & \textit{[to fill]} & \textit{[to fill]} & \textit{[to fill]} & \textit{[to fill]} & \textit{[to fill]} & \textit{[to fill]}\\
MQSI (20D, 9 heads) & Mean RMSE $\downarrow$ & \textit{[to fill]} & \textit{[to fill]} & \textit{[to fill]} & \textit{[to fill]} & \textit{[to fill]} & \textit{[to fill]}\\
TriWaveMix10D (10D) & Mean RMSE $\downarrow$ & 0.145 & 0.607 & $4.19\times$ & 1701 & 9831 & $5.78\times$\\
\hline
\end{tabular}
\caption{Summary across baseline tasks at matched parameter counts and $4000$ epochs; wall-clock time is added for completeness. For \texttt{TriWaveMix10D}, Mean RMSE values are $\sqrt{\text{MSE}}$ derived from Table~\ref{tab:triwavemix10d}.}
