...
\begin{remark}[Spline ringing artifacts]
Learned outer splines $\Phi^{(\ell)}$ often develop a pronounced central peak with adjacent undershoots, superficially reminiscent of biphasic neural waveforms (see $\Phi^{(1)}$ in Block 1 of Figure~\ref{fig:twovarsprecher_revised}). This pattern arises when approximating high‑curvature features with piecewise‑linear splines of finite resolution: the spline can exhibit ringing around sharp bends, producing local overshoot and undershoot analogous to interpolation artifacts.
\end{remark}
