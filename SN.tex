...
SNs are built upon the following principles, directly reflecting Sprecher's formula. Each functional block (mapping between layers) is organized around a shared \emph{monotonic} spline $\phi(\cdot)$ and a shared \emph{general} spline $\Phi(\cdot)$, both learnable. Each block incorporates a learnable scalar shift $\eta$ applied to inputs based on the output index $q$. Each block includes learnable mixing weights $\lambda_{i,q}$ that combine contributions from different input dimensions. The structure explicitly includes the additive shift $q$ inside the outer spline $\Phi$, mimicking Sprecher's formulation.
...
The \textbf{outer spline $\Phi^{(\ell)}$} is defined on a wider interval, e.g., $[-10, 10]$. The exact range can be fixed or made trainable via range parameters (center and radius), as explored in the reference code implementation. Trainable ranges add flexibility but also complexity to the optimization. Monotonicity is not required for these splines. They are often initialized close to the identity function ($y=x$) or a scaled identity within their domain/codomain range to facilitate initial learning.
...
\begin{enumerate}
    \item \textbf{Training speed and stability:}
        While naive implementations can be 10x slower than MLPs, recent architectural enhancements (residual connections, adaptive learning rates, better initialization) have dramatically improved training speed. Networks can now converge in comparable timeframes to MLPs, though careful implementation of the enhancements is crucial. Key improvements include using log-space parameterization for monotonic splines to maintain smooth gradients, employing gradient clipping (typically max norm $\sim 1.0$) to prevent instabilities, and progressive regularization that decreases smoothness penalties over time. With these improvements, the primary bottleneck shifts from optimization challenges to the computational cost of spline evaluations.
