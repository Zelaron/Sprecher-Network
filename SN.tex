...
\begin{itemize}
    ...
    \end{cases}$$
    where $[B^{(\ell)}(\mathbf{x})]_q$ denotes the $q$-th component of the block's output before the residual connection is applied, and $\mathbf{x}$ is the input to the block.
...
\item When residual connections are present, the output range must be adjusted by the residual contribution. For a scalar residual weight $w$, if $w \geq 0$, the residual adds $[wa, wb]$ to the range where $[a,b]$ is the input range. If $w < 0$, it adds $[wb, wa]$ (note the reversal).
\end{enumerate}
\end{lemma}
