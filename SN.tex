...
where $\alpha$ is a scaling factor (set to 1 for the original Sprecher construction, as is done in the rest of this paper, but it can be adjusted for optimization purposes). In practice, setting $\alpha = 1$ can lead to initial training plateaus, particularly for deeper networks, so smaller values (e.g., $\alpha = 0.1$) may improve optimization dynamics.
...
\item in even layers $k = 2\ell $, the learnable weight matrix $W^{(k)} \in \mathbb{R}^{d_\ell \times d_\ell d_{\ell-1}}$ is block-diagonal with blocks of size $1 \times d_{\ell-1}$:
        \begin{align*}
            W^{(k)} = \text{diag}\left([\lambda_{1,0}^{(\ell)}, \ldots, \lambda_{d_{\ell-1},0}^{(\ell)}], [\lambda_{1,1}^{(\ell)}, \ldots, \lambda_{d_{\ell-1},1}^{(\ell)}], \ldots, [\lambda_{1,d_\ell-1}^{(\ell)}, \ldots, \lambda_{d_{\ell-1},d_\ell-1}^{(\ell)}]\right),
        \end{align*}
...
\item \textbf{Shift bound.}  
      $0<\eta^{(\ell)}\le\eta_{\max}$ for some constant $\eta_{\max}$, and consequently $0\le\eta^{(\ell)}q\le\eta^{(\ell)}(d_\ell-1)\le\eta_{\max}(d_{\max}-1)=:\Delta$ for every $q=0,\dots ,d_\ell-1$, where $d_{\max} = \max_{\ell} d_\ell$.
...
\begin{itemize}
    ...
    \item \textbf{Inner spline $\phi^{(\ell)}$:}
        \begin{itemize}
            ...
            \item \textbf{Monotonicity:} Enforced during training through a log-space parameterization. The implementation uses log-increments as the learnable parameters: $\text{log\_increments} = [\ell_0, \ell_1, ..., \ell_{G-1}]$. The actual spline coefficients are computed as: increments $= \text{softplus}(\ell_i)$, followed by cumulative sum and normalization to $[0,1]$, then scaling to the output range. This approach ensures strict monotonicity throughout training without requiring explicit constraints or post-processing operations like sorting, and provides better gradient flow than alternative methods.
            \item \textbf{Initialization:} Initialized to be roughly linear and increasing within its range. In practice, the log-space parameterization (using `log\_increments' with softplus activation) ensures strict monotonicity throughout training without requiring explicit constraints or post-processing.
        \end{itemize}
