...
def train_network(dataset, architecture, total_epochs=4000, print_every=400, 
    ...
    if best_checkpoint is not None and not no_load_best:
        ...
        # Get a sample output before loading checkpoint (without mutating BN state)
        with torch.no_grad(), freeze_bn_running_stats(model):
        ...
        if 'model_params' in best_checkpoint:
            ...
            with torch.no_grad(), freeze_bn_running_stats(fresh_model):
        ...
        # Verify outputs after loading — *without* mutating BN buffers
        if has_bn and CONFIG.get('debug_checkpoint_loading', False):
        ...
        with torch.no_grad(), freeze_bn_running_stats(model):
            _ = model(x_train[:5]).cpu().numpy()
        print(f"Sample outputs AFTER loading checkpoint: [omitted; BN-frozen verification]")
        ...
        # If we have saved training data, verify the checkpoint deterministically
        if 'x_train' in best_checkpoint:
            print("\nVerifying checkpoint with saved training data (BN frozen; batch stats, no updates)...")
            saved_x = best_checkpoint['x_train'].to(device)
            saved_y = best_checkpoint['y_train'].to(device)
            saved_output = best_checkpoint['output']
            
            with torch.no_grad(), use_batch_stats_without_updating_bn(model):
            ...
            # Tolerate tiny numerical noise (GPU/CPU or minor NonDet)
            abs_tol = 1e-8 if not best_checkpoint.get('has_batchnorm', False) else 5e-4
            rel_tol = 1e-4
            loss_close = abs(current_loss - best_checkpoint['loss']) <= max(abs_tol, rel_tol * abs(best_checkpoint['loss']))
            ...
            if not (loss_close and out_close):
                ...
                print(f"Loss |Δ|: {abs(current_loss - best_checkpoint['loss']):.4e}  (tol={max(abs_tol, rel_tol*abs(best_checkpoint['loss'])):.2e})")
        ...
        if best_checkpoint.get('has_batchnorm', False):
            ...
            if bn_recalc_on_load:
                ...
                with torch.no_grad(), freeze_bn_running_stats(model):
                    _ = model(x_train[:5]).cpu().numpy()
                print(f"Sample outputs AFTER BN recalc: [omitted; BN-frozen verification]")
            else:
