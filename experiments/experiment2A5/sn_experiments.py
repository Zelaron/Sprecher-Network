...
from sn_core import (
    ...
    export_parameters,
    parse_param_types,
    evaluation_mode,  # <-- strict eval wrapper for all post-training forwards
)
...
def profile_memory_usage(model, x_sample):
    """Profile memory usage of forward pass (now under strict eval mode)."""
    import torch.cuda
    ...
    if x_sample.is_cuda:
        ...
        # STRICT EVAL: ensure BN uses running stats and nothing mutates
...
def main():
    ...
    # Verify the snapshot works correctly with multiple computations (STRICT EVAL)
    print("\nVerifying plotting snapshot consistency...")
    print("Computing loss multiple times in eval() to ensure perfect reproducibility:")

    saved_eval_loss = float(plotting_snapshot.get("loss_eval", plotting_snapshot["loss"]))
    losses_verification = []
    for i in range(5):
        with evaluation_mode(model):
    ...
    print(f"\nSaved loss from snapshot (eval): {saved_eval_loss:.4e}")
    ...
    print(f"Max difference from saved (eval): {max_diff:.4e}")

    if max_diff < 1e-8:
        print("[OK] Perfect eval consistency achieved! The snapshot is completely isolated.")

    # Keep model in training mode for any future training; evaluation_mode() handles eval forwards safely.
    # model.eval()  # do not flip global mode

    # Print final domain information (no forwards)
    print("\nFinal domain ranges:")
    ...
    # Memory usage profiling if requested (STRICT EVAL inside helper)
    ...
    # Test the full model vs just the layers (STRICT EVAL)
    ...
    # Also check current global mode (should remain training outside eval blocks)
    print(f"Model training mode: {model.training}")

    # DEBUG: Testing with same points as checkpoint loading (STRICT EVAL)
    ...
    # Export parameters if requested (no forwards)
    if CONFIG.get("export_params"):
        ...
        checkpoint_info = {
            ...
            "loss": plotting_snapshot.get("loss_eval", plotting_snapshot.get("loss", "Unknown")),  # prefer eval
        }
    ...
    try:
        ...
        if not args.no_show:
            ...
            import matplotlib.pyplot as plt  # ensure correct backend
            plt.show()
        else:
            import matplotlib.pyplot as plt
        ...
        # STRICT EVAL even in fallback path
        with evaluation_mode(model):
