...
# Force standard PyTorch evaluation semantics (eval() + no_grad()) for plotting
from .train import evaluating as evaluation_mode


# ---------------------------------------------------------------------
# Eval-mode helper for plotting/visualization forwards
# ---------------------------------------------------------------------
@contextmanager
def nonmutating_infer(model):
    ...
    Context manager for visualization/evaluation forwards that:
      ...
      - FORCES true evaluation semantics (model.eval()) and runs under torch.no_grad()
      - Restores the model's prior train/eval state afterwards

    This ensures plots use running stats for BN and disables dropout,
    matching standard PyTorch evaluation behavior.
    """
...
def plot_results(model, layers, dataset=None, save_path=None,
    ...
    All model evaluations in this routine run in **eval mode**.
    """
