...
\begin{itemize}
    \item \textbf{Residual connections (Soft Routing):} When enabled, residual connections are implemented using a learnable soft routing mechanism that adapts based on dimensional relationships between adjacent layers. This approach uses attention-like patterns to create flexible, learnable connections while maintaining parameter efficiency at $O(\max(d_{\mathrm{in}}, d_{\mathrm{out}}))$:
    \begin{itemize}
        \item \textbf{Identity} ($d_{\mathrm{in}} = d_{\mathrm{out}}$): Uses a single learnable scalar weight $w_{\text{res}}$
        \item \textbf{Broadcasting} ($d_{\mathrm{in}} < d_{\mathrm{out}}$): Each output dimension learns a soft attention pattern over inputs using temperature-controlled softmax
        \item \textbf{Pooling} ($d_{\mathrm{in}} > d_{\mathrm{out}}$): Inputs are assigned to outputs via soft assignment with optional importance weighting
    \end{itemize}
    ...
    \sum_{i=0}^{d_{\mathrm{in}}-1} \alpha_{q,i} \cdot x_i & \text{if } d_{\mathrm{in}} < d_{\mathrm{out}}\\
    \sum_{i \in \mathcal{A}_q} \beta_i \cdot \omega_i \cdot x_i & \text{if } d_{\mathrm{in}} > d_{\mathrm{out}}
    \end{cases}$$
    where:
    \begin{itemize}
        \item For broadcasting: $\alpha_{q,i} = \frac{\exp((r_q + i \cdot \tau)/\tau)}{\sum_{j=0}^{d_{\mathrm{in}}-1} \exp((r_q + j \cdot \tau)/\tau)}$ with learnable routing logits $r_q$ for each output $q$
        \item For pooling: $\mathcal{A}_q = \{i : \arg\max_j P_{i,j} = q\}$ where $P_{i,j} = \frac{\exp((r_i + j \cdot \tau)/\tau)}{\sum_{k=0}^{d_{\mathrm{out}}-1} \exp((r_i + k \cdot \tau)/\tau)}$ represents soft assignment probabilities, $\beta_i = P_{i,q}$ are the assignment weights, and $\omega_i = \sigma(r_i)$ are optional importance weights (when enabled)
        \item $\tau > 0$ is a temperature parameter controlling the sharpness of attention/assignment
        \item $r \in \mathbb{R}^{d_{\mathrm{out}}}$ (broadcasting) or $r \in \mathbb{R}^{d_{\mathrm{in}}}$ (pooling) are learnable routing logits
    \end{itemize}
...
\begin{remark}[Soft routing as learnable dimension adaptation]
The soft routing mechanism can be understood as learning optimal ways to adapt between dimensions. In the pooling case ($d_{\mathrm{in}} > d_{\mathrm{out}}$), the network learns which inputs should contribute to each output through soft assignment probabilities. The optional importance weights $\omega_i = \sigma(r_i)$ allow the network to learn that some inputs may be more globally important than others. In the broadcasting case ($d_{\mathrm{in}} < d_{\mathrm{out}}$), each output learns an attention pattern over the available inputs, effectively learning how to best utilize limited input information.
\end{remark}

\begin{remark}[Relationship to attention mechanisms]
The soft routing design bears similarity to attention mechanisms in transformers, but with crucial differences. While transformers compute pairwise attention between all positions, our routing mechanism computes attention between fixed input and output indices with a structured affinity bias ($i \cdot \tau$ or $j \cdot \tau$). This bias encourages but doesn't enforce locality - nearby indices have higher baseline affinity, but the learnable logits can override this preference. The temperature parameter $\tau$ controls this trade-off: lower temperatures create sharper, more localized routing, while higher temperatures allow more flexible assignment patterns.
\end{remark}
...
Sprecher Networks have a fundamentally different structure. Each block $\ell$ contains mixing weights $\lambda^{(\ell)}$ with $O(d_{\ell-1})$ parameters, where $d_{\ell-1}$ is the input dimension to that block (crucially, these are vectors, not matrices), shared splines $\phi^{(\ell)}, \Phi^{(\ell)}$ with $2 \times O(G) = O(G)$ parameters per block independent of $N$, and a shift parameter $\eta^{(\ell)}$ with $O(1)$ parameter per block. When normalization is used, each block may have an associated normalization layer adding $O(N)$ parameters (specifically $2N$ for the affine transformation). When soft routing residual connections are used, they add at most $O(\max(d_{\ell-1}, d_\ell))$ parameters per connection: specifically, $d_{\mathrm{out}}$ routing logits for broadcasting or $d_{\mathrm{in}}$ routing logits for pooling. This maintains the linear scaling in width while providing significantly more flexibility than fixed connection patterns. Summing over $L$ (or $L+1$) blocks, the total parameter count scales approximately as $O(LN + LG + L)$, where the linear dependence on $N$ now includes mixing weights, optional normalization parameters, and residual connections.
...
The soft routing mechanism for residual connections requires careful implementation to maintain numerical stability and efficient computation. Key considerations include:

\begin{itemize}
\item \textbf{Temperature initialization}: The temperature parameter $\tau$ is typically set to 1.0, providing a balance between sharp routing and gradient flow. Lower values create more discrete-like routing but may hinder optimization.

\item \textbf{Logit initialization}: Routing logits are initialized with small random values (e.g., $\mathcal{N}(0, 0.1)$) to break symmetry while starting with relatively uniform routing patterns.

\item \textbf{Importance weighting}: For pooling connections, the optional importance weights $\omega_i = \sigma(r_i)$ can be enabled to allow the network to learn that certain inputs should contribute more strongly across all outputs. This adds no additional parameters as it reuses the routing logits.

\item \textbf{Computational efficiency}: Despite the attention-like formulation, soft routing remains efficient by computing the routing patterns once per forward pass and reusing them across the batch dimension.
\end{itemize}

The soft routing approach provides a learnable middle ground between no residual connections and full matrix projections, allowing the network to discover appropriate connection patterns during training while maintaining the $O(N)$ parameter scaling central to the SN architecture.
...
\item When residual connections are present, the output range must be adjusted by the residual contribution. For a scalar residual weight $w$, if $w \geq 0$, the residual adds $[wa, wb]$ to the range where $[a,b]$ is the input range. If $w < 0$, it adds $[wb, wa]$ (note the reversal). For soft routing residuals:
   \begin{itemize}
   \item \textbf{Broadcasting}: The worst-case contribution is $[a, b]$ (when attention is fully concentrated on one input), so the output range is expanded by $[-\max(|a|, |b|), \max(|a|, |b|)]$.
   \item \textbf{Pooling}: Each output receives a weighted combination of inputs. With importance weighting, the worst-case contribution per output is also $[-\max(|a|, |b|), \max(|a|, |b|)]$.
   \end{itemize}
\end{enumerate}
\end{lemma}
...
For (4), the soft routing residual contributions follow from analyzing the attention and assignment patterns. In both broadcasting and pooling cases, the worst-case occurs when the routing concentrates all weight on the input with the largest absolute value.
\end{proof}
