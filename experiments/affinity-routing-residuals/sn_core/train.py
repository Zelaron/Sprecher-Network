...
def train_network(dataset, architecture, total_epochs=4000, print_every=400, 
    ...
    Returns:
        plotting_snapshot: Dictionary containing model and training data for plotting
    ...
    # Compute parameter counts for TRUE SPRECHER NETWORK with soft routing
    lambda_params = 0
    eta_params = 0
    spline_params = 0
    residual_params = 0
    residual_scalar_params = 0
    residual_routing_params = 0
    codomain_params = 0
    norm_params = 0
    ...
    for name, param in model.named_parameters():
        ...
        elif 'routing_logits' in name:
            # Soft routing parameters (new!)
            if CONFIG['use_residual_weights']:
                residual_routing_params += param.numel()
                residual_params += param.numel()
    ...
    if CONFIG['use_residual_weights'] and residual_params > 0:
        ...
        if residual_routing_params > 0:
            print(f"    * Soft routing logits: {residual_routing_params}")
            print(f"      - Temperature: {CONFIG.get('routing_temperature', 1.0)}")
            print(f"      - Use importance: {CONFIG.get('routing_use_importance', True)}")
    ...
    if CONFIG['use_advanced_scheduler']:
        ...
        params = [
            ...
            {"params": [p for n, p in model.named_parameters() if "routing_logits" in n], 
             "lr": 0.001, "lr_scale": 0.7},  # Medium-high LR for routing params
            {"params": [p for n, p in model.named_parameters() if "phi_codomain_params" not in n and "output" not in n and "routing_logits" not in n], 
        ...
        if CONFIG['train_phi_codomain']:
            params = [
                ...
                {"params": [p for n, p in model.named_parameters() if "routing_logits" in n], "lr": 0.0005},
                {"params": [p for n, p in model.named_parameters() if "phi_codomain_params" not in n and "routing_logits" not in n], "lr": 0.0003}
            ]
        else:
            params = [
                {"params": [p for n, p in model.named_parameters() if "routing_logits" in n], "lr": 0.0005},
                {"params": [p for n, p in model.named_parameters() if "routing_logits" not in n], "lr": 0.0003}
            ]
    ...
    for epoch in pbar:
        ...
        if CONFIG['train_phi_codomain']:
            ...
            if hasattr(first_block, 'phi_codomain_params') and first_block.phi_codomain_params is not None:
                monitoring_values['cc_val'] = first_block.phi_codomain_params.cc.item()
                monitoring_values['cr_val'] = first_block.phi_codomain_params.cr.item()
                monitoring_values['cc_grad'] = first_block.phi_codomain_params.cc.grad.item() if first_block.phi_codomain_params.cc.grad is not None else 0.0
                monitoring_values['cr_grad'] = first_block.phi_codomain_params.cr.grad.item() if first_block.phi_codomain_params.cr.grad is not None else 0.0
        
        # Monitor routing parameters from first layer with routing
        for layer in model.layers:
            if hasattr(layer, 'routing_logits') and layer.routing_logits is not None:
                monitoring_values['routing_mean'] = layer.routing_logits.mean().item()
                monitoring_values['routing_std'] = layer.routing_logits.std().item()
                if layer.routing_logits.grad is not None:
                    monitoring_values['routing_grad'] = layer.routing_logits.grad.abs().mean().item()
                break
        ...
        # Add monitoring values to progress bar
        if 'cc_val' in monitoring_values:
            pbar_dict['cc'] = f'{monitoring_values["cc_val"]:.3f}'
            pbar_dict['cr'] = f'{monitoring_values["cr_val"]:.3f}'
        
        if 'routing_mean' in monitoring_values:
            pbar_dict['rout_μ'] = f'{monitoring_values["routing_mean"]:.3f}'
            pbar_dict['rout_σ'] = f'{monitoring_values["routing_std"]:.3f}'
        
        if CONFIG['use_advanced_scheduler']:
            pbar_dict['lr'] = f'{current_lr:.2e}'
            if 'routing_grad' in monitoring_values:
                pbar_dict['g_rout'] = f'{monitoring_values["routing_grad"]:.3e}'
    ...
    for idx, layer in enumerate(model.layers, start=1):
        ...
        if CONFIG['use_residual_weights']:
            if hasattr(layer, 'residual_weight') and layer.residual_weight is not None:
            ...
            elif hasattr(layer, 'routing_logits') and layer.routing_logits is not None:
                print(f"Block {idx}: routing_logits shape = {tuple(layer.routing_logits.shape)}")
                print(f"Block {idx}: routing_logits =")
                print(layer.routing_logits.detach().cpu().numpy())
                ...
                with torch.no_grad():
                    ...
                    if layer.d_in > layer.d_out:
                        # Pooling case
                        print(f"Block {idx}: Soft assignment pattern (input→output):")
                        routing_matrix = layer.routing_logits.view(-1, 1).expand(layer.d_in, layer.d_out)
                        affinity = routing_matrix + torch.arange(layer.d_out, device=device).float() * temp
                        weights = F.softmax(affinity / temp, dim=1)
                        print(weights.cpu().numpy())
                        
                        if CONFIG.get('routing_use_importance', True):
                            importance = torch.sigmoid(layer.routing_logits)
                            print(f"Block {idx}: Importance weights:")
                            print(importance.cpu().numpy())
                    else:
                        # Broadcasting case
                        print(f"Block {idx}: Soft attention pattern (output←input):")
                        routing_matrix = layer.routing_logits.view(-1, 1).expand(layer.d_out, layer.d_in)
                        affinity = routing_matrix + torch.arange(layer.d_in, device=device).float() * temp
                        weights = F.softmax(affinity / temp, dim=1)
                        print(weights.cpu().numpy())
