...
def export_parameters(model, param_types, save_path, dataset_info=None, checkpoint_info=None):
    ...
    # Export Residual parameters with learnable soft routing
    if 'residual' in categories and CONFIG['use_residual_weights']:
        ...
        for i, layer in enumerate(model.layers):
            if hasattr(layer, 'residual_weight') and layer.residual_weight is not None:
                ...
                # New soft routing case
                if layer.d_in > layer.d_out:
                    lines.append(f"Layer {i} ({layer.d_in} → {layer.d_out}): SOFT POOLING")
                    lines.append(f"  Routing logits (per input):")
                else:
                    lines.append(f"Layer {i} ({layer.d_in} → {layer.d_out}): SOFT BROADCASTING")  
                    lines.append(f"  Routing logits (per output):")
                
                lines.append(format_tensor(layer.routing_logits, indent="    "))
                
                # Show the resulting attention/assignment pattern
                with torch.no_grad():
                    temp = CONFIG.get('routing_temperature', 1.0)
                    if layer.d_in > layer.d_out:
                        # Show assignment pattern for pooling
                        ...
                        lines.append("  Soft assignment weights (input→output probabilities):")
                        lines.append(format_tensor(weights, indent="    "))
                        
                        # Also show importance weights if used
                        if CONFIG.get('routing_use_importance', True):
                            importance = torch.sigmoid(layer.routing_logits)
                            lines.append("  Importance weights (per input):")
                            lines.append(format_tensor(importance, indent="    "))
                    else:
                        # Show attention pattern for broadcasting
                        ...
                        weights = F.softmax(affinity / temp, dim=1)
                        lines.append("  Soft attention weights (output←input probabilities):")
                        lines.append(format_tensor(weights, indent="    "))
