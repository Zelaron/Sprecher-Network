...
class SprecherLayerBlock(nn.Module):
    ...
    def __init__(self, d_in, d_out, layer_num=0, is_final=False, phi_knots=100, Phi_knots=100,
        ...
        if CONFIG['use_residual_weights']:
            if d_in == d_out:
                ...
                # Pooling case: learnable routing vector for inputs
                self.residual_weight = None
                self.routing_logits = nn.Parameter(torch.zeros(d_in))
                # Initialize with small noise to break symmetry
                nn.init.normal_(self.routing_logits, mean=0.0, std=CONFIG['routing_init_std'])
            else:  # d_in < d_out
                # Broadcasting case: learnable routing vector for outputs
                self.residual_weight = None
    ...
    def compute_output_range_theoretical(self):
        ...
        if CONFIG['use_residual_weights'] and self.input_range is not None:
            if self.residual_weight is not None:
                ...
                # For soft routing, we need conservative bounds
                # The routing weights are normalized probabilities, so max contribution
                # per output is the full input range (when attention is 1.0 on one input)
                input_bound = max(abs(self.input_range.min), abs(self.input_range.max))
                
                if self.d_in > self.d_out:
                    # Pooling: importance weights can amplify
                    if CONFIG.get('routing_use_importance', True):
                        importance_bound = torch.sigmoid(self.routing_logits.abs().max()).item()
                        residual_bound = importance_bound * input_bound
                    else:
                        residual_bound = input_bound
                else:
                    # Broadcasting: direct weighted sum with probabilities
                    residual_bound = input_bound
    ...
    def forward(self, x, x_original=None):
        ...
        # Add residual connection with learnable soft routing if enabled
        if CONFIG['use_residual_weights'] and x_original is not None:
            ...
            elif self.d_in > self.d_out:  # Pooling case
                # Get temperature parameter
                temp = CONFIG.get('routing_temperature', 1.0)
                
                # Compute soft assignment matrix
                routing_matrix = self.routing_logits.view(-1, 1).expand(self.d_in, self.d_out)
                affinity = routing_matrix + torch.arange(self.d_out, device=x_original.device).float() * temp
                assignment_weights = F.softmax(affinity / temp, dim=1)  # (d_in, d_out)
                
                # Apply importance scaling and routing
                if CONFIG.get('routing_use_importance', True):
                    importance = torch.sigmoid(self.routing_logits).view(1, -1)  # (1, d_in)
                    weighted_inputs = x_original * importance  # (batch, d_in)
                else:
                    weighted_inputs = x_original
                
                residual_contribution = weighted_inputs @ assignment_weights  # (batch, d_out)
                activated = activated + residual_contribution
                
            else:  # Broadcasting case (d_in < d_out)
                # Get temperature parameter
                temp = CONFIG.get('routing_temperature', 1.0)
                
                # Compute soft attention matrix
                routing_matrix = self.routing_logits.view(-1, 1).expand(self.d_out, self.d_in)
                affinity = routing_matrix + torch.arange(self.d_in, device=x_original.device).float() * temp
                attention_weights = F.softmax(affinity / temp, dim=1)  # (d_out, d_in)
                
                # Apply attention-based routing
                residual_contribution = attention_weights @ x_original.T  # (d_out, batch)
                residual_contribution = residual_contribution.T  # (batch, d_out)
