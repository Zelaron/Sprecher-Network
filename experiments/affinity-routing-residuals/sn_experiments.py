...
def parse_args():
    ...
    parser.add_argument("--no_importance", action="store_true",
                      help="Disable importance weighting in pooling (default: enabled)")
...
def get_config_suffix(args, CONFIG):
    ...
    # Check routing temperature if non-default
    if args.routing_temperature is not None and args.routing_temperature != 1.0:
        parts.append(f"Temp{args.routing_temperature:.1f}")
    
    # Check importance weighting
    if args.no_importance:
        parts.append("NoImportance")
...
def main():
    ...
    if args.no_importance:
        CONFIG['routing_use_importance'] = False
    ...
    if CONFIG.get('use_residual_weights', True):
        print(f"  - Soft routing temperature: {CONFIG.get('routing_temperature', 1.0)}")
        print(f"  - Use importance weighting: {CONFIG.get('routing_use_importance', True)}")
