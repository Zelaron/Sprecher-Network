...
class SprecherMultiLayerNetwork(nn.Module):
    ...
    def __init__(self, input_dim, architecture, final_dim=1, phi_knots=100, Phi_knots=100,
        ...
        # Calculate adaptive knot counts if enabled
        def get_adaptive_knots(layer_idx, base_knots, is_phi_spline=False):
            if CONFIG.get('use_adaptive_splines', True):
                # Check if we should only adapt Phi splines
                if CONFIG.get('adaptive_phi_only', True) and is_phi_spline:
                    # Keep phi splines at base resolution
                    return base_knots
                # Calculate knots based on layer depth
                base = CONFIG.get('adaptive_spline_base', 50)
                growth = CONFIG.get('adaptive_spline_growth', 1.5)
                return int(base * (growth ** layer_idx))
            else:
                return base_knots
        ...
        if not architecture: # No hidden layers
            ...
            Phi_domain = domain_ranges.get('layer_0_Phi') if domain_ranges else None
            # Use adaptive knots for layer 0
            layer_phi_knots = get_adaptive_knots(0, phi_knots, is_phi_spline=True)
            layer_Phi_knots = get_adaptive_knots(0, Phi_knots, is_phi_spline=False)
            layers.append(SprecherLayerBlock(
                d_in=input_dim, d_out=final_dim,
                layer_num=0, is_final=is_final,
                phi_knots=layer_phi_knots, Phi_knots=layer_Phi_knots,
            ...
            for i in range(L):
                ...
                Phi_domain = domain_ranges.get(f'layer_{i}_Phi') if domain_ranges else None
                # Use adaptive knots for layer i
                layer_phi_knots = get_adaptive_knots(i, phi_knots, is_phi_spline=True)
                layer_Phi_knots = get_adaptive_knots(i, Phi_knots, is_phi_spline=False)
                layers.append(SprecherLayerBlock(
            ...
            if self.final_dim > 1:
                ...
                Phi_domain = domain_ranges.get(f'layer_{L}_Phi') if domain_ranges else None
                # Use adaptive knots for final layer
                layer_phi_knots = get_adaptive_knots(L, phi_knots, is_phi_spline=True)
                layer_Phi_knots = get_adaptive_knots(L, Phi_knots, is_phi_spline=False)
                layers.append(SprecherLayerBlock(
