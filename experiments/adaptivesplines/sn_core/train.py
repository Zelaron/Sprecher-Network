...
def train_network(dataset, architecture, total_epochs=4000, print_every=400, 
    ...
    # Print adaptive spline information if enabled
    if CONFIG.get('use_adaptive_splines', True):
        adaptive_mode = "Phi only" if CONFIG.get('adaptive_phi_only', True) else "both phi and Phi"
        print(f"Adaptive splines: ENABLED for {adaptive_mode} (base={CONFIG.get('adaptive_spline_base', 50)}, growth={CONFIG.get('adaptive_spline_growth', 1.5)})")
        print("Knots per layer:")
        for i, layer in enumerate(model.layers):
            print(f"  Layer {i}: φ={layer.phi.num_knots} knots, Φ={layer.Phi.num_knots} knots")
    else:
        print(f"Adaptive splines: DISABLED (using {phi_knots} knots for all splines)")
