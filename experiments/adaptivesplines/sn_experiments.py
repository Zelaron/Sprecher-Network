...
def parse_args():
    ...
    parser.add_argument("--adaptive_splines", nargs='?', const=1.5, type=float,
                      help="Enable adaptive spline resolution (default growth: 1.5). "
                           "Optionally specify growth factor (e.g., --adaptive_splines 2.0)")
...
def get_config_suffix(args, CONFIG):
    ...
    # Check adaptive splines (default is disabled)
    if CONFIG.get('use_adaptive_splines', False):
        growth = CONFIG.get('adaptive_spline_growth', 1.5)
        if growth != 1.5:
            parts.append(f"Adaptive{growth}")
        else:
...
def main():
    ...
    if args.adaptive_splines is not None:
        CONFIG['use_adaptive_splines'] = True
        CONFIG['adaptive_spline_growth'] = args.adaptive_splines
