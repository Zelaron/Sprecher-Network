...
def parse_args():
    ...
    # Soft routing arguments (NEW)
    parser.add_argument("--no_soft_routing", action="store_true",
                      help="Use old round-robin routing instead of soft routing (default: soft routing enabled)")
    parser.add_argument("--routing_temperature", type=float, default=None,
                      help="Temperature for soft routing (default: from CONFIG)")
    parser.add_argument("--soft_indices_noise", type=float, default=None,
                      help="Initialization noise for soft indices (default: from CONFIG)")
    parser.add_argument("--soft_indices_reg", type=float, default=None,
                      help="L2 regularization for soft indices deviation (default: from CONFIG)")
...
def get_config_suffix(args, CONFIG):
    ...
    elif CONFIG.get('use_residual_weights', True) and not CONFIG.get('use_soft_routing', True):
        parts.append("RoundRobin")  # Explicitly mark when using old routing
    
    # Check scheduler (default is disabled)
    if CONFIG.get('use_advanced_scheduler', False):
        parts.append("AdvScheduler")
    
    # Check routing temperature if not default
    if CONFIG.get('use_soft_routing', True) and CONFIG.get('routing_temperature', 4.0) != 4.0:
        temp_str = f"Temp{CONFIG['routing_temperature']:.1f}".replace('.', '_')
...
def main():
    ...
    # Handle soft routing configuration (NEW)
    if args.no_soft_routing:
        CONFIG['use_soft_routing'] = False
    if args.routing_temperature is not None:
        CONFIG['routing_temperature'] = args.routing_temperature
    if args.soft_indices_noise is not None:
        CONFIG['soft_indices_init_noise'] = args.soft_indices_noise
    if args.soft_indices_reg is not None:
        CONFIG['soft_indices_regularization'] = args.soft_indices_reg
    ...
    if CONFIG.get('use_residual_weights', True):
        ...
        if CONFIG.get('use_soft_routing', True):
            ...
            print(f"  Soft indices init noise: {CONFIG.get('soft_indices_init_noise', 0.01)}")
            if CONFIG.get('soft_indices_regularization', 0.0) > 0:
                print(f"  Soft indices regularization: {CONFIG.get('soft_indices_regularization', 0.0)}")
