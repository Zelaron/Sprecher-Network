...
def visualize_soft_routing(layer, max_visualized=5):
    """
    Visualize the soft routing matrix for a layer.
    
    Args:
        layer: SprecherLayerBlock with soft routing
        max_visualized: Maximum number of connections to visualize
        
    Returns:
        List of strings describing the routing
    """
    lines = []
    
    if not hasattr(layer, 'soft_indices') or layer.soft_indices is None:
        return lines
    
    # Compute the routing matrix
    routing_matrix = layer.compute_soft_routing_matrix(layer.d_in, layer.d_out)
    routing_np = routing_matrix.detach().cpu().numpy()
    
    if layer.d_in > layer.d_out:
        # Pooling case: show where each input goes
        lines.append("  Routing visualization (Pooling: d_in > d_out):")
        num_show = min(max_visualized, layer.d_in)
        for i in range(num_show):
            # Get the weights for this input
            weights = routing_np[i, :]
            # Find top 3 destinations
            top_indices = np.argsort(weights)[::-1][:3]
            dest_str = ", ".join([f"out[{j}] ({weights[j]:.3f})" for j in top_indices if weights[j] > 0.01])
            lines.append(f"    Input[{i}] → {dest_str}")
        if layer.d_in > num_show:
            lines.append(f"    ... ({layer.d_in - num_show} more inputs)")
    else:
        # Broadcasting case: show where each output gets its input from
        lines.append("  Routing visualization (Broadcasting: d_in < d_out):")
        num_show = min(max_visualized, layer.d_out)
        # For broadcasting, the matrix is (d_in, d_out), so we look at columns
        for j in range(num_show):
            # Get the weights for this output
            weights = routing_np[:, j]
            # Find top 3 sources
            top_indices = np.argsort(weights)[::-1][:3]
            src_str = ", ".join([f"in[{i}] ({weights[i]:.3f})" for i in top_indices if weights[i] > 0.01])
            lines.append(f"    Output[{j}] ← {src_str}")
        if layer.d_out > num_show:
            lines.append(f"    ... ({layer.d_out - num_show} more outputs)")
...
def export_parameters(model, param_types, save_path, dataset_info=None, checkpoint_info=None):
    ...
    # Export Residual parameters (updated for soft routing)
    if 'residual' in categories and CONFIG['use_residual_weights']:
        ...
        for i, layer in enumerate(model.layers):
            ...
            # Check for soft routing
            elif hasattr(layer, 'soft_indices') and layer.soft_indices is not None:
                lines.append(f"Layer {i} ({layer.d_in} → {layer.d_out}): SOFT ROUTING residual")
                lines.append(f"  Routing temperature: {layer.routing_temperature}")
                lines.append(format_tensor(layer.soft_indices[:max(layer.d_in, layer.d_out)], "Soft indices", "  "))
                
                if layer.residual_pooling_weights is not None:
                    lines.append(format_tensor(layer.residual_pooling_weights, "Pooling weights", "  "))
                    lines.append("  Note: Each input is weighted and routed to outputs based on soft indices")
                else:
                    lines.append("  Note: Each output receives weighted combination of inputs based on soft indices")
                
                # Add routing visualization
                routing_vis = visualize_soft_routing(layer)
                lines.extend(routing_vis)
                has_residual = True
            
            # Check for old round-robin pooling weights (d_in > d_out)
            elif hasattr(layer, 'residual_pooling_weights') and layer.residual_pooling_weights is not None:
                lines.append(f"Layer {i} ({layer.d_in} → {layer.d_out}): POOLING residual (round-robin)")
                lines.append(format_tensor(layer.residual_pooling_weights, "Pooling weights", "  "))
            ...
            # Check for old round-robin broadcast weights (d_in < d_out)
            elif hasattr(layer, 'residual_broadcast_weights') and layer.residual_broadcast_weights is not None:
                lines.append(f"Layer {i} ({layer.d_in} → {layer.d_out}): BROADCAST residual (round-robin)")
    ...
    for name, param in model.named_parameters():
        ...
        if 'lambdas' in name:
            param_counts['lambda'] = param_counts.get('lambda', 0) + param.numel()
        elif 'eta' in name:
            param_counts['eta'] = param_counts.get('eta', 0) + param.numel()
        elif 'coeffs' in name or 'log_increments' in name:
            param_counts['spline'] = param_counts.get('spline', 0) + param.numel()
        elif 'soft_indices' in name:
            param_counts['soft_routing'] = param_counts.get('soft_routing', 0) + param.numel()
        elif 'residual' in name:
            param_counts['residual'] = param_counts.get('residual', 0) + param.numel()
        elif 'phi_codomain_params' in name:
            param_counts['codomain'] = param_counts.get('codomain', 0) + param.numel()
        elif 'norm_layers' in name:
            param_counts['normalization'] = param_counts.get('normalization', 0) + param.numel()
        elif 'output' in name:
            param_counts['output'] = param_counts.get('output', 0) + param.numel()
