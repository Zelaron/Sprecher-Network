...
class SprecherLayerBlock(nn.Module):
    ...
    def __init__(self, d_in, d_out, layer_num=0, is_final=False, phi_knots=100, Phi_knots=100,
        ...
        # Node-centric residual connection with soft routing
        if CONFIG['use_residual_weights']:
            if d_in == d_out:
                # When dimensions match, use a simple scalar weight
                self.residual_weight = nn.Parameter(torch.tensor(0.1))
                self.soft_indices = None
                self.residual_pooling_weights = None
            elif CONFIG.get('use_soft_routing', True):
                # Use soft routing for dimension mismatch cases
                self.residual_weight = None
                
                # Single learnable soft index vector
                max_dim = max(d_in, d_out)
                # Initialize uniformly spaced between 0 and 1
                init_indices = torch.linspace(0, 1, max_dim)
                # Add small random noise to break symmetry
                init_indices += torch.randn(max_dim) * CONFIG.get('soft_indices_init_noise', 0.01)
                init_indices = torch.clamp(init_indices, 0, 1)
                self.soft_indices = nn.Parameter(init_indices)
                
                # Store routing temperature
                self.routing_temperature = CONFIG.get('routing_temperature', 4.0)
                
                if d_in > d_out:
                    # Pooling case: each input has a learnable weight
                    init_value = 1.0 / d_in
                    self.residual_pooling_weights = nn.Parameter(
                        torch.ones(d_in) * init_value
                    )
                else:
                    # Broadcasting case: no additional weights needed
                    self.residual_pooling_weights = None
            else:
                # Fallback to old round-robin routing (if soft routing is disabled)
                self.residual_weight = None
                self.soft_indices = None
                
                if d_in > d_out:
                    # Pooling case with round-robin
                    init_value = 1.0 / d_in
                    self.residual_pooling_weights = nn.Parameter(
                        torch.ones(d_in) * init_value
                    )
                    ...
                    # Broadcasting case with round-robin
    ...
    def compute_soft_routing_matrix(self, source_dim, target_dim):
        """
        Compute the soft routing matrix based on learnable soft indices.
        
        Args:
            source_dim: Number of source dimensions
            target_dim: Number of target dimensions
            
        Returns:
            routing_matrix: Tensor of shape (source_dim, target_dim) containing routing weights
        """
        device = self.soft_indices.device
        
        if source_dim > target_dim:
            # Pooling case: soft_indices[0:source_dim] determine which output each input goes to
            source_indices = self.soft_indices[:source_dim] * (target_dim - 1)
            
            # Compute routing weights
            target_positions = torch.arange(target_dim, device=device).float()
            # Shape: (source_dim, target_dim)
            routing_weights = torch.exp(
                -self.routing_temperature * 
                (source_indices.unsqueeze(1) - target_positions.unsqueeze(0)) ** 2
            )
            # Normalize so each input's contributions sum to 1
            routing_weights = routing_weights / routing_weights.sum(dim=1, keepdim=True)
            
        else:  # source_dim < target_dim
            # Broadcasting case: soft_indices[0:target_dim] determine which input each output receives from
            target_indices = self.soft_indices[:target_dim] * (source_dim - 1)
            
            # Compute routing weights
            source_positions = torch.arange(source_dim, device=device).float()
            # Shape: (target_dim, source_dim)
            routing_weights = torch.exp(
                -self.routing_temperature * 
                (target_indices.unsqueeze(1) - source_positions.unsqueeze(0)) ** 2
            )
            # Normalize so each output's sources sum to 1
            routing_weights = routing_weights / routing_weights.sum(dim=1, keepdim=True)
            # Transpose to get (source_dim, target_dim) for consistent matmul
            routing_weights = routing_weights.t()
    ...
    def compute_output_range_theoretical(self):
        ...
        if CONFIG['use_residual_weights'] and self.input_range is not None:
            ...
            elif self.soft_indices is not None:
                # For soft routing, we need more conservative bounds
                # The routing matrix has normalized rows, so the max contribution
                # is bounded by the input range
                input_bound = max(abs(self.input_range.min), abs(self.input_range.max))
                
                if self.residual_pooling_weights is not None:
                    # Pooling case: weighted inputs
                    weight_bound = torch.max(torch.abs(self.residual_pooling_weights)).item()
                    residual_bound = weight_bound * input_bound
                else:
                    # Broadcasting case: direct routing
                    residual_bound = input_bound
                
                final_min = phi_range.min - residual_bound
                final_max = phi_range.max + residual_bound
            else:
                # Old round-robin routing
                if hasattr(self, 'residual_pooling_weights') and self.residual_pooling_weights is not None:
                    weight_sum_max = torch.sum(torch.abs(self.residual_pooling_weights)).item()
                    input_bound = max(abs(self.input_range.min), abs(self.input_range.max))
                    residual_bound = weight_sum_max * input_bound
                    final_min = phi_range.min - residual_bound
                    final_max = phi_range.max + residual_bound
                elif hasattr(self, 'residual_broadcast_weights') and self.residual_broadcast_weights is not None:
        ...
        if CONFIG['use_residual_weights'] and x_original is not None:
            if self.residual_weight is not None:
                ...
                if self.d_in > self.d_out:
                    # Pooling case
                    # Apply per-input weights
                    weighted_input = x_original * self.residual_pooling_weights.view(1, -1)
                    
                    # Compute routing matrix
                    routing_matrix = self.compute_soft_routing_matrix(self.d_in, self.d_out)
                    
                    # Apply routing: (batch, d_in) @ (d_in, d_out) -> (batch, d_out)
                    ...
                    # Apply routing: (batch, d_in) @ (d_in, d_out) -> (batch, d_out)
                    residual_contribution = torch.matmul(x_original, routing_matrix)
                    activated = activated + residual_contribution
                    
            elif hasattr(self, 'pooling_assignment'):
                # Old round-robin pooling logic
                weighted_input = x_original * self.residual_pooling_weights.view(1, -1)
            ...
            elif hasattr(self, 'broadcast_sources'):
                # Old round-robin broadcasting logic
