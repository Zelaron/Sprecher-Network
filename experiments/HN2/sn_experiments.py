...
def parse_args():
    ...
    # NEW: Spline order argument
    parser.add_argument(
        "--spline_order",
        type=int,
        default=1,
        help="Order of splines (1=Piecewise Linear, 3=Cubic B-Spline). Default: 1",
    )
    parser.add_argument(
        "--epochs",
        type=int,
...
def get_config_suffix(args, CONFIG):
    ...
    # Check spline order
    if hasattr(args, 'spline_order') and args.spline_order != 1:
        parts.append(f"Spline{args.spline_order}")
...
def verify_memory_efficient_mode(device="cpu"):
    ...
    layer = SprecherLayerBlock(d_in=100, d_out=200, layer_num=0, spline_order=CONFIG.get('spline_order', 1)).to(device)
...
def main():
    ...
    # Set spline order
    CONFIG["spline_order"] = args.spline_order
    ...
    print(f"Spline Order: {args.spline_order} ({'Cubic B-Spline' if args.spline_order == 3 else 'Piecewise Linear' if args.spline_order == 1 else f'Order {args.spline_order}'})")
