...
Also exports spline order metadata.
"""
...
def export_parameters(model, param_types, save_path, dataset_info=None, checkpoint_info=None):
    ...
    # Add Spline Order info
    spline_order = CONFIG.get('spline_order', 1)
    order_name = "Piecewise Linear" if spline_order == 1 else "Cubic B-Spline" if spline_order == 3 else f"Order {spline_order}"
    lines.append(f"Spline Type: {order_name} (Order {spline_order})")
    ...
    if 'spline' in categories:
        ...
        for i, layer in enumerate(model.layers):
            ...
            lines.append(f"  phi spline (monotonic, order {spline_order}):")
            ...
            # Phi spline (general)
            lines.append(f"  Phi spline (general, order {spline_order}):")
            lines.append(f"    Domain: [{layer.Phi.in_min:.6f}, {layer.Phi.in_max:.6f}]")
