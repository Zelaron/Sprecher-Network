...
class SprecherLayerBlock(nn.Module):
    ...
    def __init__(self, d_in, d_out, layer_num=0, is_final=False, phi_knots=100, Phi_knots=100,
        ...
        # NEW: Adaptive initialization with alternating signs and decay
        lambda_vals = torch.zeros(d_in)
        for i in range(d_in):
            sign = 1.0 if i % 2 == 0 else -1.0
            decay = 1.0 / (1.0 + 0.1 * i)  # Gentle decay factor
            magnitude = np.sqrt(2.0 / d_in) * decay
            lambda_vals[i] = sign * magnitude
        self.lambdas = nn.Parameter(lambda_vals)
