...
class SprecherLayerBlock(nn.Module):
    ...
    def __init__(self, d_in, d_out, layer_num=0, is_final=False, phi_knots=100, Phi_knots=100,
                 phi_domain=None, Phi_domain=None, lambda_init_method='baseline'):
        super().__init__()
        self.d_in = d_in
        self.d_out = d_out
        ...
        self.lambdas = nn.Parameter(self._initialize_lambdas(d_in, layer_num, lambda_init_method))
    ...
    def _initialize_lambdas(self, d_in, layer_num, method):
        """Initialize lambda weights based on selected method."""
        if method == 'baseline':
            # Current initialization
            return torch.randn(d_in) * np.sqrt(2.0 / d_in)
        
        elif method == 'fixed_variance':
            # Constant variance regardless of dimension
            return torch.randn(d_in) * 0.5
        
        elif method == 'uniform':
            # Uniform distribution that considers φ's [0,1] output
            return torch.zeros(d_in).uniform_(-1.0, 1.0)
        
        elif method == 'sparse':
            # Only a few dimensions contribute initially
            lambdas = torch.zeros(d_in)
            active_dims = min(d_in, max(2, int(np.sqrt(d_in))))
            indices = torch.randperm(d_in)[:active_dims]
            lambdas[indices] = torch.randn(active_dims)
            return lambdas
        
        elif method == 'balanced':
            # Ensure roughly equal positive and negative weights
            lambdas = torch.randn(d_in)
            lambdas = lambdas / lambdas.abs().mean()  # Normalize by mean absolute value
            return lambdas
        
        elif method == 'theory_inspired':
            # Based on Sprecher's construction, use carefully chosen values
            if d_in == 2:  # Special case for 2D (Sprecher's original)
                return torch.tensor([1.0, np.sqrt(2)])
            else:
                # Use prime-like multipliers for linear independence
                lambdas = torch.tensor([1.0 + 0.1*i for i in range(d_in)], dtype=torch.float32)
                lambdas[1::2] *= -1  # Alternate signs
                return lambdas
...
class SprecherMultiLayerNetwork(nn.Module):
    ...
    def __init__(self, input_dim, architecture, final_dim=1, phi_knots=100, Phi_knots=100,
                 ...
                 domain_ranges=None, lambda_init_method='baseline'):
        super().__init__()
