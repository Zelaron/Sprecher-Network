...
def main():
    ...
    plotting_snapshot, losses = train_network(
        ...
        lambda_init_method=args.lambda_init
    )
    ...
    try:
        ...
        # Include lambda_init in filename
        lambda_init_str = f"-{args.lambda_init}" if args.lambda_init != "baseline" else ""
        filename = f"{args.lambda_init}-{prefix}-{args.dataset}-{arch_str}-{args.epochs}-epochs-outdim{dataset.output_dim}{config_suffix}.png"
        ...
        # Plot loss curve
        loss_filename = f"{args.lambda_init}-loss-{args.dataset}-{arch_str}-{args.epochs}-epochs{config_suffix}.png"
        loss_save_path = os.path.join("plots", loss_filename) if args.save_plots else None
        plot_loss_curve(losses, loss_save_path)
        
        # Save final loss to losses.txt
        final_loss = losses[-1] if losses else float('inf')
        with open("losses.txt", "a") as f:
            f.write(f"{args.lambda_init},{args.dataset},{arch_str},{args.epochs},{final_loss:.4e}\n")
