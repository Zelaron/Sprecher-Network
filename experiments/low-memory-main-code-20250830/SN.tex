...
Note that $\lambda_i$ depends only on the input index $i$, not on the output index $q$, maintaining fidelity to Sprecher's original 1965 construction. When lateral mixing is enabled, the pre-activation values undergo weighted mixing with neighboring outputs before the $\Phi$ transformation.
...
\begin{align}
    \label{eq:SN}\mathbf{h}^{(\ell)}_q = \Phi^{(\ell)}\Biggl(\,\sum_{i=1}^{d_{\ell-1}} \lambda^{(\ell)}_{i}\,\phi^{(\ell)}\Bigl(\mathbf{h}^{(\ell-1)}_i+\eta^{(\ell)}\,q\Bigr) + q + \tau^{(\ell)} \sum_{j \in \mathcal{N}(q)} \omega_{q,j}^{(\ell)} s_j^{(\ell)}\Biggr),\quad q=0,\dots,d_\ell-1.
\end{align}
...
$$ \Bigl(T^{(\ell)}(z)\Bigr)_q = \Phi^{(\ell)}\Biggl(\,\sum_{i=1}^{d_{\ell-1}} \lambda^{(\ell)}_{i}\,\phi^{(\ell)}\Bigl(z_i+\eta^{(\ell)}\,q\Bigr) + q + \tau^{(\ell)} \sum_{j \in \mathcal{N}(q)} \omega_{q,j}^{(\ell)} s_j^{(\ell)}\Biggr), $$
...
$$ y_q = \Bigl(T^{(L+1)}(\mathbf{h}^{(L)})\Bigr)_q = \Phi^{(L+1)}\Biggl(\,\sum_{r=0}^{d_L-1} \lambda^{(L+1)}_{r}\,\phi^{(L+1)}\Bigl(\mathbf{h}^{(L)}_r+\eta^{(L+1)}\,q\Bigr)+q + \tau^{(L+1)} \sum_{j \in \mathcal{N}(q)} \omega_{q,j}^{(L+1)} s_j^{(L+1)}\Biggr), $$
...
$$f(\mathbf{x}) = \sum_{q=0}^{d_1-1} \mathbf{h}^{(1)}_q = \sum_{q=0}^{d_1-1} \Phi^{(1)}\Biggl(\sum_{i=1}^{d_0} \lambda^{(1)}_{i}\,\phi^{(1)}\Bigl(x_i+\eta^{(1)}\,q\Bigr)+q + \tau^{(1)} \sum_{j \in \mathcal{N}(q)} \omega_{q,j}^{(1)} s_j^{(1)}\Biggr).$$
When lateral mixing is disabled ($\tau^{(1)} = 0$ or $\mathcal{N}(q) = \emptyset$), this reduces to Sprecher's 1965 construction if we choose $d_1=2d_0+1$ and identify $\phi^{(1)}=\phi$, $\Phi^{(1)}=\Phi$, and $\lambda^{(1)}_{i} = \lambda_i$.
...
$$\mathbf{h}^{(1)}_r=\Phi^{(1)}\Bigl(\sum_{i=1}^{d_0}\lambda^{(1)}_{i}\,\phi^{(1)}\Bigl(x_i+\eta^{(1)}\,r\Bigr)+r + \tau^{(1)} \sum_{j \in \mathcal{N}(r)} \omega_{r,j}^{(1)} s_j^{(1)}\Bigr),\quad r=0,\dots,d_1-1.$$
The second block computes $\mathbf{h}^{(2)} \in \mathbb{R}^{d_2}$ using $\mathbf{h}^{(1)}$ as input:
$$\mathbf{h}^{(2)}_q=\Phi^{(2)}\Bigl(\sum_{r=0}^{d_1-1}\lambda^{(2)}_{r}\,\phi^{(2)}\Bigl(\mathbf{h}^{(1)}_r+\eta^{(2)}\,q\Bigr)+q + \tau^{(2)} \sum_{j \in \mathcal{N}(q)} \omega_{q,j}^{(2)} s_j^{(2)}\Bigr),\quad q=0,\dots,d_2-1.$$
...
f(\mathbf{x})=\sum_{q=0}^{d_2-1}\Phi^{(2)}\Biggl(\sum_{r=0}^{d_1-1}\lambda^{(2)}_{r}\,\phi^{(2)}\Biggl(\underbrace{\Phi^{(1)}\Biggl(\sum_{i=1}^{d_0}\lambda^{(1)}_{i}\,\phi^{(1)}\Bigl(x_i+\eta^{(1)}\,r\Bigr)+r\Biggr)}_{\text{Output of first block}}+\eta^{(2)}\,q\Biggr)+q\Biggr).
\end{equation}
...
$$\begin{aligned}
\mathbf{h}^{(1)}_r &= \Phi^{(1)}\Bigl(\sum_{i=1}^{d_0}\lambda^{(1)}_{i}\,\phi^{(1)}\Bigl(x_i+\eta^{(1)}\,r\Bigr)+r + \tau^{(1)} \sum_{j \in \mathcal{N}(r)} \omega_{r,j}^{(1)} s_j^{(1)}\Bigr),\quad r=0,\dots,d_1-1,\\[1mm]
\mathbf{h}^{(2)}_s &= \Phi^{(2)}\Bigl(\sum_{r=0}^{d_1-1}\lambda^{(2)}_{r}\,\phi^{(2)}\Bigl(\mathbf{h}^{(1)}_r+\eta^{(2)}\,s\Bigr)+s + \tau^{(2)} \sum_{j \in \mathcal{N}(s)} \omega_{s,j}^{(2)} s_j^{(2)}\Bigr),\quad s=0,\dots,d_2-1,\\[1mm]
\mathbf{h}^{(3)}_q &= \Phi^{(3)}\Bigl(\sum_{s=0}^{d_2-1}\lambda^{(3)}_{s}\,\phi^{(3)}\Bigl(\mathbf{h}^{(2)}_s+\eta^{(3)}\,q\Bigr)+q + \tau^{(3)} \sum_{j \in \mathcal{N}(q)} \omega_{q,j}^{(3)} s_j^{(3)}\Bigr),\quad q=0,\dots,d_3-1.
\end{aligned}$$
...
\textcolor{red}{\begin{remark}[Note on weight structure]
...
\textcolor{red}{\begin{prop} \label{prop:SNareLAN}
    An SN with matrix weights (cf. \eqref{eq:defsn}) and without lateral mixing is a LAN, where: 
    \begin{itemize}
...
\begin{itemize}
    ...
    \item \textcolor{red}{\textbf{Potential advantage:} For equivalent structure, theoretical reduction factor $\approx 12-15$x. Actual performance requires empirical validation.}
\end{itemize}
...
$$T(\mathbf{z})_q = \Phi\left(\sum_{i=1}^{d_{in}} \lambda_{i} \phi(z_i + \eta q) + q + \tau \sum_{j \in \mathcal{N}(q)} \omega_{q,j} s_j\right), \quad q = 0, \ldots, d_{out}-1$$
where $\phi \in C^{k+1}(\mathbb{R})$ is monotonic, $\Phi \in C^{k+1}(\mathbb{R})$, $|\lambda_{i}| \leq \Lambda$, $\eta > 0$, and $\tau, \omega_{q,j}$ are the lateral mixing parameters.
...
\item $I_\Phi := [-R_{out}, R_{out}]$ where $R_{out} := (1 + \tau \|\omega\|_\infty |\mathcal{N}|_{max})(\Lambda d_{in} \|\phi\|_{L^\infty(I_\phi)} + d_{out})$ (domain accounting for lateral mixing)
\end{itemize}
...
where 
$$K_T := \frac{L_\Phi \Lambda d_{in} (1 + \tau \|\omega\|_\infty |\mathcal{N}|_{max}) \|\phi^{(k+1)}\|_{L^\infty(I_\phi)} + \|\Phi^{(k+1)}\|_{L^\infty(I_\Phi)}}{(k+1)!}$$
...
\item Each block satisfies $|\lambda_{i}^{(\ell)}| \leq \Lambda$, has shift parameter $\eta^{(\ell)} > 0$, and if lateral mixing is enabled, $|\tau^{(\ell)}| \leq T$ and $\|\omega^{(\ell)}\|_\infty \leq W$.
...
where the constants account for lateral mixing when present.
...
\begin{remark}[Impact of lateral mixing on approximation]
The presence of lateral mixing introduces additional terms in the error bounds through the factors $(1 + \tau \|\omega\|_\infty |\mathcal{N}|_{max})$. While this might suggest worse approximation rates, the enhanced expressivity from lateral mixing may allow for smaller required values of $G$ to achieve the same accuracy, potentially offsetting this theoretical disadvantage. The trade-off between approximation rate constants and required spline resolution remains an empirical question.
\end{remark}
...
All learnable parameters (spline coefficients, $\eta^{(\ell)}$, $\lambda^{(\ell)}$, lateral mixing parameters, and potentially range parameters for $\Phi^{(\ell)}$) are trained jointly using gradient-based optimization methods like Adam \cite{kingma2014adam} or LBFGS. The loss function is typically Mean Squared Error (MSE) for regression tasks. Due to the constrained parameter space and shared spline structure, SNs often require more training iterations than equivalent MLPs or KANs to converge, though the per-iteration computational cost is typically lower due to fewer parameters.
...
$$s_{b,q} = \sum_{i=1}^{d_{\mathrm{in}}} \lambda_i \phi(x_{b,i} + \eta q) + q$$
computed sequentially for $q = 0, \ldots, d_{\mathrm{out}}-1$.

This reformulation reduces peak memory from $O(B \cdot N^2)$ to $O(B \cdot N)$ during computation, while producing mathematically identical results. Combined with SNs' $O(N)$ parameter memory, the total memory complexity becomes $O(N + B \cdot N) = O(N)$ when treating batch size as a constant, compared to MLPs' $O(N^2)$ total memory requirement.

In practice, this enables training of architectures that would otherwise be infeasible. For example, a network with layers of width 512 might exhaust 32GB of RAM in parallel computation but trains comfortably with sequential processing. Empirically, we observe memory reductions of 30-60\% for wide architectures (layers exceeding 128 units) with computational overhead of only 15-25\%, making this a favorable trade-off when memory is the binding constraint.

\begin{remark}[Preservation of mathematical structure]
The sequential computation is a pure implementation optimization that maintains exact mathematical equivalence to the parallel formulation. The Sprecher formula remains unchanged; only the order of operations differs. This contrasts with approximation techniques like gradient checkpointing or reduced precision, which alter the computed values.
\end{remark}

\begin{remark}[Parallelism vs. memory trade-off]
The sequential computation sacrifices GPU parallelism for memory efficiency. Modern accelerators with abundant parallelism but limited memory often benefit from this trade-off, especially during experimentation with architecture search where wide layers might be explored. The approach can be viewed as a form of gradient checkpointing applied at the intra-block level rather than between layers.
\end{remark}
...
\item With lateral mixing, the domain for $\Phi$ must be expanded to account for the mixing terms. For cyclic mixing with scale $\tau$ and weights $\omega$:
$$\mathcal{D}_\Phi = \left[(1 - |\tau| \|\omega\|_\infty) \sum_{i: \lambda_i < 0} \lambda_i, (1 + |\tau| \|\omega\|_\infty)\left(\sum_{i: \lambda_i > 0} \lambda_i + \alpha(d_{out}-1)\right)\right]$$
...
\item When residual connections are present, the output range must be adjusted by the residual contribution. For a scalar residual weight $w$, if $w \geq 0$, the residual adds $[wa, wb]$ to the range where $[a,b]$ is the input range. If $w < 0$, it adds $[wb, wa]$ (note the reversal). For Cyclic residuals:
   \begin{itemize}
   \item \textbf{Broadcast}: The output range is expanded by $[\min_q w_q^{\text{bcast}} \cdot a, \max_q w_q^{\text{bcast}} \cdot b]$ when all weights are non-negative, with appropriate sign handling for negative weights.
   \item \textbf{Pooling}: Each output's contribution depends on which inputs are assigned to it and their respective weights.
   \end{itemize}
\end{enumerate}
\end{lemma}
...
For (2), without lateral mixing, the argument to $\Phi$ is $\sum_{i} \lambda_i \phi(x_i + \eta q) + \alpha q$. Since $\phi$ outputs in $[0,1]$, the weighted sum achieves its minimum when positive $\lambda_i$ multiply 0 and negative $\lambda_i$ multiply 1, yielding $\sum_{i: \lambda_i < 0} \lambda_i$. The maximum adds the contribution from positive weights and the maximum $\alpha q$ term.

For (3), with lateral mixing, each pre-activation $s_q = \sum_{i} \lambda_i \phi(x_i + \eta q) + \alpha q$ undergoes mixing: $\tilde{s}_q = s_q + \tau \sum_{j \in \mathcal{N}(q)} \omega_{q,j} s_j$. In the worst case, all mixing contributions align constructively. Since $|s_q| \leq |\sum_{i: \lambda_i > 0} \lambda_i| + \alpha(d_{out}-1)$ and $|\tau| \|\omega\|_\infty |\mathcal{N}|_{max}$ bounds the mixing contribution, the expanded domain becomes as stated.

For (4), for piecewise linear splines, the function is linear between adjacent knots, so extrema must occur at knot points. By evaluating $\Phi$ at all knots $t_j$ within $\mathcal{D}_\Phi$, we obtain the exact output range.

For (5), the Cyclic residual contributions follow from analyzing the specific assignment patterns and weight vectors. For broadcasting with non-negative weights, each output $q$ receives $w_q^{\text{bcast}} \cdot x_{q \bmod d_{\mathrm{in}}}$, contributing $[w_q^{\text{bcast}} \cdot a, w_q^{\text{bcast}} \cdot b]$ to the range. Taking the union over all $q$ gives the stated bounds.
\end{proof}

This lemma enables a forward propagation algorithm for computing all spline domains throughout the network. Crucially, the algorithm's efficiency relies on the property of piecewise-linear splines (formalized in Lemma~\ref{lem:domain_prop}, part 4), which allows for the exact computation of output ranges by simply finding the extrema of the spline's values at its knots. In practical implementations, this computation is performed periodically during training as the parameters $\lambda^{(\ell)}$, $\eta^{(\ell)}$, and lateral mixing parameters evolve, ensuring that splines operate on appropriate domains:

\begin{algorithm}
\caption{Forward domain propagation with lateral mixing}
\label{alg:domain_prop}
\begin{algorithmic}[1]
    \State \textbf{Input:} Network parameters $\{\lambda^{(\ell)}, \eta^{(\ell)}, \tau^{(\ell)}, \omega^{(\ell)}\}_{\ell=1}^L$, input domain $[0,1]^n$
    ...
    \For{each block $\ell = 1, \ldots, L$}
        ...
        \State $\mathcal{D}_\Phi^{(\ell)} \leftarrow$ apply Lemma \ref{lem:domain_prop} part (2) or (3) based on lateral mixing
...
This targeted, dual-strategy approach is theoretically sounder than naive rescaling, as it avoids the most direct sources of information loss and instability from domain changes. However, the fundamental difficulty of optimizing highly oscillatory splines within dynamically shifting domains remains a key challenge, and this approach should be seen as a mitigation strategy rather than a complete solution to training stability. The addition of lateral mixing can sometimes exacerbate this challenge by introducing additional dynamics in the domain evolution.
\end{remark}

One particularly elegant application of this domain computation is the initialization of $\Phi^{(\ell)}$ splines. By computing each $\Phi^{(\ell)}$'s theoretical domain before training begins, we can initialize both its domain and codomain parameters to match this computed range, making each $\Phi^{(\ell)}$ start as an approximate identity function on its natural operating range. This provides a principled initialization strategy: by starting each $\Phi^{(\ell)}$ as an approximate identity function on its computed domain, we ensure that the initial network performs a series of near-identity transformations, with lateral mixing initially having minimal effect. However, the effectiveness of this initialization strategy remains an empirical question, as identity initialization may not always lead to better optimization dynamics than alternatives such as variance-preserving initialization or carefully chosen random initialization.
...
The interval arithmetic used for domain computation provides worst-case bounds that may be overly conservative in practice, particularly when lateral mixing is enabled. When domains become unnecessarily wide, splines must distribute their knots sparsely, often leading to the oscillatory behavior observed in learned $\Phi^{(\ell)}$ functions. This suggests that tighter domain estimation or regularization of weight magnitudes and lateral mixing strengths during early training could improve both optimization dynamics and the interpretability of learned splines. Future work might explore adaptive knot placement strategies that concentrate knots in regions of high data density while maintaining the fixed parameter count that is central to SNs' theoretical appeal.
\end{remark}
...
We train SNs on datasets sampled from known target functions $f$. The network learns the parameters ($\eta^{(\ell)}$, $\lambda^{(\ell)}$), spline coefficients ($\phi^{(\ell)}$, $\Phi^{(\ell)}$), and when enabled, lateral mixing parameters ($\tau^{(\ell)}$, $\omega^{(\ell)}$) via gradient descent (Adam optimizer, typically $O(10^5)$ updates) using MSE loss plus regularization.
...
Learned outer splines $\Phi^{(\ell)}$ often develop pronounced oscillations, particularly when using vector weights rather than matrix weights. These oscillations can be understood as a compensation mechanism: with fewer degrees of freedom in the mixing weights, the outer splines must work harder to achieve the necessary transformations. The addition of lateral mixing partially alleviates this issue by providing an alternative pathway for cross-dimensional information flow, often resulting in smoother splines and better optimization dynamics. This phenomenon parallels observations in other constrained architectures, where limiting one component's flexibility forces other components to develop more complex behaviors. From an optimization perspective, these oscillations may indicate that the network is operating near the limits of its expressivity for the given architecture, suggesting that wider or deeper networks with lateral mixing might achieve the same function approximation with smoother splines.
\end{remark}

The domain management strategy also plays a crucial role in these oscillations. The current implementation's use of horizontal rescaling to adapt spline domains, while theoretically sound, may exacerbate oscillatory behavior by compressing existing oscillations when domains shrink. The inclusion of lateral mixing adds another layer of complexity to domain management, as the mixing terms can expand the required domain for $\Phi^{(\ell)}$. This suggests that the interplay between domain computation, parameter constraints, lateral mixing, and spline smoothness is more subtle than initially apparent and deserves further theoretical and empirical investigation.
...
While comprehensive benchmarking remains future work, the architectural differences suggest SNs might excel on functions with compositional structure aligned with their shift-based design (e.g., additive functions, functions with known Sprecher representations) while potentially struggling with functions requiring independent processing of different output dimensions. The addition of lateral mixing partially addresses this limitation, enabling SNs to capture cross-dimensional dependencies more effectively. Systematic comparison with MLPs and KANs across diverse function classes would clarify these trade-offs.
...
Current implementations face practical challenges including significantly slower training than MLPs (empirically 3-10× slower for parallel computation, though somewhat mitigated by lateral mixing). When memory-efficient sequential computation is employed to enable training of wider architectures, this can increase to 4-15× slower, representing a fundamental trade-off between memory usage and computational efficiency. The sequential computation mode proves most beneficial for architectures with individual layers exceeding 128-256 units in width, while offering minimal advantage for very deep networks with modest layer widths where the memory bottleneck lies in storing activations across many layers rather than within-block computations. Optimization difficulties manifest as oscillatory splines that appear to compensate for the reduced parameter flexibility but complicate optimization. While lateral mixing often produces smoother splines, it introduces additional hyperparameters (mixing type, initialization scales) that require tuning. The development of adaptive knot placement strategies that concentrate resolution where data lives while maintaining fixed parameter counts could address this issue, as could specialized training algorithms tailored to the shared-spline structure with lateral connections.
