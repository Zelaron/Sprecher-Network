...
\begin{itemize}
    \item \textbf{Residual connections (Learnable soft routing):} When enabled, residual connections adapt to dimensional mismatches through a learnable routing mechanism that maintains architectural coherence while providing gradient highways. The implementation varies based on dimensional relationships:
    \begin{itemize}
        \item \textbf{Identity} ($d_{\mathrm{in}} = d_{\mathrm{out}}$): Uses a single learnable scalar weight $w_{\text{res}}$:
        $$[B^{(\ell)}_{\text{res}}(\mathbf{x})]_q = [B^{(\ell)}(\mathbf{x})]_q + w_{\text{res}} \cdot x_q$$
        
        \item \textbf{Pooling} ($d_{\mathrm{in}} > d_{\mathrm{out}}$): Each input dimension has a learnable selectivity parameter $\rho_i \in \mathbb{R}$ that determines how strongly it connects to different outputs. The routing is based on circular distances:
        \begin{align}
        p_i &= i / d_{\mathrm{in}}, \quad q_j = j / d_{\mathrm{out}} \quad \text{(normalized positions)}\\
        d_{ij} &= \min(|p_i - q_j|, 1 - |p_i - q_j|) \quad \text{(circular distance)}\\
        a_{ij} &= \exp(-\sigma(\rho_i) \cdot s \cdot d_{ij}^2) \quad \text{(affinity)}\\
        r_{ij} &= \frac{\exp(a_{ij} / \tau)}{\sum_{k} \exp(a_{ik} / \tau)} \quad \text{(routing weights)}
        \end{align}
        where $\sigma(\cdot)$ is the sigmoid function, $s$ is a selectivity scale (typically 10), and $\tau$ is a temperature parameter. The residual contribution becomes:
        $$[B^{(\ell)}_{\text{res}}(\mathbf{x})]_j = [B^{(\ell)}(\mathbf{x})]_j + \sum_{i=1}^{d_{\mathrm{in}}} r_{ij} \cdot \sigma(\rho_i) \cdot s_{\text{input}} \cdot x_i$$
        where $s_{\text{input}}$ is an input scaling factor (typically 2).
        
        \item \textbf{Broadcasting} ($d_{\mathrm{in}} < d_{\mathrm{out}}$): Each output dimension has a learnable selectivity parameter that determines which inputs it draws from:
        \begin{align}
        r_{ji} &= \frac{\exp(a_{ji} / \tau)}{\sum_{k} \exp(a_{jk} / \tau)} \quad \text{(routing weights for output } j \text{)}\\
        [B^{(\ell)}_{\text{res}}(\mathbf{x})]_j &= [B^{(\ell)}(\mathbf{x})]_j + \sum_{i=1}^{d_{\mathrm{in}}} r_{ji} \cdot x_i
        \end{align}
    \end{itemize}
    This learnable routing mechanism uses $O(\max(d_{\mathrm{in}}, d_{\mathrm{out}}))$ parameters while providing flexible, data-driven connections that can learn to focus on relevant input-output pairs.
...
\begin{remark}[Learnable routing as attention-like mechanism]
The learnable soft routing can be viewed as a lightweight attention mechanism specifically designed for dimensional adaptation. Unlike full attention which would require $O(d_{\mathrm{in}} \times d_{\mathrm{out}})$ parameters, our approach uses only $O(\max(d_{\mathrm{in}}, d_{\mathrm{out}}))$ selectivity parameters that modulate pre-defined circular distance relationships. The circular distance metric ensures that: (1) nearby dimensions in the normalized position space have higher default affinity, (2) the first and last dimensions are considered neighbors (wraparound), and (3) the learned selectivity can either sharpen these relationships (high selectivity) or broaden them (low selectivity). This design is motivated by the observation that in many structured problems, there exists a natural ordering or topology in the feature space that can be exploited.
\end{remark}

\begin{remark}[Structured sparsity and architectural coherence]
The learnable soft routing design can be viewed as implementing a structured sparse projection where each output connects to a weighted combination of inputs based on learned affinities. In the pooling case, inputs with higher selectivity values will concentrate their contributions to fewer outputs, while inputs with lower selectivity distribute more broadly. The unexpected effectiveness of this constrained routing suggests that for gradient flow and information propagation, the specific connection pattern may matter less than ensuring smooth, learnable paths that can adapt to the data distribution while maintaining computational efficiency.
\end{remark}
...
\textbf{Residual design}    & Matrix projection   & Matrix projection  & None (typically)      & Learnable Soft Routing \\
...
Sprecher Networks have a fundamentally different structure. Each block $\ell$ contains mixing weights $\lambda^{(\ell)}$ with $O(d_{\ell-1})$ parameters, where $d_{\ell-1}$ is the input dimension to that block (crucially, these are vectors, not matrices), shared splines $\phi^{(\ell)}, \Phi^{(\ell)}$ with $2 \times O(G) = O(G)$ parameters per block independent of $N$, and a shift parameter $\eta^{(\ell)}$ with $O(1)$ parameter per block. When normalization is used, each block may have an associated normalization layer adding $O(N)$ parameters (specifically $2N$ for the affine transformation). When residual connections are used, they add at most $O(\max(d_{\ell-1}, d_\ell))$ parameters per block using the learnable soft routing approach. Specifically:
\begin{itemize}
    \item Identity case ($d_{\ell-1} = d_\ell$): 1 scalar parameter
    \item Pooling case ($d_{\ell-1} > d_\ell$): $d_{\ell-1}$ selectivity parameters
    \item Broadcasting case ($d_{\ell-1} < d_\ell$): $d_\ell$ selectivity parameters
\end{itemize}
This maintains the linear scaling in width while providing significantly more flexibility than fixed routing patterns. Summing over $L$ (or $L+1$) blocks, the total parameter count scales approximately as $O(LN + LG + L)$, where the linear dependence on $N$ now includes mixing weights, optional normalization parameters, and residual connections.
...
\paragraph{Learnable routing parameters.}
When residual connections are enabled, each block may include additional routing selectivity parameters $\rho^{(\ell)}$. These are initialized near zero (using a small random initialization with configurable standard deviation) to begin with approximately uniform routing. During training, these parameters learn to create focused or distributed routing patterns based on the data. The sigmoid activation ensures selectivity values remain bounded, while the exponential affinity function creates smooth gradients for optimization. Key hyperparameters include the selectivity scale $s$ (default: 10.0), temperature $\tau$ (default: 1.0), and input scale $s_{\text{input}}$ (default: 2.0), which can be tuned based on the specific application.

All learnable parameters (spline coefficients, $\eta^{(\ell)}$, $\lambda^{(\ell)}$, routing selectivity $\rho^{(\ell)}$, and potentially range parameters for $\Phi^{(\ell)}$) are trained jointly using gradient-based optimization methods like Adam \cite{kingma2014adam} or LBFGS. The loss function is typically Mean Squared Error (MSE) for regression tasks.
...
\item When residual connections are present, the output range must be adjusted by the residual contribution. For a scalar residual weight $w$, if $w \geq 0$, the residual adds $[wa, wb]$ to the range where $[a,b]$ is the input range. If $w < 0$, it adds $[wb, wa]$ (note the reversal). For learnable soft routing:
   \begin{itemize}
   \item \textbf{Pooling}: The contribution is bounded by $[-s_{\text{input}} |a|, s_{\text{input}} |b|]$ where $s_{\text{input}}$ is the input scaling factor, as the routing weights are normalized and selectivity-scaled inputs are bounded.
   \item \textbf{Broadcasting}: Each output's range is expanded by at most $[-\max(|a|, |b|), \max(|a|, |b|)]$ as the routing weights sum to 1.
   \end{itemize}
\end{enumerate}
\end{lemma}
...
For (4), the learnable soft routing contributions follow from analyzing the normalized routing weights and bounded selectivity values.
\end{proof}
