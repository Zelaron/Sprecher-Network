...
class SprecherLayerBlock(nn.Module):
    ...
    def __init__(self, d_in, d_out, layer_num=0, is_final=False, phi_knots=100, Phi_knots=100,
        ...
        # Learnable soft routing for residual connections (only create if enabled in config)
        if CONFIG['use_residual_weights']:
            if d_in == d_out:
                # When dimensions match, use a simple scalar weight
                self.residual_weight = nn.Parameter(torch.tensor(0.1))
                self.routing_selectivity = None
            elif d_in > d_out:
                # Pooling case: d_in selectivity parameters
                self.residual_weight = None
                init_std = CONFIG.get('routing_init_std', 0.0)
                self.routing_selectivity = nn.Parameter(torch.randn(d_in) * init_std)
            else:  # d_in < d_out
                # Broadcasting case: d_out selectivity parameters
                self.residual_weight = None
                init_std = CONFIG.get('routing_init_std', 0.0)
                self.routing_selectivity = nn.Parameter(torch.randn(d_out) * init_std)
        else:
            self.residual_weight = None
            self.routing_selectivity = None
    ...
    def compute_output_range_theoretical(self):
        ...
        if CONFIG['use_residual_weights'] and self.input_range is not None:
            ...
            elif self.routing_selectivity is not None:
                # For learnable routing, be conservative and assume maximum possible contribution
                # The routing weights are normalized (sum to 1), so max contribution is input_bound
                input_bound = max(abs(self.input_range.min), abs(self.input_range.max))
                
                if self.d_in > self.d_out:
                    # Pooling: inputs are scaled by sigmoid(selectivity) * routing_input_scale
                    max_scale = CONFIG.get('routing_input_scale', 2.0)
                    residual_bound = max_scale * input_bound
                else:
                    # Broadcasting: direct routing without additional scaling
    ...
    def forward(self, x, x_original=None):
        ...
        if CONFIG['use_residual_weights'] and x_original is not None:
            if self.residual_weight is not None:
                ...
                # Learnable soft routing
                if self.d_in > self.d_out:
                    # Pooling case: aggregate inputs to outputs
                    input_positions = torch.arange(self.d_in, dtype=torch.float32, device=x.device) / self.d_in
                    output_positions = torch.arange(self.d_out, dtype=torch.float32, device=x.device) / self.d_out
                    
                    # Compute circular distances
                    distances = torch.abs(input_positions.unsqueeze(1) - output_positions.unsqueeze(0))
                    circular_distances = torch.min(distances, 1.0 - distances)  # [d_in, d_out]
                    
                    # Apply learned selectivity
                    selectivity = torch.sigmoid(self.routing_selectivity).unsqueeze(1) * CONFIG['routing_selectivity_scale'] + 0.1
                    affinities = torch.exp(-selectivity * circular_distances ** 2)
                    
                    # Normalize to create routing weights (each input distributes to outputs)
                    routing_weights = F.softmax(affinities * CONFIG['routing_temperature'], dim=1)  # [d_in, d_out]
                    
                    # Apply routing with input scaling
                    input_scales = torch.sigmoid(self.routing_selectivity) * CONFIG['routing_input_scale']
                    scaled_input = x_original * input_scales.unsqueeze(0)  # [batch, d_in]
                    
                    # Route inputs to outputs
                    if x_original.dim() == 2:
                        routed = torch.matmul(scaled_input.unsqueeze(1), routing_weights).squeeze(1)
                    else:
                        # Handle batch dimensions properly
                        routed = torch.matmul(scaled_input.transpose(-2, -1), routing_weights).transpose(-2, -1)
                    
                    activated = activated + routed
                    ...
                    # Compute circular distances
                    distances = torch.abs(output_positions.unsqueeze(1) - input_positions.unsqueeze(0))
                    circular_distances = torch.min(distances, 1.0 - distances)  # [d_out, d_in]
                    
                    # Apply learned selectivity
                    selectivity = torch.sigmoid(self.routing_selectivity).unsqueeze(1) * CONFIG['routing_selectivity_scale'] + 0.1
                    affinities = torch.exp(-selectivity * circular_distances ** 2)
                    
                    # Normalize to create routing weights (each output selects from inputs)
                    routing_weights = F.softmax(affinities * CONFIG['routing_temperature'], dim=1)  # [d_out, d_in]
                    
                    # Route inputs to outputs
                    if x_original.dim() == 2:
                        routed = torch.matmul(routing_weights, x_original.unsqueeze(-1)).squeeze(-1)
                    else:
                        # Handle batch dimensions properly
                        routed = torch.matmul(routing_weights, x_original.transpose(-2, -1)).transpose(-2, -1)
