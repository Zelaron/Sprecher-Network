...
def migrate_old_residual_params(model, checkpoint_state_dict):
    """
    Migrate old round-robin residual parameters to new learnable routing format.
    
    Args:
        model: Fresh model instance with new routing parameters
        checkpoint_state_dict: State dict from old checkpoint
        
    Returns:
        Updated state dict with migrated parameters
    """
    print("\nMigrating old residual parameters to learnable routing...")
    
    # Create a copy of the state dict to modify
    new_state_dict = checkpoint_state_dict.copy()
    
    for i, layer in enumerate(model.layers):
        layer_prefix = f'layers.{i}.'
        
        # Check for old pooling weights (d_in > d_out)
        old_pooling_key = layer_prefix + 'residual_pooling_weights'
        if old_pooling_key in checkpoint_state_dict and layer.d_in > layer.d_out:
            print(f"  Layer {i} ({layer.d_in} → {layer.d_out}): Migrating pooling weights")
            
            # Remove old parameters
            del new_state_dict[old_pooling_key]
            if layer_prefix + 'pooling_assignment' in new_state_dict:
                del new_state_dict[layer_prefix + 'pooling_assignment']
            if layer_prefix + 'pooling_counts' in new_state_dict:
                del new_state_dict[layer_prefix + 'pooling_counts']
            
            # Initialize new routing selectivity to produce uniform routing
            # (zeros in raw space = 0.5 after sigmoid = moderate selectivity)
            new_state_dict[layer_prefix + 'routing_selectivity'] = torch.zeros(layer.d_in)
            
        # Check for old broadcast weights (d_in < d_out)
        old_broadcast_key = layer_prefix + 'residual_broadcast_weights'
        if old_broadcast_key in checkpoint_state_dict and layer.d_in < layer.d_out:
            print(f"  Layer {i} ({layer.d_in} → {layer.d_out}): Migrating broadcast weights")
            
            # Remove old parameters
            del new_state_dict[old_broadcast_key]
            if layer_prefix + 'broadcast_sources' in new_state_dict:
                del new_state_dict[layer_prefix + 'broadcast_sources']
            
            # Initialize new routing selectivity
            new_state_dict[layer_prefix + 'routing_selectivity'] = torch.zeros(layer.d_out)
...
def train_network(dataset, architecture, total_epochs=4000, print_every=400, 
    ...
    for name, param in model.named_parameters():
        ...
        elif 'routing_selectivity' in name:
            # Learnable routing selectivity
            if CONFIG['use_residual_weights']:
    ...
    if CONFIG['use_residual_weights'] and residual_params > 0:
        ...
        if residual_routing_params > 0:
            print(f"    * Routing selectivity: {residual_routing_params}")
    ...
    if best_checkpoint is not None and not no_load_best:
        ...
        if 'model_params' in best_checkpoint:
            ...
            # Check if we need to migrate old residual parameters
            needs_migration = False
            for key in best_checkpoint['model_state_dict']:
                if 'residual_pooling_weights' in key or 'residual_broadcast_weights' in key:
                    needs_migration = True
                    break
            
            if needs_migration:
                # Migrate old parameters to new format
                migrated_state_dict = migrate_old_residual_params(fresh_model, best_checkpoint['model_state_dict'])
                best_checkpoint['model_state_dict'] = migrated_state_dict
    ...
    for idx, layer in enumerate(model.layers, start=1):
        ...
        if CONFIG['use_residual_weights']:
            if hasattr(layer, 'residual_weight') and layer.residual_weight is not None:
                ...
                print(f"Block {idx}: routing_selectivity ({routing_type}) shape = {tuple(layer.routing_selectivity.shape)}")
                print(f"Block {idx}: routing_selectivity (raw) =")
                print(layer.routing_selectivity.detach().cpu().numpy())
                print(f"Block {idx}: routing_selectivity (sigmoid) =")
                print(torch.sigmoid(layer.routing_selectivity).detach().cpu().numpy())
        if CONFIG['train_phi_codomain']:
