    ...
    if 'residual' in categories and CONFIG['use_residual_weights']:
        ...
        for i, layer in enumerate(model.layers):
            if hasattr(layer, 'residual_weight') and layer.residual_weight is not None:
                ...
                # Factorized residual projection (different dimensions)
                lines.append(f"Layer {i} ({layer.d_in} → {layer.d_out}): factorized residual projection")
                lines.append(f"  Architecture: {layer.d_in} → {layer.residual_intermediate_dim} → {layer.d_out}")
                lines.append("  Input combination weights:")
                lines.append(format_tensor(layer.residual_input_weights, "residual_input_weights", "    "))
                lines.append("  Intermediate bias:")
                lines.append(format_tensor(layer.residual_intermediate_bias, "residual_intermediate_bias", "    "))
                lines.append("  Output distribution weights:")
                lines.append(format_tensor(layer.residual_output_weights, "residual_output_weights", "    "))
                
                # Calculate total parameters for this residual connection
                total_residual_params = (layer.residual_input_weights.numel() + 
                                       layer.residual_output_weights.numel() + 
                                       layer.residual_intermediate_bias.numel())
                lines.append(f"  Total residual parameters: {total_residual_params}")
                has_residual = True
        if not has_residual:
