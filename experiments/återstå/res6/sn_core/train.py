...
def train_network(dataset, architecture, total_epochs=4000, print_every=400, 
    ...
    for name, param in model.named_parameters():
        ...
        elif 'residual_input_weights' in name or 'residual_output_weights' in name or 'residual_intermediate_bias' in name:
            if CONFIG['use_residual_weights']:
                residual_factorized_params += param.numel()
                residual_params += param.numel()
    ...
    if CONFIG['use_residual_weights'] and residual_params > 0:
        ...
        if residual_scalar_params > 0:
            print(f"    - Scalar weights (same dimensions): {residual_scalar_params}")
        if residual_factorized_params > 0:
            print(f"    - Factorized projection (different dimensions): {residual_factorized_params}")
            # Print details about factorized residuals
            for layer in model.layers:
                if hasattr(layer, 'residual_intermediate_dim') and layer.residual_intermediate_dim is not None:
                    k = layer.residual_intermediate_dim
                    input_params = layer.d_in * k
                    output_params_layer = k * layer.d_out
                    bias_params = k
                    print(f"      Layer {layer.layer_num}: {layer.d_in}→{k}→{layer.d_out} "
                          f"({input_params} + {output_params_layer} + {bias_params} = "
                          f"{input_params + output_params_layer + bias_params} params)")
    ...
    for idx, layer in enumerate(model.layers, start=1):
        ...
        if CONFIG['use_residual_weights']:
            ...
            elif hasattr(layer, 'residual_input_weights') and layer.residual_input_weights is not None:
                print(f"Block {idx}: residual projection (factorized):")
                print(f"  Input weights shape: {tuple(layer.residual_input_weights.shape)}")
                print(f"  Output weights shape: {tuple(layer.residual_output_weights.shape)}")
                print(f"  Intermediate dimension: {layer.residual_intermediate_dim}")
                print(f"  Intermediate bias shape: {tuple(layer.residual_intermediate_bias.shape)}")
        if CONFIG['train_phi_codomain']:
