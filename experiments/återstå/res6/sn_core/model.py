...
class SprecherLayerBlock(nn.Module):
    ...
    def __init__(self, d_in, d_out, layer_num=0, is_final=False, phi_knots=100, Phi_knots=100,
        ...
        if CONFIG['use_residual_weights']:
            if d_in == d_out:
                ...
                self.residual_input_weights = None
                self.residual_output_weights = None
                self.residual_intermediate_bias = None
                self.residual_intermediate_dim = None
            else:
                # When dimensions don't match, use factorized vector projection
                self.residual_weight = None
                
                # Determine intermediate dimension (heuristic: geometric mean)
                k = int(np.sqrt(d_in * d_out))
                k = max(min(k, d_in, d_out), 1)  # Ensure valid range
                self.residual_intermediate_dim = k
                
                # Input combination weights (d_in, k)
                self.residual_input_weights = nn.Parameter(
                    torch.randn(d_in, k) * 0.1 / np.sqrt(d_in)
                )
                
                # Output distribution weights (k, d_out)  
                self.residual_output_weights = nn.Parameter(
                    torch.randn(k, d_out) * 0.1 / np.sqrt(k)
                )
                
                # Learnable intermediate bias for stability
                self.residual_intermediate_bias = nn.Parameter(
                    torch.zeros(k)
                )
        else:
            self.residual_weight = None
    ...
    def compute_output_range_theoretical(self):
        ...
        if CONFIG['use_residual_weights'] and self.input_range is not None:
            ...
            elif self.residual_input_weights is not None:
                # For factorized projection, we need to be more conservative
                # The residual contribution depends on the interaction of both weight matrices
                # Conservative estimate: assume worst-case amplification
                input_weight_norm = torch.norm(self.residual_input_weights, p=2).item()
                output_weight_norm = torch.norm(self.residual_output_weights, p=2).item()
                
                # Account for intermediate activation (tanh) which bounds values to [-1, 1]
                # Worst case: all inputs aligned to maximize output
                max_intermediate = min(input_weight_norm * max(abs(self.input_range.min), abs(self.input_range.max)), 1.0)
                residual_bound = output_weight_norm * max_intermediate
    ...
    def forward(self, x, x_original=None):
        ...
        if CONFIG['use_residual_weights'] and x_original is not None:
            if self.residual_weight is not None:
                ...
                # Different dimensions: factorized projection
                # Step 1: Weighted combination to intermediate dimension
                # x_original: (batch, d_in)
                # residual_input_weights: (d_in, k)
                intermediate = torch.matmul(
                    x_original.unsqueeze(1),  # (batch, 1, d_in)
                    self.residual_input_weights.unsqueeze(0)  # (1, d_in, k)
                ).squeeze(1)  # (batch, k)
                
                # Add bias for stability
                intermediate = intermediate + self.residual_intermediate_bias
                
                # Apply activation (tanh) to intermediate
                intermediate = torch.tanh(intermediate)
                
                # Step 2: Distribute to output dimensions
                # intermediate: (batch, k)
                # residual_output_weights: (k, d_out)
                residual_contribution = torch.matmul(
                    intermediate.unsqueeze(1),  # (batch, 1, k)
                    self.residual_output_weights.unsqueeze(0)  # (1, k, d_out)
