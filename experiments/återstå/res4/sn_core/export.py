...
def export_parameters(model, param_types, save_path, dataset_info=None, checkpoint_info=None):
    ...
    for i, layer in enumerate(model.layers):
        ...
        if CONFIG['use_residual_weights'] and layer.d_in != layer.d_out:
            if layer.residual_spline is not None:
                lines.append(f"  - Has NODE-CENTRIC residual connection")
    lines.append("")
    ...
    if 'spline' in categories:
        ...
        for i, layer in enumerate(model.layers):
            ...
            # Residual spline (if exists)
            if layer.residual_spline is not None:
                lines.append("  Residual spline (node-centric transform):")
                lines.append(f"    Domain: [{layer.residual_spline.in_min:.6f}, {layer.residual_spline.in_max:.6f}]")
                lines.append(f"    Number of knots: {layer.residual_spline.num_knots}")
                lines.append(format_tensor(layer.residual_spline.coeffs, "Coefficients", "    "))
        lines.append("")
    ...
    if 'residual' in categories and CONFIG['use_residual_weights']:
        ...
        for i, layer in enumerate(model.layers):
            if hasattr(layer, 'residual_weight') and layer.residual_weight is not None:
                # Same dimensions - scalar weight
                lines.append(f"Layer {i} ({layer.d_in} → {layer.d_out}): residual_weight = {layer.residual_weight.item():.6f}")
                has_residual = True
            elif layer.residual_spline is not None:
                # Different dimensions - node-centric projection
                lines.append(f"Layer {i} ({layer.d_in} → {layer.d_out}): NODE-CENTRIC residual connection")
                
                # Pool weights (if d_in > d_out)
                if layer.residual_pool_weights is not None:
                    lines.append("  Pooling weights (raw):")
                    lines.append(format_tensor(layer.residual_pool_weights, indent="    "))
                    # Also show softmax normalized version
                    softmax_weights = F.softmax(layer.residual_pool_weights, dim=0)
                    lines.append("  Pooling weights (softmax normalized):")
                    lines.append(format_tensor(softmax_weights, indent="    "))
                
                # Expand weights (if d_in < d_out)
                if layer.residual_expand_weights is not None:
                    lines.append("  Expansion weights (raw):")
                    lines.append(format_tensor(layer.residual_expand_weights, indent="    "))
                    # Also show softmax normalized version
                    softmax_weights = F.softmax(layer.residual_expand_weights, dim=0)
                    lines.append("  Expansion weights (softmax normalized):")
                    lines.append(format_tensor(softmax_weights, indent="    "))
                
                # Output scale vector
                lines.append("  Output scale vector:")
                lines.append(format_tensor(layer.residual_scale, indent="    "))
                
                # Residual spline info
                lines.append(f"  Residual spline: {layer.residual_spline.num_knots} knots")
                lines.append(f"    Domain: [{layer.residual_spline.in_min:.6f}, {layer.residual_spline.in_max:.6f}]")
