...
class SprecherLayerBlock(nn.Module):
    ...
    def __init__(self, d_in, d_out, layer_num=0, is_final=False, phi_knots=100, Phi_knots=100,
        ...
        if CONFIG['use_residual_weights']:
            if d_in == d_out:
                ...
                self.residual_spline = None
                self.residual_pool_weights = None
                self.residual_expand_weights = None
                self.residual_scale = None
            else:
                # When dimensions don't match, use node-centric factorized projection
                self.residual_weight = None
                
                # 1. Shared transform spline (similar to phi/Phi but simpler)
                self.residual_spline = SimpleSpline(
                    num_knots=30,  # Fewer knots for efficiency
                    in_range=(-2, 2),  # Initial guess for residual input range
                    out_range=(-1, 1),  # Fixed output range
                    monotonic=False
                )
                
                # 2. Dimension mapping vectors
                if d_in > d_out:
                    # Pooling case: learn importance weights for input dimensions
                    self.residual_pool_weights = nn.Parameter(torch.ones(d_in))
                    self.residual_expand_weights = None
                else:
                    # Expansion case: learn distribution weights for output dimensions
                    self.residual_pool_weights = None
                    self.residual_expand_weights = nn.Parameter(torch.ones(d_out))
                
                # 3. Output scaling vector
                self.residual_scale = nn.Parameter(torch.ones(d_out) * 0.1)
        else:
            self.residual_weight = None
            self.residual_spline = None
            self.residual_pool_weights = None
    ...
    def update_phi_domain_theoretical(self, input_range, allow_resampling=True, force_resample=False):
        ...
        # Also update residual spline domain if it exists
        if self.residual_spline is not None:
            # The residual spline sees the raw input values
            self.residual_spline.update_domain((input_range.min, input_range.max), 
    ...
    def compute_output_range_theoretical(self):
        ...
        if CONFIG['use_residual_weights'] and self.input_range is not None:
            ...
            elif self.residual_spline is not None:
                # For node-centric projection, we need to consider the spline output range
                # and the effect of pooling/expansion and scaling
                spline_range = self.residual_spline.get_actual_output_range()
                
                # Get the maximum possible scaling factor
                max_scale = torch.max(torch.abs(self.residual_scale)).item()
                
                # Conservative estimate of contribution
                residual_bound = max_scale * max(abs(spline_range.min), abs(spline_range.max))
    ...
    def _weighted_pool(self, x, target_dim):
        """
        Pool input dimensions to target dimension using learned weights.
        
        Args:
            x: Input tensor of shape (batch_size, d_in)
            target_dim: Target number of dimensions (d_out)
        
        Returns:
            Pooled tensor of shape (batch_size, target_dim)
        """
        batch_size = x.shape[0]
        d_in = x.shape[1]
        
        # Get normalized weights (softmax for importance)
        weights = F.softmax(self.residual_pool_weights, dim=0)
        
        # Strategy: Weighted grouping
        # Divide input dimensions into groups, each group contributes to one output
        base_group_size = d_in // target_dim
        remainder = d_in % target_dim
        
        output = torch.zeros(batch_size, target_dim, device=x.device)
        
        start_idx = 0
        for i in range(target_dim):
            # Some groups get an extra dimension if there's a remainder
            group_size = base_group_size + (1 if i < remainder else 0)
            end_idx = start_idx + group_size
            
            # Extract group and corresponding weights
            group_data = x[:, start_idx:end_idx]
            group_weights = weights[start_idx:end_idx]
            
            # Normalize weights within group
            group_weights = group_weights / (group_weights.sum() + 1e-8)
            
            # Weighted sum for this output dimension
            output[:, i] = (group_data * group_weights.unsqueeze(0)).sum(dim=1)
            
            start_idx = end_idx
        
        return output
    
    def _pattern_expand(self, x, target_dim):
        """
        Expand input dimensions to target dimension using learned pattern.
        ...
        Returns:
            Expanded tensor of shape (batch_size, target_dim)
        """
        batch_size = x.shape[0]
        d_in = x.shape[1]
        
        # Get normalized expansion weights
        pattern = F.softmax(self.residual_expand_weights, dim=0)
        
        # Strategy: Each input dimension contributes to multiple outputs
        # The pattern determines the contribution strength
        
        # Create contribution matrix of shape (d_in, d_out)
        # Each row represents how much an input contributes to each output
        contribution_matrix = torch.zeros(d_in, target_dim, device=x.device)
        
        # Simple circular assignment with learned weighting
        for i in range(d_in):
            # Each input primarily affects a subset of outputs
            base_idx = (i * target_dim) // d_in
            
            # Apply Gaussian-like spread around base index
            for offset in range(-2, 3):  # Spread of 5
                out_idx = (base_idx + offset) % target_dim
                # Distance-based weighting modulated by learned pattern
                weight = pattern[out_idx] * torch.exp(torch.tensor(-0.5 * offset**2, device=x.device))
                contribution_matrix[i, out_idx] = weight
        
        # Normalize each row so total contribution sums to 1
        contribution_matrix = contribution_matrix / (contribution_matrix.sum(dim=1, keepdim=True) + 1e-8)
        
        # Apply contribution matrix
        # x: (batch_size, d_in)
        # contribution_matrix.T: (d_out, d_in)
        output = torch.matmul(x, contribution_matrix)  # (batch_size, d_out)
    ...
    def forward(self, x, x_original=None):
        ...
        if CONFIG['use_residual_weights'] and x_original is not None:
            if self.residual_weight is not None:
                ...
                # Dimensions don't match, use node-centric factorized projection
                # Step 1: Apply element-wise transform
                x_transformed = self.residual_spline(x_original)
                
                # Step 2: Handle dimension change
                if self.d_in > self.d_out:
                    # Weighted pooling
                    x_proj = self._weighted_pool(x_transformed, self.d_out)
                else:
                    # Pattern-based expansion
                    x_proj = self._pattern_expand(x_transformed, self.d_out)
                
                # Step 3: Apply output scaling
                activated = activated + x_proj * self.residual_scale
    ...
    def get_domain_violation_stats(self):
        ...
        if self.residual_spline is not None:
            stats['residual'] = self.residual_spline.get_domain_violation_stats()
        return stats
    ...
    def reset_domain_violation_stats(self):
        ...
        self.Phi.reset_domain_violation_stats()
        if self.residual_spline is not None:
            self.residual_spline.reset_domain_violation_stats()
...
class SprecherMultiLayerNetwork(nn.Module):
    ...
    def update_all_domains(self, allow_resampling=True, force_resample=False):
        ...
        for layer in self.layers:
            ...
            if CONFIG.get('debug_domains', False):
                ...
                print(f"  Phi domain: [{layer.Phi.in_min:.3f}, {layer.Phi.in_max:.3f}]")
                if layer.residual_spline is not None:
                    print(f"  residual spline domain: [{layer.residual_spline.in_min:.3f}, {layer.residual_spline.in_max:.3f}]")
    ...
    def print_domain_violation_report(self):
        ...
        for layer_name, layer_stats in stats.items():
            ...
            print(f"  Phi violations: {layer_stats['Phi']:.2%}")
            if 'residual' in layer_stats:
                print(f"  residual violations: {layer_stats['residual']:.2%}")
    ...
    def get_all_domain_states(self):
        ...
        for i, layer in enumerate(self.layers):
            ...
            domain_states[f'layer_{i}_Phi'] = layer.Phi.get_domain_state()
            # Also save residual spline state if exists
            if layer.residual_spline is not None:
                domain_states[f'layer_{i}_residual'] = layer.residual_spline.get_domain_state()
    ...
    def get_domain_ranges(self):
        ...
        for i, layer in enumerate(self.layers):
            ...
            domain_ranges[f'layer_{i}_Phi'] = (layer.Phi.in_min, layer.Phi.in_max)
            if layer.residual_spline is not None:
                domain_ranges[f'layer_{i}_residual'] = (layer.residual_spline.in_min, layer.residual_spline.in_max)
    ...
    def set_all_domain_states(self, domain_states):
        ...
        for i, layer in enumerate(self.layers):
            ...
            Phi_key = f'layer_{i}_Phi'
            residual_key = f'layer_{i}_residual'
            ...
            if Phi_key in domain_states:
                layer.Phi.set_domain_state(domain_states[Phi_key])
            # Restore residual spline state if exists
            if residual_key in domain_states and layer.residual_spline is not None:
                layer.residual_spline.set_domain_state(domain_states[residual_key])
