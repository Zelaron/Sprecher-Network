...
def train_network(dataset, architecture, total_epochs=4000, print_every=400, 
    ...
    for name, param in model.named_parameters():
        ...
        if 'lambdas' in name:
            ...
            # Spline parameters (including residual splines)
            if 'residual_spline' not in name:
        ...
        elif 'residual_spline' in name:
            # Node-centric residual spline parameters
            if CONFIG['use_residual_weights']:
                residual_params += param.numel()
        elif 'residual_pool_weights' in name or 'residual_expand_weights' in name:
            # Node-centric residual dimension mapping weights
            if CONFIG['use_residual_weights']:
                residual_params += param.numel()
        elif 'residual_scale' in name:
            # Node-centric residual output scaling
            if CONFIG['use_residual_weights']:
                residual_params += param.numel()
    ...
    if CONFIG['use_residual_weights'] and residual_params > 0:
        print(f"  - Residual connection parameters: {residual_params} (NODE-CENTRIC!)")
            ...
            if CONFIG.get('debug_domains', False):
                ...
                for idx, layer in enumerate(model.layers):
                    ...
                    if layer.residual_spline is not None:
                        print(f"            residual domain=[{layer.residual_spline.in_min:.3f}, {layer.residual_spline.in_max:.3f}]")
                print()
    ...
    if best_checkpoint is not None and not no_load_best:
        ...
        if best_checkpoint.get('has_batchnorm', False) and 'bn_statistics' in best_checkpoint:
            ...
            for i, layer in enumerate(model.layers):
                ...
                if layer.residual_spline is not None:
                    print(f"         residual=[{layer.residual_spline.in_min:.6f}, {layer.residual_spline.in_max:.6f}]")
        ...
        if 'x_train' in best_checkpoint:
            ...
            if abs(current_loss - best_checkpoint['loss']) > tolerance:
                ...
                for i, layer in enumerate(model.layers):
                    ...
                    if layer.residual_spline is not None:
                        res_coeffs = layer.residual_spline.get_coeffs()
                        print(f"         residual coeffs=[{res_coeffs.min():.6f}, {res_coeffs.max():.6f}]")
            else:
    ...
    for idx, layer in enumerate(model.layers, start=1):
        ...
        if CONFIG['use_residual_weights']:
            ...
            elif hasattr(layer, 'residual_spline') and layer.residual_spline is not None:
                print(f"Block {idx}: residual connection (NODE-CENTRIC):")
                if layer.residual_pool_weights is not None:
                    print(f"  - Pool weights shape: {tuple(layer.residual_pool_weights.shape)}")
                    print(f"  - Pool weights (softmax): {F.softmax(layer.residual_pool_weights, dim=0).detach().cpu().numpy()[:5]}...")
                if layer.residual_expand_weights is not None:
                    print(f"  - Expand weights shape: {tuple(layer.residual_expand_weights.shape)}")
                    print(f"  - Expand weights (softmax): {F.softmax(layer.residual_expand_weights, dim=0).detach().cpu().numpy()[:5]}...")
                print(f"  - Scale vector: {layer.residual_scale.detach().cpu().numpy()[:5]}...")
                print(f"  - Spline knots: {layer.residual_spline.num_knots}")
        if CONFIG['train_phi_codomain']:
