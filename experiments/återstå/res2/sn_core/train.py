...
from .model import SprecherMultiLayerNetwork, NodeCentricProjection
...
def train_network(dataset, architecture, total_epochs=4000, print_every=400, 
    ...
    for name, param in model.named_parameters():
        ...
        if 'lambdas' in name:
            ...
            # Node-centric projection parameters (input_scale, output_scale, group_weights/mixing_weights)
            if CONFIG['use_residual_weights']:
                residual_params += param.numel()
    ...
    for idx, layer in enumerate(model.layers, start=1):
        ...
        if CONFIG['use_residual_weights']:
            if hasattr(layer, 'residual_weight') and layer.residual_weight is not None:
                ...
                print(f"Block {idx}: Node-centric residual projection ({proj.d_in} → {proj.d_out}):")
                print(f"  - input_scale: {proj.input_scale.detach().cpu().numpy()}")
                print(f"  - output_scale: {proj.output_scale.detach().cpu().numpy()}")
                if hasattr(proj, 'group_weights'):
                    print(f"  - group_weights: {proj.group_weights.detach().cpu().numpy()}")
                if hasattr(proj, 'mixing_weights'):
                    print(f"  - mixing_weights: {proj.mixing_weights.detach().cpu().numpy()}")
        if CONFIG['train_phi_codomain']:
