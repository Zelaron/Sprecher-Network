...
def export_parameters(model, param_types, save_path, dataset_info=None, checkpoint_info=None):
    ...
    # Export Residual parameters (updated for node-centric projection)
    if 'residual' in categories and CONFIG['use_residual_weights']:
        ...
        for i, layer in enumerate(model.layers):
            if hasattr(layer, 'residual_weight') and layer.residual_weight is not None:
                ...
                lines.append(f"Layer {i} ({layer.d_in} → {layer.d_out}): Node-centric residual projection")
                proj = layer.residual_projection
                lines.append(f"  Projection type: {proj.d_in} → {proj.d_out}")
                lines.append(format_tensor(proj.input_scale, "Input scale", "  "))
                lines.append(format_tensor(proj.output_scale, "Output scale", "  "))
                
                if hasattr(proj, 'group_weights'):
                    lines.append("  Dimension reduction via grouping:")
                    lines.append(format_tensor(proj.group_indices, "Group indices", "    "))
                    lines.append(format_tensor(proj.group_weights, "Group weights", "    "))
                
                if hasattr(proj, 'mixing_weights'):
                    lines.append("  Dimension expansion via replication:")
                    lines.append(format_tensor(proj.source_indices, "Source indices", "    "))
                    lines.append(format_tensor(proj.mixing_weights, "Mixing weights", "    "))
