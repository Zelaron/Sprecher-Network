...
class NodeCentricProjection(nn.Module):
    """
    Node-centric projection for residual connections when dimensions don't match.
    Uses only learnable vectors, no matrices, aligning with Sprecher Network philosophy.
    
    Three-stage design:
    1. Input scaling (learnable vector)
    2. Dimension change (structured pooling/expansion with learnable weights)
    3. Output scaling (learnable vector)
    """
    def __init__(self, d_in, d_out):
        ...
        # Stage 1: Learnable input scaling
        self.input_scale = nn.Parameter(torch.ones(d_in) * 0.1)
        
        # Stage 2: Dimension change with learnable importance/mixing
        if d_in > d_out:
            # Dimension reduction: create grouping pattern
            self.register_buffer('group_indices', self._create_grouping(d_in, d_out))
            # Learnable importance weights for pooling
            self.group_weights = nn.Parameter(torch.ones(d_in) / np.sqrt(d_in))
        elif d_in < d_out:
            # Dimension expansion: create replication pattern
            self.register_buffer('source_indices', self._create_replication(d_in, d_out))
            # Learnable mixing weights for expansion
            self.mixing_weights = nn.Parameter(torch.ones(d_out) / np.sqrt(d_out))
        
        # Stage 3: Learnable output scaling
        self.output_scale = nn.Parameter(torch.ones(d_out) * 1.0)
    
    def _create_grouping(self, d_in, d_out):
        """Create grouping pattern for dimension reduction."""
        # Distribute input dimensions as evenly as possible across outputs
        groups = [[] for _ in range(d_out)]
        for i in range(d_in):
            groups[i % d_out].append(i)
        
        # Convert to tensor format: for each input, which output group it belongs to
        group_assignment = torch.zeros(d_in, dtype=torch.long)
        for group_idx, indices in enumerate(groups):
            for idx in indices:
                group_assignment[idx] = group_idx
        
        return group_assignment
    
    def _create_replication(self, d_in, d_out):
        """Create replication pattern for dimension expansion."""
        # Each output dimension gets its source from input dimensions cyclically
        source_indices = torch.arange(d_out) % d_in
        return source_indices
    
    def _weighted_pool(self, x, group_indices, weights):
        """Perform weighted pooling based on grouping."""
        batch_size = x.shape[0]
        d_out = group_indices.max().item() + 1
        
        # Apply importance weights
        x_weighted = x * weights
        
        # Initialize output
        pooled = torch.zeros(batch_size, d_out, device=x.device, dtype=x.dtype)
        
        # Sum weighted inputs for each group
        for out_idx in range(d_out):
            mask = group_indices == out_idx
            pooled[:, out_idx] = x_weighted[:, mask].sum(dim=1)
        
        return pooled
    
    def _controlled_expand(self, x, source_indices):
        """Perform controlled expansion based on source indices."""
        # Simply gather from source indices
        return x[:, source_indices]
    
    def forward(self, x):
        # Stage 1: Scale inputs (node-centric operation)
        x_scaled = x * self.input_scale
        
        # Stage 2: Change dimensions
        if self.d_in > self.d_out:
            # Weighted pooling
            x_proj = self._weighted_pool(x_scaled, self.group_indices, self.group_weights)
        elif self.d_in < self.d_out:
            # Controlled expansion with mixing
            x_expanded = self._controlled_expand(x_scaled, self.source_indices)
            x_proj = x_expanded * self.mixing_weights
        else:
            # Same dimension (shouldn't happen, but handle gracefully)
            x_proj = x_scaled
        
        # Stage 3: Scale outputs (node-centric operation)
        return x_proj * self.output_scale
...
class SprecherLayerBlock(nn.Module):
    ...
    def __init__(self, d_in, d_out, layer_num=0, is_final=False, phi_knots=100, Phi_knots=100,
        ...
        # Residual connection - now using node-centric approach
        if CONFIG['use_residual_weights']:
            if d_in == d_out:
                # When dimensions match, use a simple scalar weight (unchanged)
                self.residual_weight = nn.Parameter(torch.tensor(0.1))
                self.residual_projection = None
            else:
                # When dimensions don't match, use node-centric projection
                self.residual_weight = None
                self.residual_projection = NodeCentricProjection(d_in, d_out)
        else:
            self.residual_weight = None
            self.residual_projection = None
    ...
    def compute_output_range_theoretical(self):
        ...
        if CONFIG['use_residual_weights'] and self.input_range is not None:
            if self.residual_weight is not None:
                ...
                # For node-centric projection, we need to consider all the scaling factors
                # Conservative estimate: sum of absolute values of all scaling parameters
                with torch.no_grad():
                    # Maximum possible amplification through the projection
                    input_scale_bound = torch.abs(self.residual_projection.input_scale).max().item()
                    output_scale_bound = torch.abs(self.residual_projection.output_scale).max().item()
                    
                    if self.residual_projection.d_in > self.residual_projection.d_out:
                        # Pooling case
                        weight_bound = torch.abs(self.residual_projection.group_weights).sum().item()
                    else:
                        # Expansion case
                        weight_bound = torch.abs(self.residual_projection.mixing_weights).max().item()
                    
                    # Conservative bound on residual contribution
                    max_amplification = input_scale_bound * weight_bound * output_scale_bound
                    residual_bound = max_amplification * max(abs(self.input_range.min), abs(self.input_range.max))
    ...
    def forward(self, x, x_original=None):
        ...
        if CONFIG['use_residual_weights'] and x_original is not None:
            if self.residual_weight is not None:
                ...
                # Dimensions don't match, use node-centric projection
