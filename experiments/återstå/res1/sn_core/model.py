...
class SprecherLayerBlock(nn.Module):
    ...
    def __init__(self, d_in, d_out, layer_num=0, is_final=False, phi_knots=100, Phi_knots=100,
        ...
        # Residual connection - now using factorized projection for d_in != d_out
        if CONFIG['use_residual_weights']:
            if d_in == d_out:
                ...
                # When dimensions don't match, use factorized projection
                self.residual_weight = None
                
                # Determine rank - use min of configured rank and theoretical max
                rank = min(CONFIG.get('residual_rank', 3), min(d_in, d_out))
                self.residual_rank = rank
                
                # Initialize factorization vectors with appropriate scaling
                scale = 0.1 / np.sqrt(rank)
                self.residual_U = nn.Parameter(torch.randn(rank, d_out) * scale)
                self.residual_V = nn.Parameter(torch.randn(rank, d_in) * scale)
                self.residual_scales = nn.Parameter(torch.ones(rank) * 0.1)
        else:
            self.residual_weight = None
            self.residual_U = None
            self.residual_V = None
    ...
    def compute_output_range_theoretical(self):
        ...
        if CONFIG['use_residual_weights'] and self.input_range is not None:
            ...
            elif self.residual_U is not None:
                # For factorized projection, we need to be more conservative
                # The contribution depends on the norms of U, V vectors and scales
                max_contribution = 0
                for i in range(self.residual_rank):
                    u_norm = torch.norm(self.residual_U[i]).item()
                    v_norm = torch.norm(self.residual_V[i]).item()
                    scale = abs(self.residual_scales[i].item())
                    # Worst case: all input dimensions at max value, all v components same sign
                    max_contribution += scale * u_norm * v_norm * max(abs(self.input_range.min), abs(self.input_range.max))
                
                final_min = phi_range.min - max_contribution
                final_max = phi_range.max + max_contribution
            else:
    ...
    def forward(self, x, x_original=None):
        ...
        if CONFIG['use_residual_weights'] and x_original is not None:
            ...
            elif self.residual_U is not None and self.residual_V is not None:
                # Dimensions don't match, use factorized projection
                # Compute: sum_i scale_i * (u_i ⊗ v_i) @ x
                residual = torch.zeros_like(activated)
                for i in range(self.residual_rank):
                    # Efficient computation: (u ⊗ v) @ x = u * (v @ x)
                    v_dot_x = torch.matmul(x_original, self.residual_V[i])  # (batch,)
                    u_scaled = self.residual_U[i] * self.residual_scales[i]  # (d_out,)
                    residual += v_dot_x.unsqueeze(-1) * u_scaled.unsqueeze(0)
                activated = activated + residual
