    ...
    if 'residual' in categories and CONFIG['use_residual_weights']:
        ...
        for i, layer in enumerate(model.layers):
            if hasattr(layer, 'residual_weight') and layer.residual_weight is not None:
                ...
                lines.append(f"Layer {i} ({layer.d_in} → {layer.d_out}): factorized residual connection")
                lines.append(f"  Rank: {layer.residual_rank}")
                lines.append(format_tensor(layer.residual_U, "U vectors", "  "))
                lines.append(format_tensor(layer.residual_V, "V vectors", "  "))
                lines.append(format_tensor(layer.residual_scales, "Scales", "  "))
                has_residual = True
        if not has_residual:
