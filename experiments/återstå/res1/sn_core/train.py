...
def train_network(dataset, architecture, total_epochs=4000, print_every=400, 
    ...
    for name, param in model.named_parameters():
        ...
        elif 'residual_U' in name or 'residual_V' in name or 'residual_scales' in name:
            # Factorized residual parameters
            if CONFIG['use_residual_weights']:
                residual_params += param.numel()
    ...
    if CONFIG['use_residual_weights'] and residual_params > 0:
        ...
        # Count how many use scalar vs factorized
        scalar_residuals = 0
        factorized_residuals = 0
        for layer in model.layers:
            if hasattr(layer, 'residual_weight') and layer.residual_weight is not None:
                scalar_residuals += 1
            elif hasattr(layer, 'residual_U') and layer.residual_U is not None:
                factorized_residuals += 1
        if scalar_residuals > 0:
            print(f"    - Scalar residuals (d_in == d_out): {scalar_residuals} blocks")
        if factorized_residuals > 0:
            print(f"    - Factorized residuals (d_in != d_out): {factorized_residuals} blocks")
            print(f"    - Factorization rank: {CONFIG.get('residual_rank', 3)}")
    ...
    for idx, layer in enumerate(model.layers, start=1):
        ...
        if CONFIG['use_residual_weights']:
            if hasattr(layer, 'residual_weight') and layer.residual_weight is not None:
                ...
                print(f"Block {idx}: residual connection (factorized):")
                print(f"  Rank: {layer.residual_rank}")
                print(f"  U shape: {tuple(layer.residual_U.shape)}")
                print(f"  V shape: {tuple(layer.residual_V.shape)}")
                print(f"  Scales: {layer.residual_scales.detach().cpu().numpy()}")
        if CONFIG['train_phi_codomain']:
