...
def train_network(dataset, architecture, total_epochs=4000, print_every=400, 
    ...
    for name, param in model.named_parameters():
        ...
        if 'lambdas' in name:
            ...
            # Updated to count all residual-related parameters
    ...
    for idx, layer in enumerate(model.layers, start=1):
        ...
        if CONFIG['use_residual_weights']:
            ...
            elif layer.d_in != layer.d_out:
                # Print node-centric residual parameters
                if layer.d_in > layer.d_out:
                    print(f"Block {idx}: residual downsampling parameters:")
                    print(f"  input_importance: {layer.residual_input_importance.detach().cpu().numpy()}")
                    print(f"  output_scale: {layer.residual_output_scale.detach().cpu().numpy()}")
                else:
                    print(f"Block {idx}: residual upsampling parameters:")
                    print(f"  input_scale: {layer.residual_input_scale.detach().cpu().numpy()}")
                    print(f"  output_weights: {layer.residual_output_weights.detach().cpu().numpy()}")
        if CONFIG['train_phi_codomain']:
