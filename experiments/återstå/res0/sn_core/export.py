        ...
        for i, layer in enumerate(model.layers):
            if hasattr(layer, 'residual_weight') and layer.residual_weight is not None:
                ...
                if layer.d_in > layer.d_out:
                    lines.append(f"Layer {i} ({layer.d_in} → {layer.d_out}): Node-centric downsampling")
                    lines.append(format_tensor(layer.residual_input_importance, "Input importance", "  "))
                    lines.append(format_tensor(layer.residual_output_scale, "Output scale", "  "))
                    has_residual = True
                else:  # layer.d_in < layer.d_out
                    lines.append(f"Layer {i} ({layer.d_in} → {layer.d_out}): Node-centric upsampling")
                    lines.append(format_tensor(layer.residual_input_scale, "Input scale", "  "))
                    lines.append(format_tensor(layer.residual_output_weights, "Output weights", "  "))
                    has_residual = True
        if not has_residual:
