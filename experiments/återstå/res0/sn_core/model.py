    ...
    def __init__(self, d_in, d_out, layer_num=0, is_final=False, phi_knots=100, Phi_knots=100,
        ...
        if CONFIG['use_residual_weights']:
            if d_in == d_out:
                ...
                # When dimensions don't match, use node-centric approach
                self.residual_weight = None
                self.residual_projection = None  # No longer using nn.Linear
                
                if d_in > d_out:
                    # Downsampling: learnable pooling
                    self.residual_input_importance = nn.Parameter(torch.ones(d_in) * 0.1)
                    self.residual_output_scale = nn.Parameter(torch.ones(d_out) * 0.1)
                    # Store pooling indices as regular attributes (not buffers)
                    self.pooling_indices = self._compute_pooling_indices(d_in, d_out)
                else:
                    # Upsampling: learnable interpolation  
                    self.residual_input_scale = nn.Parameter(torch.ones(d_in) * 0.1)
                    self.residual_output_weights = nn.Parameter(torch.ones(d_out) * 0.1)
                    # Store interpolation indices as regular attributes (not buffers)
                    self.interp_indices = self._compute_interp_indices(d_in, d_out)
        else:
            self.residual_weight = None
            self.residual_projection = None
    
    def _compute_pooling_indices(self, d_in, d_out):
        """Compute which input indices contribute to each output for pooling."""
        indices = []
        base_size = d_in // d_out
        remainder = d_in % d_out
        
        start = 0
        for i in range(d_out):
            # Distribute remainder evenly
            size = base_size + (1 if i < remainder else 0)
            indices.append(torch.arange(start, start + size))
            start += size
        
        return indices
    
    def _compute_interp_indices(self, d_in, d_out):
        """Compute which output indices each input contributes to for interpolation."""
        indices = []
        base_outputs_per_input = d_out // d_in
        extra_outputs = d_out % d_in
        
        output_idx = 0
        for i in range(d_in):
            # Distribute extra outputs evenly
            n_outputs = base_outputs_per_input + (1 if i < extra_outputs else 0)
            indices.append(torch.arange(output_idx, output_idx + n_outputs))
            output_idx += n_outputs
    ...
    def compute_output_range_theoretical(self):
        ...
        if CONFIG['use_residual_weights'] and self.input_range is not None:
            if self.residual_weight is not None:
                ...
                # For node-centric residual, we need to be more conservative
                # since we don't have a simple weight norm bound
                if self.d_in > self.d_out:
                    # Downsampling case
                    max_importance = self.residual_input_importance.abs().max().item()
                    max_scale = self.residual_output_scale.abs().max().item()
                    residual_bound = max_importance * max_scale * max(abs(self.input_range.min), abs(self.input_range.max))
                else:
                    # Upsampling case
                    max_input_scale = self.residual_input_scale.abs().max().item()
                    max_output_weight = self.residual_output_weights.abs().max().item()
                    residual_bound = max_input_scale * max_output_weight * max(abs(self.input_range.min), abs(self.input_range.max))
    ...
    def forward(self, x, x_original=None):
        ...
        if CONFIG['use_residual_weights'] and x_original is not None:
            if self.residual_weight is not None:
                ...
                # Different dimensions - use node-centric approach
                if self.d_in > self.d_out:
                    # Downsampling via weighted pooling
                    # 1. Apply input importance weights
                    weighted_input = x_original * self.residual_input_importance.view(1, -1)
                    
                    # 2. Pool according to precomputed pattern
                    pooled = torch.zeros(x_original.shape[0], self.d_out, device=x_original.device)
                    for out_idx in range(self.d_out):
                        input_slice = self.pooling_indices[out_idx]
                        pooled[:, out_idx] = weighted_input[:, input_slice].mean(dim=1)
                    
                    # 3. Apply output scaling
                    residual_contrib = pooled * self.residual_output_scale.view(1, -1)
                    
                else:  # self.d_in < self.d_out  
                    # Upsampling via weighted interpolation
                    # 1. Scale inputs
                    scaled_input = x_original * self.residual_input_scale.view(1, -1)
                    
                    # 2. Distribute to outputs according to pattern
                    distributed = torch.zeros(x_original.shape[0], self.d_out, device=x_original.device)
                    for in_idx in range(self.d_in):
                        output_slice = self.interp_indices[in_idx]
                        distributed[:, output_slice] += scaled_input[:, in_idx:in_idx+1]
                    
                    # 3. Apply output weights
                    residual_contrib = distributed * self.residual_output_weights.view(1, -1)
                
                activated = activated + residual_contrib
