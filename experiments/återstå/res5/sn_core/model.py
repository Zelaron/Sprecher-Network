...
class SprecherLayerBlock(nn.Module):
    ...
    def __init__(self, d_in, d_out, layer_num=0, is_final=False, phi_knots=100, Phi_knots=100,
        ...
        if CONFIG['use_residual_weights']:
            if d_in == d_out:
                ...
                # No need for node-centric components when dimensions match
                self.residual_in_embed = None
                self.residual_out_embed = None
                self.residual_scale = None
                self.residual_spline = None
            else:
                # When dimensions don't match, use node-centric projection
                self.residual_weight = None
                self.residual_projection = None  # No Linear layer!
                
                # Learnable embeddings for input and output dimensions
                self.residual_in_embed = nn.Parameter(torch.randn(d_in) * 0.1)
                self.residual_out_embed = nn.Parameter(torch.randn(d_out) * 0.1)
                
                # Per-output scaling factors
                self.residual_scale = nn.Parameter(torch.ones(d_out) * 0.1)
                
                # Shared spline for computing attention weights
                # This spline learns a similarity function between dimension embeddings
                self.residual_spline = SimpleSpline(
                    num_knots=20,
                    in_range=(-2, 2),  # Initial range for embedding differences
                    out_range=(0, 1),   # Output will be passed through softmax
                    monotonic=False
                )
        else:
            self.residual_weight = None
    ...
    def update_Phi_domain_theoretical(self, allow_resampling=True, force_resample=False):
        ...
        with torch.no_grad():
            ...
            # Update residual spline domain if it exists
            if hasattr(self, 'residual_spline') and self.residual_spline is not None:
                # Update domain based on embedding differences
                embed_diff_max = 3.0  # Conservative estimate
                self.residual_spline.update_domain((-embed_diff_max, embed_diff_max), 
    ...
    def compute_output_range_theoretical(self):
        ...
        if CONFIG['use_residual_weights'] and self.input_range is not None:
            ...
            elif self.residual_scale is not None:
                # For node-centric projection, we need to be conservative
                # The attention weights sum to 1, so the residual contribution is bounded
                max_scale = torch.abs(self.residual_scale).max().item()
                residual_bound = max_scale * max(abs(self.input_range.min), abs(self.input_range.max))
    ...
    def _compute_residual_projection(self, x):
        """
        Node-centric projection for residual connections.
        Uses learned embeddings and a spline to compute attention weights.
        """
        batch_size = x.shape[0]
        
        # Compute pairwise embedding differences
        # Shape: [d_out, d_in]
        embed_diff = self.residual_out_embed.unsqueeze(1) - self.residual_in_embed.unsqueeze(0)
        
        # Apply spline to compute attention logits
        # The spline learns a similarity function between dimensions
        attention_logits = self.residual_spline(embed_diff)
        
        # Apply softmax to get normalized attention weights
        # Each output dimension attends to all input dimensions
        attention_weights = torch.softmax(attention_logits, dim=1)
        
        # Apply attention pooling: [batch, d_out] = [batch, d_in] @ [d_in, d_out]
        # This is equivalent to: output[i] = sum_j(attention[i,j] * input[j])
        pooled = torch.matmul(x, attention_weights.T)
        
        # Apply per-output scaling
        # This allows each output dimension to have its own gain
        return pooled * self.residual_scale
    ...
    def forward(self, x, x_original=None):
        ...
        if CONFIG['use_residual_weights'] and x_original is not None:
            if self.residual_weight is not None:
                ...
                residual_out = self._compute_residual_projection(x_original)
                activated = activated + residual_out
    ...
    def get_domain_violation_stats(self):
        ...
        # Add residual spline stats if it exists
        if hasattr(self, 'residual_spline') and self.residual_spline is not None:
            stats['residual_spline'] = self.residual_spline.get_domain_violation_stats()
        return stats
...
class SprecherMultiLayerNetwork(nn.Module):
    ...
    def print_domain_violation_report(self):
        ...
        for layer_name, layer_stats in stats.items():
            ...
            print(f"  Phi violations: {layer_stats['Phi']:.2%}")
            if 'residual_spline' in layer_stats:
                print(f"  residual_spline violations: {layer_stats['residual_spline']:.2%}")
    ...
    def get_all_domain_states(self):
        ...
        for i, layer in enumerate(self.layers):
            ...
            domain_states[f'layer_{i}_Phi'] = layer.Phi.get_domain_state()
            # Also save residual spline state if it exists
            if hasattr(layer, 'residual_spline') and layer.residual_spline is not None:
                domain_states[f'layer_{i}_residual_spline'] = layer.residual_spline.get_domain_state()
    ...
    def get_domain_ranges(self):
        ...
        for i, layer in enumerate(self.layers):
            ...
            domain_ranges[f'layer_{i}_Phi'] = (layer.Phi.in_min, layer.Phi.in_max)
            if hasattr(layer, 'residual_spline') and layer.residual_spline is not None:
                domain_ranges[f'layer_{i}_residual_spline'] = (layer.residual_spline.in_min, layer.residual_spline.in_max)
    ...
    def set_all_domain_states(self, domain_states):
        ...
        for i, layer in enumerate(self.layers):
            ...
            Phi_key = f'layer_{i}_Phi'
            residual_key = f'layer_{i}_residual_spline'
            ...
            if Phi_key in domain_states:
                layer.Phi.set_domain_state(domain_states[Phi_key])
            # Restore residual spline state if it exists
            if residual_key in domain_states and hasattr(layer, 'residual_spline') and layer.residual_spline is not None:
