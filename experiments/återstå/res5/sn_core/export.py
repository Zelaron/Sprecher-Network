...
def export_parameters(model, param_types, save_path, dataset_info=None, checkpoint_info=None):
    ...
    for i, layer in enumerate(model.layers):
        ...
        if CONFIG['use_residual_weights']:
            if hasattr(layer, 'residual_weight') and layer.residual_weight is not None:
                residual_type = " [scalar residual]"
            elif hasattr(layer, 'residual_scale') and layer.residual_scale is not None:
                residual_type = " [node-centric residual]"
        lines.append(f"Layer {i}: {layer.d_in} → {layer.d_out}{is_final_str}{residual_type}")
    lines.append("")
    ...
    if 'spline' in categories:
        ...
        for i, layer in enumerate(model.layers):
            ...
            # Residual spline (if it exists)
            if hasattr(layer, 'residual_spline') and layer.residual_spline is not None:
                lines.append("  Residual spline (for attention weights):")
    ...
    if 'residual' in categories and CONFIG['use_residual_weights']:
        ...
        for i, layer in enumerate(model.layers):
            if hasattr(layer, 'residual_weight') and layer.residual_weight is not None:
                ...
                # Node-centric residual case
                lines.append(f"Layer {i} ({layer.d_in} → {layer.d_out}): NODE-CENTRIC RESIDUAL")
                lines.append(format_tensor(layer.residual_in_embed, "Input embeddings", "  "))
                lines.append(format_tensor(layer.residual_out_embed, "Output embeddings", "  "))
                lines.append(format_tensor(layer.residual_scale, "Output scale", "  "))
                lines.append("  Residual spline: see 'spline' section for details")
                has_residual = True
        if not has_residual:
