...
def train_network(dataset, architecture, total_epochs=4000, print_every=400, 
    ...
    for name, param in model.named_parameters():
        ...
        if 'lambdas' in name:
            ...
            # Spline parameters (including residual spline)
            spline_params += param.numel()
        elif 'residual_weight' in name:
            # Scalar residual weight for same-dimension case
            if CONFIG['use_residual_weights']:
                residual_params += param.numel()
        elif 'residual_in_embed' in name or 'residual_out_embed' in name or 'residual_scale' in name:
            # Node-centric residual parameters
            if CONFIG['use_residual_weights']:
                residual_params += param.numel()
    ...
    if CONFIG['use_residual_weights'] and residual_params > 0:
        ...
        # Count how many blocks use node-centric vs scalar residuals
        node_centric_blocks = 0
        scalar_blocks = 0
        for layer in model.layers:
            if hasattr(layer, 'residual_weight') and layer.residual_weight is not None:
                scalar_blocks += 1
            elif hasattr(layer, 'residual_scale') and layer.residual_scale is not None:
                node_centric_blocks += 1
        if node_centric_blocks > 0:
            print(f"    - {node_centric_blocks} blocks use node-centric projection")
        if scalar_blocks > 0:
            print(f"    - {scalar_blocks} blocks use scalar weight")
            ...
            if CONFIG.get('debug_domains', False):
                ...
                for idx, layer in enumerate(model.layers):
                    ...
                    if hasattr(layer, 'residual_spline') and layer.residual_spline is not None:
                        print(f"  Layer {idx}: residual_spline domain=[{layer.residual_spline.in_min:.3f}, {layer.residual_spline.in_max:.3f}]")
                print()
    ...
    for idx, layer in enumerate(model.layers, start=1):
        ...
        if CONFIG['use_residual_weights']:
            if hasattr(layer, 'residual_weight') and layer.residual_weight is not None:
                ...
                print(f"Block {idx}: Node-centric residual projection:")
                print(f"  - Input embeddings shape: {tuple(layer.residual_in_embed.shape)}")
                print(f"  - Output embeddings shape: {tuple(layer.residual_out_embed.shape)}")
                print(f"  - Output scale: {layer.residual_scale.detach().cpu().numpy()}")
                print(f"  - Residual spline knots: {layer.residual_spline.num_knots}")
        if CONFIG['train_phi_codomain']:
