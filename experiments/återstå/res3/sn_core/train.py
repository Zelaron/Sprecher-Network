...
def train_network(dataset, architecture, total_epochs=4000, print_every=400, 
    ...
    for name, param in model.named_parameters():
        ...
        elif 'residual_weight' in name or 'residual_alpha' in name or 'residual_beta' in name:
            # Updated to count the new node-centric residual parameters
    ...
    for idx, layer in enumerate(model.layers, start=1):
        ...
        if CONFIG['use_residual_weights']:
            ...
            elif hasattr(layer, 'residual_alpha') and layer.residual_alpha is not None:
                # Print the new node-centric residual parameters
                print(f"Block {idx}: residual_alpha shape = {tuple(layer.residual_alpha.shape)}")
                print(f"Block {idx}: residual_alpha =")
                print(layer.residual_alpha.detach().cpu().numpy())
                print(f"Block {idx}: residual_beta shape = {tuple(layer.residual_beta.shape)}")
                print(f"Block {idx}: residual_beta =")
                print(layer.residual_beta.detach().cpu().numpy())
        if CONFIG['train_phi_codomain']:
