    ...
    if 'residual' in categories and CONFIG['use_residual_weights']:
        ...
        for i, layer in enumerate(model.layers):
            if hasattr(layer, 'residual_weight') and layer.residual_weight is not None:
                ...
                # Export the new node-centric residual parameters
                lines.append(f"Layer {i} ({layer.d_in} → {layer.d_out}): node-centric residual")
                lines.append(format_tensor(layer.residual_alpha, "Alpha (input modulation)", "  "))
                lines.append(format_tensor(layer.residual_beta, "Beta (output modulation)", "  "))
                # Also show the basis matrix info
                lines.append(f"  Basis matrix shape: {tuple(layer.residual_basis.shape)} (fixed)")
                lines.append(f"  Note: Basis implements soft dimensional mapping with Gaussian-like receptive fields")
                has_residual = True
        if not has_residual:
