...
class SprecherLayerBlock(nn.Module):
    ...
    def __init__(self, d_in, d_out, layer_num=0, is_final=False, phi_knots=100, Phi_knots=100,
        ...
        if CONFIG['use_residual_weights']:
            if d_in == d_out:
                ...
                # Learnable vectors for modulation
                self.residual_alpha = nn.Parameter(torch.randn(d_in) * 0.1)
                self.residual_beta = nn.Parameter(torch.randn(d_out) * 0.1)
                
                # Create fixed basis matrix for dimension mapping
                self.register_buffer('residual_basis', self._create_residual_basis(d_in, d_out))
        else:
            self.residual_weight = None
            self.residual_projection = None
            self.residual_alpha = None
    ...
    def _create_residual_basis(self, d_in, d_out):
        """
        Create a fixed basis matrix for dimension mapping using Gaussian-like receptive fields.
        Each output dimension gathers from a window of input dimensions with smooth weighting.
        """
        # Create position arrays for input and output dimensions
        in_positions = torch.arange(d_in, dtype=torch.float32)
        out_positions = torch.arange(d_out, dtype=torch.float32)
        
        # Normalize positions to [0, 1] range
        if d_in > 1:
            in_positions = in_positions / (d_in - 1)
        if d_out > 1:
            out_positions = out_positions / (d_out - 1)
        
        # Compute the receptive field width
        # Make it proportional to the dimension ratio for smooth coverage
        sigma = 0.5 * max(1.0 / d_out, 1.0 / d_in)
        
        # Create the basis matrix using Gaussian-like weights
        # Shape: (d_in, d_out)
        basis = torch.zeros(d_in, d_out)
        
        for j in range(d_out):
            # For each output dimension, create weights over input dimensions
            # Center the Gaussian at the corresponding position
            center = out_positions[j]
            
            # Compute distances from this center to all input positions
            distances = (in_positions - center).abs()
            
            # Apply Gaussian weighting
            weights = torch.exp(-0.5 * (distances / sigma) ** 2)
            
            # Normalize so each output dimension receives unit total weight
    ...
    def compute_output_range_theoretical(self):
        ...
        if CONFIG['use_residual_weights'] and self.input_range is not None:
            ...
            elif self.residual_alpha is not None and self.residual_beta is not None:
                # For node-centric projection, we need to be more conservative
                # The contribution depends on the interplay of alpha, beta, and the basis
                # Compute bounds based on the maximum possible amplification
                alpha_bound = torch.abs(self.residual_alpha).max().item()
                beta_bound = torch.abs(self.residual_beta).max().item()
                input_bound = max(abs(self.input_range.min), abs(self.input_range.max))
                
                # Since basis is normalized (each column sums to 1), the maximum
                # contribution is bounded by the product of bounds
                residual_bound = alpha_bound * beta_bound * input_bound
    ...
    def forward(self, x, x_original=None):
        ...
        if CONFIG['use_residual_weights'] and x_original is not None:
            if self.residual_weight is not None:
                ...
                # Different dimensions - use NODE-CENTRIC projection
                
                # Step 1: Modulate input by alpha
                x_modulated = x_original * self.residual_alpha.view(1, -1)
                
                # Step 2: Apply basis transformation
                # This "gathers" inputs according to the fixed pattern
                # Shape: (batch, d_in) @ (d_in, d_out) -> (batch, d_out)
                x_gathered = torch.matmul(x_modulated, self.residual_basis)
                
                # Step 3: Modulate output by beta
                x_projected = x_gathered * self.residual_beta.view(1, -1)
                
                activated = activated + x_projected
