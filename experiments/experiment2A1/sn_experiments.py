...
from sn_core import (
    ...
    export_parameters,
    parse_param_types,
    evaluation_mode,  # <-- NEW: use safe eval context for ALL evaluation forwards
)
...
def profile_memory_usage(model, x_sample):
    """Profile memory usage of forward pass (non-mutating, eval semantics)."""
    import torch.cuda
    # Always evaluate without mutating BN / running stats
    if x_sample.is_cuda:
...
def main():
    ...
    # ------------------------------------------------------------------
    # ENFORCE EVAL SEMANTICS for *all* evaluation work below
    # ------------------------------------------------------------------
    model.eval()  # global guard; we still use evaluation_mode(...) for each forward

    # Verify the snapshot works correctly with multiple computations
    print("\nVerifying plotting snapshot consistency...")
    print("Computing EVAL-mode loss multiple times to ensure perfect reproducibility:")
    ...
    # Compare against the checkpoint's eval loss if present (falls back to train loss)
    ref_loss = plotting_snapshot.get("loss_eval", plotting_snapshot["loss"])
    print(f"\nSaved snapshot eval loss: {plotting_snapshot.get('loss_eval', float('nan')):.4e}")
    print(f"(For reference) Saved snapshot train loss: {plotting_snapshot['loss']:.4e}")
    print(f"Mean of verifications: {np.mean(losses_verification):.4e}")
    print(f"Std of verifications: {np.std(losses_verification):.4e}")

    max_diff = max(abs(l - ref_loss) for l in losses_verification)
    print(f"Max difference from saved EVAL loss: {max_diff:.4e}")
    ...
    print(f"Model training mode (False means EVAL): {model.training}")
