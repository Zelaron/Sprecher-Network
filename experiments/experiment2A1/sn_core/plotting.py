...
def plot_loss_curve(losses, save_path=None):
    ...
    # Add minor ticks for better readability; scale w.r.t. run length
    # Default to ~10 minor tick steps.
    minor_step = max(1, len(losses) // 10)
    ax = plt.gca()
    ...
    plt.text(0.65, 0.95, textstr, transform=ax.transAxes, fontsize=10,
