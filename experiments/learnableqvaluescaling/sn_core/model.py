...
class SprecherLayerBlock(nn.Module):
    ...
    def __init__(self, d_in, d_out, layer_num=0, is_final=False, phi_knots=100, Phi_knots=100,
        ...
        # Learnable Q-value scaling factor
    ...
    def update_Phi_domain_theoretical(self, allow_resampling=True, force_resample=False):
        ...
        with torch.no_grad():
            ...
            # Plus q term adds [0, q_scale*(d_out-1)]
            q_scale_val = self.q_scale.item()
            if q_scale_val >= 0:
                phi_domain_min = lambda_sum_neg
                phi_domain_max = lambda_sum_pos + q_scale_val * (self.d_out - 1)
            else:
                # When q_scale is negative, it can decrease the minimum
                phi_domain_min = lambda_sum_neg + q_scale_val * (self.d_out - 1)
                phi_domain_max = lambda_sum_pos
    ...
    def forward(self, x, x_original=None):
        ...
        s = weighted.sum(dim=1) + self.q_scale * self.q_values  # shape: (batch_size, d_out)
