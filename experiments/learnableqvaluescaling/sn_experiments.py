...
def main():
    ...
    try:
        ...
        print(f"DEBUG: About to call plot_results with save_path={save_path}")
        try:
            plot_results(model, layers, dataset, save_path, x_train=x_train, y_train=y_train)
            print("DEBUG: plot_results completed successfully")
        except Exception as plot_e:
            print(f"ERROR in plot_results: {type(plot_e).__name__}: {plot_e}")
            import traceback
            traceback.print_exc()
        
        loss_filename = f"loss-{args.dataset}-{arch_str}-{args.epochs}-epochs{config_suffix}.png"
        loss_save_path = os.path.join("plots", loss_filename)
        print(f"DEBUG: About to call plot_loss_curve with save_path={loss_save_path}")
        try:
            plot_loss_curve(losses, loss_save_path)
            print("DEBUG: plot_loss_curve completed successfully")
        except Exception as loss_e:
            print(f"ERROR in plot_loss_curve: {type(loss_e).__name__}: {loss_e}")
            import traceback
            traceback.print_exc()
