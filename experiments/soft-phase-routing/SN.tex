...
\begin{itemize}
    \item \textbf{Residual connections (Learnable routing):} When enabled, residual connections adapt to dimensional mismatches using a learnable soft routing mechanism inspired by attention mechanisms. This approach maintains parameter efficiency while providing more flexibility than fixed patterns:
    \begin{itemize}
        \item \textbf{Identity} ($d_{\mathrm{in}} = d_{\mathrm{out}}$): Uses a single learnable scalar weight $w_{\text{res}}$
        \item \textbf{Broadcasting} ($d_{\mathrm{in}} < d_{\mathrm{out}}$): Each output learns a phase $\theta_q \in [0, 2\pi]$ to softly select from inputs
        \item \textbf{Pooling} ($d_{\mathrm{in}} > d_{\mathrm{out}}$): Each input has a learnable phase $\theta_i \in [0, 2\pi]$ and importance weight $w_i$
    \end{itemize}
    
    For the pooling case ($d_{\mathrm{in}} > d_{\mathrm{out}}$), the routing matrix $R \in \mathbb{R}^{d_{\mathrm{in}} \times d_{\mathrm{out}}}$ is computed as:
    \begin{align}
    A_{iq} &= \frac{1}{2}\left(1 + \cos\left(\theta_i - \frac{2\pi q}{d_{\mathrm{out}}}\right)\right) \\
    R_{iq} &= \frac{\exp(\tau A_{iq})}{\sum_{q'=0}^{d_{\mathrm{out}}-1} \exp(\tau A_{iq'})}
    \end{align}
    where $\tau$ is a temperature parameter. The residual contribution becomes:
    $$[B^{(\ell)}_{\text{res}}(\mathbf{x})]_q = [B^{(\ell)}(\mathbf{x})]_q + \sum_{i=0}^{d_{\mathrm{in}}-1} R_{iq} w_i x_i$$
    
    For broadcasting ($d_{\mathrm{in}} < d_{\mathrm{out}}$), a similar mechanism assigns outputs to inputs:
    \begin{align}
    A_{qi} &= \frac{1}{2}\left(1 + \cos\left(\theta_q - \frac{2\pi i}{d_{\mathrm{in}}}\right)\right) \\
    R_{qi} &= \frac{\exp(\tau A_{qi})}{\sum_{i'=0}^{d_{\mathrm{in}}-1} \exp(\tau A_{qi'})}
    \end{align}
    with residual contribution:
    $$[B^{(\ell)}_{\text{res}}(\mathbf{x})]_q = [B^{(\ell)}(\mathbf{x})]_q + \sum_{i=0}^{d_{\mathrm{in}}-1} R_{qi} x_i$$
    
    This learnable routing mechanism allows the network to discover optimal connection patterns during training while maintaining $O(\max(d_{\mathrm{in}}, d_{\mathrm{out}}))$ parameters per skip connection.
...
\begin{remark}[Learnable routing as soft weight sharing]
The learnable routing mechanism can be understood as implementing soft, differentiable weight sharing. The phase parameters $\theta_i$ or $\theta_q$ determine which inputs connect to which outputs through a continuous affinity function. The cosine similarity creates smooth transitions between connections, allowing gradient-based optimization to discover effective routing patterns. The temperature parameter $\tau$ controls the sharpness of the routing: as $\tau \to \infty$, the routing approaches hard assignment, while $\tau \to 0$ creates uniform distribution. In practice, $\tau = 1$ provides a good balance. The initialization strategy (uniform, random, or identity-approximating) can significantly impact optimization dynamics.
\end{remark}

\begin{remark}[Structured sparsity and architectural coherence]
The learnable routing design can be viewed as implementing a structured sparse projection where the sparsity pattern itself is learned. In the pooling case, each output learns to attend to a soft subset of inputs, while in the broadcasting case, each output learns which input to primarily follow. This represents another form of weight sharing that complements the main Sprecher architecture: while Sprecher blocks share splines across output dimensions with diversity through shifts, residual connections share structure across connections with diversity through learned routing. The effectiveness of this approach suggests that for gradient flow and information propagation, the flexibility to learn connection patterns may be more important than the specific pattern used, motivating this more sophisticated approach over fixed cyclic assignments.
\end{remark}
...
\textbf{Residual design}    & Matrix projection   & Matrix projection  & None (typically)      & Learnable routing \\
...
Sprecher Networks have a fundamentally different structure. Each block $\ell$ contains mixing weights $\lambda^{(\ell)}$ with $O(d_{\ell-1})$ parameters, where $d_{\ell-1}$ is the input dimension to that block (crucially, these are vectors, not matrices), shared splines $\phi^{(\ell)}, \Phi^{(\ell)}$ with $2 \times O(G) = O(G)$ parameters per block independent of $N$, and a shift parameter $\eta^{(\ell)}$ with $O(1)$ parameter per block. When normalization is used, each block may have an associated normalization layer adding $O(N)$ parameters (specifically $2N$ for the affine transformation). When learnable routing is used for residual connections, the parameter count increases modestly:
\begin{itemize}
\item For $d_{\mathrm{in}} = d_{\mathrm{out}}$: adds 1 parameter (scalar weight)
\item For $d_{\mathrm{in}} > d_{\mathrm{out}}$ (pooling): adds $2d_{\mathrm{in}}$ parameters ($d_{\mathrm{in}}$ phases + $d_{\mathrm{in}}$ importance weights)
\item For $d_{\mathrm{in}} < d_{\mathrm{out}}$ (broadcasting): adds $d_{\mathrm{out}}$ parameters (phases only)
\end{itemize}
This maintains the $O(N)$ scaling per block, as the maximum addition is $2\max(d_{\mathrm{in}}, d_{\mathrm{out}})$. Summing over $L$ (or $L+1$) blocks, the total parameter count scales approximately as $O(LN + LG + L)$, where the linear dependence on $N$ now includes mixing weights, optional normalization parameters, and residual connections.
...
\subsection{Learnable routing parameters}
When residual connections encounter dimensional mismatches, the network employs learnable routing parameters:
\begin{itemize}
\item \textbf{Routing phases} $\theta \in \mathbb{R}^d$: Initialized according to strategy (uniform spacing, random, or identity-approximating)
\item \textbf{Importance weights} $w \in \mathbb{R}^d$ (pooling only): Initialized to $1/\sqrt{d}$ for variance preservation
\item \textbf{Temperature} $\tau$: Typically fixed at 1.0, though can be made learnable or scheduled
\end{itemize}

The routing matrices are recomputed at each forward pass using the current phase parameters, making the routing fully differentiable. This allows the network to learn task-specific connection patterns while maintaining the parameter efficiency that is central to the SN philosophy.
...
\item When residual connections are present, the output range must be adjusted by the residual contribution. For a scalar residual weight $w$, if $w \geq 0$, the residual adds $[wa, wb]$ to the range where $[a,b]$ is the input range. If $w < 0$, it adds $[wb, wa]$ (note the reversal). For learnable routing residuals:
   \begin{itemize}
   \item \textbf{Broadcast}: The contribution depends on the routing matrix, but is bounded by $[a, b]$ since each output receives a convex combination of inputs
   \item \textbf{Pooling}: Each output receives a weighted sum where the weights $w_i$ can be positive or negative, so the contribution is bounded by $[\min(0, \sum_i |w_i| \cdot \text{sign}(w_i) \cdot a), \max(0, \sum_i |w_i| \cdot \text{sign}(w_i) \cdot b)]$
   \end{itemize}
\end{enumerate}
\end{lemma}
...
For (4), the learnable routing residual contributions follow from analyzing the routing matrices and weight vectors.
\end{proof}
