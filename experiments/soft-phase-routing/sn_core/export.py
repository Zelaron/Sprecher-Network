...
def export_parameters(model, param_types, save_path, dataset_info=None, checkpoint_info=None):
    ...
    # Export Residual parameters (updated for learnable routing)
    if 'residual' in categories and CONFIG['use_residual_weights']:
        lines.append("RESIDUAL CONNECTION PARAMETERS (Learnable Routing)")
        lines.append("-" * 40)
        has_residual = False
        for i, layer in enumerate(model.layers):
            ...
            # Check for learnable routing (different dimensions)
            elif hasattr(layer, 'residual_routing_phases') and layer.residual_routing_phases is not None:
                if layer.d_in > layer.d_out:
                    lines.append(f"Layer {i} ({layer.d_in} → {layer.d_out}): POOLING with learnable routing")
                    lines.append(f"  Routing mechanism: Soft attention based on cosine similarity")
                    lines.append(f"  Temperature: {CONFIG.get('routing_temperature', 1.0)}")
                    lines.append(format_tensor(layer.residual_routing_phases, "Routing phases", "  "))
                    lines.append(format_tensor(layer.residual_routing_weights, "Importance weights", "  "))
                    
                    # Optionally show the computed routing matrix
                    if CONFIG.get('export_routing_matrices', False):
                        routing_matrix = layer._compute_routing_matrix_pooling()
                        lines.append(format_tensor(routing_matrix, "Routing matrix (input→output)", "  "))
                        lines.append("  Note: Each row shows how input i distributes to outputs")
                        
                        # Show effective routing summary
                        lines.append("  Effective routing (strongest connections):")
                        with torch.no_grad():
                            for in_idx in range(min(layer.d_in, 10)):  # Show first 10 inputs
                                row = routing_matrix[in_idx]
                                max_idx = torch.argmax(row).item()
                                max_val = row[max_idx].item()
                                lines.append(f"    Input {in_idx} → Output {max_idx} (strength: {max_val:.3f})")
                        if layer.d_in > 10:
                            lines.append(f"    ... ({layer.d_in - 10} more inputs)")
                    
                else:  # d_in < d_out
                    lines.append(f"Layer {i} ({layer.d_in} → {layer.d_out}): BROADCAST with learnable routing")
                    lines.append(f"  Routing mechanism: Soft attention based on cosine similarity")
                    lines.append(f"  Temperature: {CONFIG.get('routing_temperature', 1.0)}")
                    lines.append(format_tensor(layer.residual_routing_phases, "Routing phases", "  "))
                    
                    if CONFIG.get('export_routing_matrices', False):
                        routing_matrix = layer._compute_routing_matrix_broadcast()
                        lines.append(format_tensor(routing_matrix, "Routing matrix (output→input)", "  "))
                        lines.append("  Note: Each row shows how output i selects from inputs")
                        
                        # Show effective routing summary
                        lines.append("  Effective routing (strongest connections):")
                        with torch.no_grad():
                            for out_idx in range(min(layer.d_out, 10)):  # Show first 10 outputs
                                row = routing_matrix[out_idx]
                                max_idx = torch.argmax(row).item()
                                max_val = row[max_idx].item()
                                lines.append(f"    Output {out_idx} ← Input {max_idx} (strength: {max_val:.3f})")
                        if layer.d_out > 10:
                            lines.append(f"    ... ({layer.d_out - 10} more outputs)")
