...
class SprecherLayerBlock(nn.Module):
    ...
    def __init__(self, d_in, d_out, layer_num=0, is_final=False, phi_knots=100, Phi_knots=100,
        ...
        # Node-centric residual connection with learnable routing
        if CONFIG['use_residual_weights']:
            if d_in == d_out:
                # When dimensions match, use a simple scalar weight
                self.residual_weight = nn.Parameter(torch.tensor(0.1))
                # No routing needed for same dimensions
                self.residual_routing_phases = None
                self.residual_routing_weights = None
                # Clear old parameters
                self.residual_pooling_weights = None
                self.residual_broadcast_weights = None
                self.pooling_assignment = None
                self.pooling_counts = None
                self.broadcast_sources = None
            elif d_in > d_out:
                # Pooling case: aggregate multiple inputs per output
                self.residual_weight = None
                
                # Initialize routing phases based on configuration
                routing_init = CONFIG.get('routing_init', 'uniform')
                if routing_init == 'uniform':
                    # Uniform spacing in phase space
                    init_phases = torch.linspace(0, 2*np.pi, d_in, dtype=torch.float32)
                elif routing_init == 'random':
                    # Random phases
                    init_phases = torch.rand(d_in) * 2 * np.pi
                elif routing_init == 'identity':
                    # Try to approximate round-robin initially
                    init_phases = torch.zeros(d_in)
                    for i in range(d_in):
                        # Assign phase based on which output this would go to in round-robin
                        output_idx = i % d_out
                        init_phases[i] = 2 * np.pi * output_idx / d_out
                else:
                    # Default to uniform
                    init_phases = torch.linspace(0, 2*np.pi, d_in, dtype=torch.float32)
                
                # Learnable routing phases for each input
                self.residual_routing_phases = nn.Parameter(init_phases)
                
                # Learnable importance weights for each input
                self.residual_routing_weights = nn.Parameter(
                    torch.ones(d_in) / np.sqrt(d_in)
                )
                ...
                if routing_init == 'uniform':
                    # Uniform spacing in phase space
                    init_phases = torch.linspace(0, 2*np.pi, d_out, dtype=torch.float32)
                elif routing_init == 'random':
                    # Random phases
                    init_phases = torch.rand(d_out) * 2 * np.pi
                elif routing_init == 'identity':
                    # Try to approximate round-robin initially
                    init_phases = torch.zeros(d_out)
                    for i in range(d_out):
                        # Assign phase based on which input this would take in round-robin
                        input_idx = i % d_in
                        init_phases[i] = 2 * np.pi * input_idx / d_in
                else:
                    # Default to uniform
                    init_phases = torch.linspace(0, 2*np.pi, d_out, dtype=torch.float32)
                
                # Learnable routing phases for each output
                self.residual_routing_phases = nn.Parameter(init_phases)
                
                # No additional weights needed for broadcasting
                self.residual_routing_weights = None
    ...
    def _compute_routing_matrix_pooling(self):
        """Compute soft routing from inputs to outputs (d_in > d_out)."""
        # Create position encodings for outputs
        output_positions = 2 * np.pi * torch.arange(
            self.d_out, device=self.residual_routing_phases.device, 
            dtype=torch.float32
        ) / self.d_out
        
        # Compute affinity matrix using cosine similarity
        # Shape: (d_in, d_out)
        affinity = 0.5 * (1 + torch.cos(
            self.residual_routing_phases.unsqueeze(1) - output_positions.unsqueeze(0)
        ))
        
        # Apply temperature and softmax normalization
        temperature = CONFIG.get('routing_temperature', 1.0)
        routing_matrix = torch.softmax(affinity * temperature, dim=1)
        
        return routing_matrix

    def _compute_routing_matrix_broadcast(self):
        """Compute soft routing from inputs to outputs (d_in < d_out)."""
        # Create position encodings for inputs
        input_positions = 2 * np.pi * torch.arange(
            self.d_in, device=self.residual_routing_phases.device,
            dtype=torch.float32
        ) / self.d_in
        ...
        # Shape: (d_out, d_in)
        affinity = 0.5 * (1 + torch.cos(
            self.residual_routing_phases.unsqueeze(1) - input_positions.unsqueeze(0)
        ))
    ...
    def compute_output_range_theoretical(self):
        ...
        if CONFIG['use_residual_weights'] and self.input_range is not None:
            ...
            elif self.residual_routing_phases is not None:
                # For learnable routing, we need conservative bounds
                # Since routing weights are normalized (sum to 1), the contribution is bounded
                if self.d_in > self.d_out and self.residual_routing_weights is not None:
                    # Pooling case: weighted sum of inputs
                    # Conservative estimate: all weights could be positive or negative
                    weight_sum_max = torch.sum(torch.abs(self.residual_routing_weights)).item()
                    ...
                    # Broadcasting case: each output gets weighted sum of inputs
                    # Since routing matrix rows sum to 1, contribution is bounded by input range
                    input_bound = max(abs(self.input_range.min), abs(self.input_range.max))
                    final_min = phi_range.min - input_bound
                    final_max = phi_range.max + input_bound
            else:
    ...
    def forward(self, x, x_original=None):
        ...
        if CONFIG['use_residual_weights'] and x_original is not None:
            ...
            elif self.d_in > self.d_out and self.residual_routing_phases is not None:
                # Pooling case with learnable routing
                # Compute soft routing matrix
                routing_matrix = self._compute_routing_matrix_pooling()
                
                # Apply importance weights and route
                weighted_input = x_original * self.residual_routing_weights.view(1, -1)
                # Shape: (batch_size, d_in) @ (d_in, d_out) -> (batch_size, d_out)
                residual_contribution = torch.matmul(weighted_input, routing_matrix)
                
                activated = activated + residual_contribution
                
            elif self.d_in < self.d_out and self.residual_routing_phases is not None:
                # Broadcasting case with learnable routing
                # Compute soft routing matrix
                routing_matrix = self._compute_routing_matrix_broadcast()
                
                # Route inputs to outputs
                # Shape: (batch_size, d_out) <- (d_out, d_in) @ (batch_size, d_in)^T then transpose
                residual_contribution = torch.matmul(routing_matrix, x_original.t()).t()
