...
def train_network(dataset, architecture, total_epochs=4000, print_every=400, 
    ...
    # Compute parameter counts for TRUE SPRECHER NETWORK with learnable routing
    ...
    residual_routing_phase_params = 0
    residual_routing_weight_params = 0
    codomain_params = 0
    norm_params = 0
    ...
    for name, param in model.named_parameters():
        ...
        elif 'residual_routing_phases' in name:
            # Routing phases for learnable routing
            if CONFIG['use_residual_weights']:
                residual_routing_phase_params += param.numel()
                residual_params += param.numel()
        elif 'residual_routing_weights' in name:
            # Importance weights for pooling
            if CONFIG['use_residual_weights']:
                residual_routing_weight_params += param.numel()
                residual_params += param.numel()
    ...
    if CONFIG['use_residual_weights'] and residual_params > 0:
        print(f"  - Residual connection parameters: {residual_params}")
        if residual_scalar_params > 0:
            print(f"    * Scalar weights (same dims): {residual_scalar_params}")
        if residual_routing_phase_params > 0:
            print(f"    * Routing phases (learnable): {residual_routing_phase_params}")
        if residual_routing_weight_params > 0:
            print(f"    * Routing importance weights: {residual_routing_weight_params}")
    ...
    # Print routing configuration if using residual connections
    if CONFIG['use_residual_weights']:
        print(f"\nRouting configuration:")
        print(f"  - Initialization: {CONFIG.get('routing_init', 'uniform')}")
        ...
        if CONFIG['use_residual_weights']:
            if hasattr(layer, 'residual_weight') and layer.residual_weight is not None:
                ...
                if layer.d_in > layer.d_out:
                    print(f"Block {idx}: POOLING residual with learnable routing")
                    print(f"Block {idx}: routing_phases shape = {tuple(layer.residual_routing_phases.shape)}")
                    # Show a few phase values
                    phases = layer.residual_routing_phases.detach().cpu().numpy()
                    if len(phases) <= 10:
                        print(f"Block {idx}: routing_phases = {phases}")
                    else:
                        print(f"Block {idx}: routing_phases (first 10) = {phases[:10]}")
                        print(f"             ... ({len(phases) - 10} more)")
                    print(f"Block {idx}: routing_weights shape = {tuple(layer.residual_routing_weights.shape)}")
                    weights = layer.residual_routing_weights.detach().cpu().numpy()
                    if len(weights) <= 10:
                        print(f"Block {idx}: routing_weights = {weights}")
                    else:
                        print(f"Block {idx}: routing_weights (first 10) = {weights[:10]}")
                        print(f"             ... ({len(weights) - 10} more)")
                else:  # d_in < d_out
                    print(f"Block {idx}: BROADCAST residual with learnable routing")
                    print(f"Block {idx}: routing_phases shape = {tuple(layer.residual_routing_phases.shape)}")
                    phases = layer.residual_routing_phases.detach().cpu().numpy()
                    if len(phases) <= 10:
                        print(f"Block {idx}: routing_phases = {phases}")
