...
def parse_args():
    ...
    parser.add_argument("--routing_temp", type=float, default=1.0,
                      help="Temperature for soft routing: 0.1-0.5 (sharp), 1.0 (balanced), 2.0-5.0 (smooth). "
                           "Only affects layers where d_in != d_out. (default: 1.0)")
    parser.add_argument("--routing_init_noise", type=float, default=0.1,
                      help="Noise level for routing initialization (default: 0.1)")
    parser.add_argument("--no_circular_routing", action="store_true",
                      help="Disable circular distance for routing (default: enabled)")
...
def get_config_suffix(args, CONFIG):
    ...
    # Check routing temperature (only add if non-default)
    if args.routing_temp != 1.0:
        parts.append(f"RoutingTemp{args.routing_temp}")
...
def main():
    ...
    if args.routing_temp != 1.0:
        CONFIG['routing_temperature'] = args.routing_temp
        print(f"Routing temperature set to: {args.routing_temp}")
    if args.routing_init_noise != 0.1:
        CONFIG['routing_init_noise'] = args.routing_init_noise
    if args.no_circular_routing:
        CONFIG['routing_circular'] = False
    ...
    if CONFIG.get('use_residual_weights', True):
        ...
        print(f"  Routing circular distance: {CONFIG.get('routing_circular', True)}")
        print(f"  Routing init noise: {CONFIG.get('routing_init_noise', 0.1)}")
