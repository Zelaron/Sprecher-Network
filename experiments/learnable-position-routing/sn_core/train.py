...
def train_network(dataset, architecture, total_epochs=4000, print_every=400, 
    ...
    if CONFIG['use_residual_weights'] and residual_params > 0:
        ...
        if routing_params > 0:
            print(f"    * Learnable routing parameters: {routing_params}")
    ...
    for idx, layer in enumerate(model.layers, start=1):
        ...
        if CONFIG['use_residual_weights']:
            if hasattr(layer, 'residual_weight') and layer.residual_weight is not None:
                ...
                # Show the learned routing pattern
                with torch.no_grad():
                    temperature = CONFIG.get('routing_temperature', 1.0)
                    if layer.d_in > layer.d_out:
                        routing_type = "pooling"
                        routing_weights = layer.compute_soft_routing_weights(
                            layer.routing_logits, layer.d_in, layer.d_out, temperature
                        )
                    else:
                        routing_type = "broadcasting"
                        routing_weights = layer.compute_soft_routing_weights(
                            layer.routing_logits, layer.d_in, layer.d_out, temperature
                        )
                    print(f"Block {idx}: routing type = {routing_type}")
                    print(f"Block {idx}: routing weights shape = {tuple(routing_weights.shape)}")
                    print(f"Block {idx}: routing weights (first 5x5):")
                    print(routing_weights[:5, :5].cpu().numpy())
        if CONFIG['train_phi_codomain']:
