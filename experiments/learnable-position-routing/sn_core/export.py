...
def export_parameters(model, param_types, save_path, dataset_info=None, checkpoint_info=None):
    ...
    # Export Residual parameters with learnable routing
        ...
        for i, layer in enumerate(model.layers):
            ...
            # Check for learnable routing (different dimensions)
                ...
                if layer.d_in > layer.d_out:
                    lines.append(f"Layer {i} ({layer.d_in} → {layer.d_out}): LEARNABLE ROUTING (pooling)")
                else:
                    lines.append(f"Layer {i} ({layer.d_in} → {layer.d_out}): LEARNABLE ROUTING (broadcasting)")
                
                lines.append(format_tensor(layer.routing_logits, "Routing logits", "  "))
                
                # Compute and show the soft routing matrix
                with torch.no_grad():
                    temperature = CONFIG.get('routing_temperature', 1.0)
                    if layer.d_in > layer.d_out:
                        routing_weights = layer.compute_soft_routing_weights(
                            layer.routing_logits, layer.d_in, layer.d_out, temperature
                        )
                        lines.append("  Soft routing matrix (rows=inputs, cols=outputs):")
                        lines.append("  Each row shows how that input distributes to outputs")
                    else:
                        routing_weights = layer.compute_soft_routing_weights(
                            layer.routing_logits, layer.d_in, layer.d_out, temperature
                        )
                        lines.append("  Soft routing matrix (rows=outputs, cols=inputs):")
                        lines.append("  Each row shows how that output selects from inputs")
                    lines.append(format_tensor(routing_weights, "", "    "))
                    
                    # Add interpretable position information
                    learned_positions = torch.sigmoid(layer.routing_logits)
                    if layer.d_in > layer.d_out:
                        scaled_positions = learned_positions * (layer.d_out - 1)
                        lines.append(f"  Learned positions (mapped to output space [0, {layer.d_out-1}]):")
                    else:
                        scaled_positions = learned_positions * (layer.d_in - 1)
                        lines.append(f"  Learned positions (mapped to input space [0, {layer.d_in-1}]):")
                    lines.append(format_tensor(scaled_positions, "", "    "))
