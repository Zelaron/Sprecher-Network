...
class SprecherLayerBlock(nn.Module):
    ...
    def __init__(self, d_in, d_out, layer_num=0, is_final=False, phi_knots=100, Phi_knots=100,
        ...
        # Learnable routing-based residual connections
        if CONFIG['use_residual_weights']:
            if d_in == d_out:
                ...
                # Pooling case: learnable routing for inputs
                self.residual_weight = None
                self.routing_logits = nn.Parameter(torch.zeros(d_in))
                # Initialize near uniform distribution with small noise
                with torch.no_grad():
                    positions = torch.linspace(0, d_out-1, d_in) + CONFIG['routing_init_noise'] * torch.randn(d_in)
                    self.routing_logits.data = positions
            else:  # d_in < d_out
                # Broadcasting case: learnable routing for outputs
                self.residual_weight = None
                self.routing_logits = nn.Parameter(torch.zeros(d_out))
                # Initialize near uniform distribution
                with torch.no_grad():
                    positions = torch.linspace(0, d_in-1, d_out) + CONFIG['routing_init_noise'] * torch.randn(d_out)
                    self.routing_logits.data = positions
        else:
            self.residual_weight = None
            self.routing_logits = None
    
    def compute_soft_routing_weights(self, routing_logits, source_dim, target_dim, temperature=1.0):
        """
        Compute soft routing weights from routing logits.
        
        Args:
            routing_logits: Learnable parameters (size = max(source, target))
            source_dim: Number of source positions
            target_dim: Number of target positions  
            temperature: Softness control (lower = sharper)
        
        Returns:
            Soft routing matrix
        """
        # Create position grids
        if len(routing_logits) == target_dim:
            # Broadcasting: routing_logits represent target (output) positions
            # Each maps to a position in source (input) space
            learned_positions = torch.sigmoid(routing_logits) * (source_dim - 1)
            source_positions = torch.arange(source_dim, device=routing_logits.device, dtype=torch.float32)
            
            # Compute distances
            distances = torch.abs(learned_positions.unsqueeze(1) - source_positions.unsqueeze(0))
            
            # Apply circular distance if configured
            if CONFIG.get('routing_circular', True):
                distances = torch.min(distances, source_dim - distances)
            
            # Convert to weights using Gaussian kernel
            bandwidth = source_dim / (2 * target_dim)  # Adaptive bandwidth
            weights = torch.exp(-(distances ** 2) / (2 * bandwidth ** 2 * temperature))
            
            # Normalize each target to sum to 1 over sources
            routing_weights = weights / weights.sum(dim=1, keepdim=True)
            
        else:  # len(routing_logits) == source_dim
            # Pooling: routing_logits represent source (input) positions
            # Each maps to a position in target (output) space
            learned_positions = torch.sigmoid(routing_logits) * (target_dim - 1)
            target_positions = torch.arange(target_dim, device=routing_logits.device, dtype=torch.float32)
            
            # Compute distances
            distances = torch.abs(learned_positions.unsqueeze(1) - target_positions.unsqueeze(0))
            
            # Apply circular distance if configured
            if CONFIG.get('routing_circular', True):
                distances = torch.min(distances, target_dim - distances)
            
            # Convert to weights
            bandwidth = target_dim / (2 * source_dim)
            weights = torch.exp(-(distances ** 2) / (2 * bandwidth ** 2 * temperature))
            
            # Normalize each source to sum to 1 over targets
            routing_weights = weights / weights.sum(dim=1, keepdim=True)
    ...
    def compute_output_range_theoretical(self):
        ...
        if CONFIG['use_residual_weights'] and self.input_range is not None:
            ...
            elif self.routing_logits is not None:
                # For soft routing, the contribution is harder to bound precisely
                # We use a conservative estimate based on the fact that routing weights
                # are normalized (sum to 1) along one dimension
                input_bound = max(abs(self.input_range.min), abs(self.input_range.max))
                
                # For both pooling and broadcasting cases with soft routing,
                # the residual contribution is bounded by the input range
                # since routing weights are probabilities
                residual_bound = input_bound
    ...
    def forward(self, x, x_original=None):
        ...
        if CONFIG['use_residual_weights'] and x_original is not None:
            if self.residual_weight is not None:
                ...
                temperature = CONFIG.get('routing_temperature', 1.0)
                
                if self.d_in > self.d_out:
                    # Pooling case
                    routing_weights = self.compute_soft_routing_weights(
                        self.routing_logits, self.d_in, self.d_out, temperature
                    )
                    # routing_weights shape: (d_in, d_out)
                    # Each row sums to 1, representing how input i distributes to outputs
                    
                    # Apply routing: x_original @ routing_weights
                    residual_contribution = torch.matmul(
                        x_original.unsqueeze(1),  # (batch, 1, d_in)
                        routing_weights.unsqueeze(0)  # (1, d_in, d_out)
                    ).squeeze(1)  # (batch, d_out)
                    
                else:  # self.d_in < self.d_out
                    # Broadcasting case  
                    routing_weights = self.compute_soft_routing_weights(
                        self.routing_logits, self.d_in, self.d_out, temperature
                    )
                    # routing_weights shape: (d_out, d_in)
                    # Each row sums to 1, representing how output i selects from inputs
                    
                    # Apply routing: routing_weights @ x_original.T
                    residual_contribution = torch.matmul(
                        routing_weights.unsqueeze(0),  # (1, d_out, d_in)
                        x_original.unsqueeze(2)  # (batch, d_in, 1)
                    ).squeeze(2)  # (batch, d_out)
