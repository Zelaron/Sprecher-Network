...
\begin{itemize}
    \item \textbf{Residual connections (Learnable routing):} When enabled, residual connections are implemented using a learnable soft routing mechanism that adapts to dimensional mismatches while maintaining parameter efficiency. This approach uses learnable position parameters that define soft assignments between input and output dimensions:
    \begin{itemize}
        \item \textbf{Identity} ($d_{\mathrm{in}} = d_{\mathrm{out}}$): Uses a single learnable scalar weight $w_{\text{res}}$
        \item \textbf{Broadcasting} ($d_{\mathrm{in}} < d_{\mathrm{out}}$): Each output dimension learns a position in the input space, with soft selection via Gaussian kernels
        \item \textbf{Pooling} ($d_{\mathrm{in}} > d_{\mathrm{out}}$): Each input dimension learns a position in the output space, with soft routing via Gaussian kernels
    \end{itemize}
    
    Specifically, for the non-identity cases, we introduce learnable logits $\ell_i$ that are transformed to positions via sigmoid and scaled to the target dimension range. The soft routing weights are computed using a Gaussian kernel:
    $$\mathcal{W}_{ij} = \frac{\exp\left(-\frac{d_{ij}^2}{2\sigma^2\tau}\right)}{\sum_k \exp\left(-\frac{d_{ik}^2}{2\sigma^2\tau}\right)}$$
    where $d_{ij}$ is the distance between learned position $i$ and target position $j$, $\sigma$ is an adaptive bandwidth, and $\tau$ is a temperature parameter controlling routing sharpness\footnote{Common temperature values include $\tau \in [0.1, 5.0]$, where lower values create sharper, more selective routing (approaching one-hot assignments), while higher values create smoother, more distributed routing. The default value of $\tau = 1.0$ provides a balanced trade-off between selectivity and gradient flow.}.
    
    The block output with learnable routing becomes:
    $$[B^{(\ell)}_{\text{res}}(\mathbf{x})]_q = [B^{(\ell)}(\mathbf{x})]_q + \begin{cases}
    w_{\text{res}} \cdot x_q & \text{if } d_{\mathrm{in}} = d_{\mathrm{out}}\\
    \sum_{i=1}^{d_{\mathrm{in}}} \mathcal{W}_{qi} \cdot x_i & \text{if } d_{\mathrm{in}} < d_{\mathrm{out}}\\
    \sum_{i: \mathcal{W}_{iq} > 0} \mathcal{W}_{iq} \cdot x_i & \text{if } d_{\mathrm{in}} > d_{\mathrm{out}}
    \end{cases}$$
    
    This soft routing mechanism can be interpreted as learning which input dimensions should contribute to each output dimension (or vice versa), with the temperature parameter controlling whether the routing is sharp (approaching hard assignment) or smooth (allowing gradual transitions). Despite the increased expressivity compared to fixed cyclic patterns, this approach maintains $O(\max(d_{\mathrm{in}}, d_{\mathrm{out}}))$ parameter scaling.
...
\begin{remark}[Learnable routing as adaptive weight sharing]
The learnable routing mechanism can be viewed as a form of adaptive weight sharing that discovers optimal connection patterns during training. Unlike fixed cyclic patterns in our initial design, learnable routing allows the network to learn which dimensions should be connected based on the data. The Gaussian kernel formulation ensures smooth gradients and can interpolate between sparse (low temperature) and dense (high temperature) connection patterns. Interestingly, this mechanism bears resemblance to attention mechanisms, but with learned static positions rather than dynamic query-key interactions, making it computationally efficient while remaining expressive.
\end{remark}

\begin{remark}[Structured sparsity and architectural coherence]
The learnable routing design implements a structured sparse projection where connectivity patterns are learned rather than fixed. For pooling ($d_{\mathrm{in}} > d_{\mathrm{out}}$), each input learns its affinity to different outputs, while for broadcasting ($d_{\mathrm{in}} < d_{\mathrm{out}}$), each output learns its preference for different inputs. This represents another form of weight sharing that complements the main Sprecher architecture: while Sprecher blocks share splines across output dimensions with diversity through shifts, residual connections share structure across connections with diversity through learned routing. The effectiveness of this approach suggests that for gradient flow and information propagation, the ability to learn connection patterns may be more important than the specific pattern chosen a priori.
\end{remark}
...
Sprecher Networks have a fundamentally different structure. Each block $\ell$ contains mixing weights $\lambda^{(\ell)}$ with $O(d_{\ell-1})$ parameters, where $d_{\ell-1}$ is the input dimension to that block (crucially, these are vectors, not matrices), shared splines $\phi^{(\ell)}, \Phi^{(\ell)}$ with $2 \times O(G) = O(G)$ parameters per block independent of $N$, and a shift parameter $\eta^{(\ell)}$ with $O(1)$ parameter per block. When normalization is used, each block may have an associated normalization layer adding $O(N)$ parameters (specifically $2N$ for the affine transformation). When learnable routing is used for residual connections, each connection between layers of different dimensions requires at most $\max(d_{\ell-1}, d_\ell)$ position parameters. Specifically, for broadcasting ($d_{\ell-1} < d_\ell$), we need $d_\ell$ learnable positions, and for pooling ($d_{\ell-1} > d_\ell$), we need $d_{\ell-1}$ learnable positions. These position parameters are transformed into soft routing weights via Gaussian kernels during the forward pass, maintaining the $O(N)$ scaling while providing significantly more flexibility than fixed connection patterns. Summing over $L$ (or $L+1$) blocks, the total parameter count scales approximately as $O(LN + LG + L)$, where the linear dependence on $N$ now includes mixing weights, optional normalization parameters, and learnable routing positions.
...
\item When residual connections with learnable routing are present, the output range must be adjusted by the residual contribution. For a scalar residual weight $w$, if $w \geq 0$, the residual adds $[wa, wb]$ to the range. For learnable routing:
   \begin{itemize}
   \item \textbf{Broadcasting} ($d_{\mathrm{in}} < d_{\mathrm{out}}$): Each output's contribution depends on the soft weighted combination of inputs determined by the routing weights $\mathcal{W}_{qi}$. The contribution range is conservatively estimated as $[a \min_q \sum_i \mathcal{W}_{qi}, b \max_q \sum_i \mathcal{W}_{qi}] = [a, b]$ since routing weights are normalized.
   \item \textbf{Pooling} ($d_{\mathrm{in}} > d_{\mathrm{out}}$): Similar analysis applies with routing weights $\mathcal{W}_{iq}$ determining how inputs route to outputs.
   \end{itemize}
\end{enumerate}
\end{lemma}
...
For (4), the learnable routing contributions follow from the fact that routing weights $\mathcal{W}$ form probability distributions (sum to 1) along the appropriate dimension, ensuring the weighted combinations remain within the input bounds.
\end{proof}
