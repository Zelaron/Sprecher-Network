...
\begin{itemize}
    \item \textbf{Residual connections (Round-Robin/BSP):} When enabled, residual connections are implemented using a node-centric approach that maintains architectural coherence. The method, which we term \emph{Round-Robin Residuals} or BSP (Broadcast-Scalar-Pooling)\footnote{Pedantic readers noting that scalar weights aren't ``round-robin'' may prefer BSP (Broadcast-Scalar-Pooling), which has the added benefit of evoking binary space partitioning between adjacent node spaces.}, adapts based on dimensional relationships:
    \begin{itemize}
        \item \textbf{Scalar} ($d_{\mathrm{in}} = d_{\mathrm{out}}$): Uses a single learnable weight $w_{\text{res}}$
        \item \textbf{Broadcast} ($d_{\mathrm{in}} < d_{\mathrm{out}}$): Each output selects one input via round-robin assignment and applies a learnable scale
        \item \textbf{Pooling} ($d_{\mathrm{in}} > d_{\mathrm{out}}$): Inputs are weighted and aggregated via round-robin assignment to outputs
    \end{itemize}
    ...
    w_q^{\text{bcast}} \cdot x_{s_q} & \text{if } d_{\mathrm{in}} < d_{\mathrm{out}}\\
    \sum_{i: a_i = q} w_i^{\text{pool}} \cdot x_i & \text{if } d_{\mathrm{in}} > d_{\mathrm{out}}
    \end{cases}$$
    where $s_q = q \bmod d_{\mathrm{in}}$ are the broadcast source indices, $a_i = i \bmod d_{\mathrm{out}}$ are the pooling assignments, and $w^{\text{bcast}}, w^{\text{pool}}$ are learnable weight vectors of size $d_{\mathrm{out}}$ and $d_{\mathrm{in}}$ respectively. This approach uses only $\max(d_{\mathrm{in}}, d_{\mathrm{out}})$ parameters for dimension-changing residuals, compared to $d_{\mathrm{in}} \times d_{\mathrm{out}}$ for traditional matrix projections.
...
\begin{remark}[Node-centric residual philosophy]
The Round-Robin Residual design maintains the node-centric philosophy of Sprecher Networks. Rather than learning arbitrary edge weights (as in a projection matrix), the approach uses learnable vectors that operate on nodes, with diversity arising from the fixed assignment patterns. This can be viewed as a form of structured weight sharing: just as the main Sprecher transformation shares weights across output dimensions (with shifts providing diversity), the residual connections share weights across connections through the round-robin patterns. This architectural coherence may contribute to the empirical observation that these constrained residuals often perform as well as or better than full matrix projections.
\end{remark}

\begin{remark}[Round-robin justification]
The round-robin pattern ensures (i) complete input coverage, (ii) uniform distribution across outputs, and (iii) direct gradient paths. While other patterns (e.g., contiguous blocks) share these properties, round-robin has the advantage of maximally distributing adjacent inputs, potentially decorrelating local features. This arbitrary but principled choice represents one reasonable default in the absence of domain-specific knowledge. That various simple patterns likely perform similarly suggests residual connections may primarily serve gradient flow rather than complex feature transformation.
\end{remark}
...
\textbf{Residual design}    & Matrix projection   & Matrix projection  & None (typically)      & Round-Robin/BSP \\
...
Sprecher Networks have a fundamentally different structure. Each block $\ell$ contains mixing weights $\lambda^{(\ell)}$ with $O(d_{\ell-1})$ parameters, where $d_{\ell-1}$ is the input dimension to that block (crucially, these are vectors, not matrices), shared splines $\phi^{(\ell)}, \Phi^{(\ell)}$ with $2 \times O(G) = O(G)$ parameters per block independent of $N$, and a shift parameter $\eta^{(\ell)}$ with $O(1)$ parameter per block. When normalization is used, each block may have an associated normalization layer adding $O(N)$ parameters (specifically $2N$ for the affine transformation). When residual connections are used, they add at most $O(N)$ parameters per block using the Round-Robin approach (specifically $\max(d_{\ell-1}, d_\ell)$ for block $\ell$, or just 1 when dimensions match). Summing over $L$ (or $L+1$) blocks, the total parameter count scales approximately as $O(LN + LG + L)$, where the linear dependence on $N$ now includes mixing weights, optional normalization parameters, and residual connections.
...
\item When residual connections are present, the output range must be adjusted by the residual contribution. For a scalar residual weight $w$, if $w \geq 0$, the residual adds $[wa, wb]$ to the range where $[a,b]$ is the input range. If $w < 0$, it adds $[wb, wa]$ (note the reversal). For Round-Robin residuals:
   \begin{itemize}
...
For (4), the Round-Robin residual contributions follow from analyzing the specific assignment patterns and weight vectors.
\end{proof}
