...
def main():
    ...
    for i in range(5):
        with torch.no_grad():
            ...
            # If we have normalization stats, compute loss on normalized targets
            if 'y_train_norm' in plotting_snapshot:
                y_train_norm = plotting_snapshot['y_train_norm']
                snapshot_loss = torch.mean((snapshot_output - y_train_norm) ** 2).item()
            else:
                # Old format - assume y_train is already normalized or no normalization
    ...
    try:
        ...
        fig_results = plot_results(
            model, layers, dataset, save_path, 
            x_train=x_train, y_train=y_train,
            y_mean=plotting_snapshot.get('y_mean'), 
            y_std=plotting_snapshot.get('y_std')
        )
