...
def train_network(dataset, architecture, total_epochs=4000, print_every=400, 
    ...
    y_mean = y_train.mean()
    y_std = y_train.std()
    # Store original unnormalized data for plotting
    y_train_unnorm = y_train.clone()
    # Normalize
    y_train = (y_train - y_mean) / (y_std + 1e-8)
    print(f"Target normalization: mean={y_mean.item():.3f}, std={y_std.item():.3f}")
    ...
    output_params = 0  # No output parameters
    
    if CONFIG['train_phi_codomain']:
        total_params = lambda_params + eta_params + spline_params + residual_params + codomain_params + norm_params
    else:
        total_params = lambda_params + eta_params + spline_params + residual_params + norm_params
    ...
    for epoch in pbar:
        ...
        # Simple MSE loss only (on normalized targets)
        ...
        if loss.item() < best_loss:
            ...
            plotting_snapshot = {
                ...
                'y_train': y_train_unnorm.clone(),  # Store UNNORMALIZED targets for plotting
                'y_train_norm': y_train.clone(),    # Store normalized targets too
                'y_mean': y_mean,                   # Store normalization stats
                'y_std': y_std,
            ...
            best_checkpoint = {
                ...
                'y_train': y_train_unnorm.cpu().clone(),  # Save UNNORMALIZED target values
                'y_train_norm': y_train.cpu().clone(),  # Save normalized targets
                'y_mean': y_mean,  # Save normalization stats
                'y_std': y_std,
        ...
        # For normalized targets, std should be ~1
    ...
    if best_checkpoint is not None and not no_load_best:
        ...
        if 'plotting_snapshot' in best_checkpoint:
            ...
            y_train = plotting_snapshot['y_train'].to(device)  # This is unnormalized
            y_train_norm = plotting_snapshot.get('y_train_norm', (y_train - y_mean) / (y_std + 1e-8)).to(device)
        ...
        if 'x_train' in best_checkpoint:
            ...
            saved_y = best_checkpoint['y_train'].to(device)  # This is unnormalized
            saved_output = best_checkpoint['output']
            
            # Check if we have normalization stats
            if 'y_mean' in best_checkpoint and 'y_std' in best_checkpoint:
                y_mean = best_checkpoint['y_mean']
                y_std = best_checkpoint['y_std']
                # Normalize saved_y for loss calculation
                saved_y_norm = (saved_y - y_mean) / (y_std + 1e-8)
                print(f"Using saved normalization stats: mean={y_mean.item():.3f}, std={y_std.item():.3f}")
            else:
                # Old checkpoint without normalization
                print("WARNING: Old checkpoint without normalization info")
                saved_y_norm = saved_y
            ...
            with torch.no_grad():
                ...
                current_loss = torch.mean((current_output - saved_y_norm) ** 2).item()
            ...
            x_train = saved_x
            y_train = saved_y  # Unnormalized
            y_train_norm = saved_y_norm
    ...
    if 'plotting_snapshot' not in locals():
        ...
        with torch.no_grad():
            ...
            # Calculate loss on normalized targets
            current_loss = torch.mean((current_output - y_train_norm) ** 2).item()
        
        plotting_snapshot = {
            'model': copy.deepcopy(model),
            'x_train': x_train.clone(),
            'y_train': y_train_unnorm.clone() if 'y_train_unnorm' in locals() else y_train.clone(),
            'y_train_norm': y_train_norm.clone() if 'y_train_norm' in locals() else y_train.clone(),
            'y_mean': y_mean if 'y_mean' in locals() else torch.tensor(0.0),
            'y_std': y_std if 'y_std' in locals() else torch.tensor(1.0),
