...
def plot_high_dim_function(ax, model, dataset, device, y_mean=None, y_std=None):
    ...
    if input_dim <= 10:
        ...
        if output_dim == 1:
            ...
            for i in range(num_slices):
                ...
                with torch.no_grad():
                    y_pred = model(x_test)
                    # Denormalize predictions if normalization stats provided
...
def plot_results(model, layers, dataset=None, save_path=None, 
                 ...
                 title_suffix="", x_train=None, y_train=None, y_mean=None, y_std=None):
    """
    ...
    Args:
        ...
        y_mean: Mean used for target normalization (optional)
        y_std: Std used for target normalization (optional)
    ...
    if y_mean is not None and y_std is not None:
        print(f"DEBUG in plot_results: Using denormalization with mean={y_mean.item():.3f}, std={y_std.item():.3f}")
        ...
        if input_dim == 1:
            ...
            if final_dim == 1:
                ...
                with torch.no_grad():
                    ...
                    # Denormalize predictions if normalization stats provided
                    if y_mean is not None and y_std is not None:
                        Z_pred_all = Z_pred_all * y_std + y_mean
            ...
            gs_bottom.update(left=0.05, right=0.95, top=gs_bottom_top, bottom=0.05)
            ax_func = fig.add_subplot(gs_bottom[0, 0])
            plot_high_dim_function(ax_func, model, dataset, next(model.parameters()).device, y_mean, y_std)
