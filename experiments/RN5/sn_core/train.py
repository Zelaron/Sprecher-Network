...
def train_network(dataset, architecture, total_epochs=4000, print_every=400, 
    ...
    for epoch in pbar:
        ...
        if loss.item() < best_loss:
            ...
            # Compute a BN-batch-stat "eval" output/loss to support deterministic verification later.
            # We evaluate with BatchNorm using the current batch statistics, but without
            # mutating the running_mean / running_var buffers, so this closely mirrors
            # training-mode behavior while remaining side‑effect free.
            was_training_eval = model.training
            model.eval()
            with use_batch_stats_without_updating_bn(model):
                with torch.no_grad():
                    eval_output = model(x_train)
                    eval_loss = torch.mean((eval_output - y_train) ** 2).item()
            model.train(was_training_eval)
            ...
            plotting_snapshot = {
                ...
                'output_eval': eval_output.detach().clone(),   # eval (BN batch-stat) output
                'loss_eval': eval_loss,                        # eval (BN batch-stat) loss
                'epoch': epoch,
                'device': device
            }
            ...
            best_checkpoint = {
                ...
                'loss': loss.item(),
                'loss_eval': eval_loss,                               # BN batch-stat eval loss
                ...
                'output_eval': eval_output.detach().cpu().clone(),   # BN batch-stat eval output
    ...
    if best_checkpoint is not None and not no_load_best:
        ...
        # If we have saved training data, verify the checkpoint deterministically (BN batch-stat eval)
        if 'x_train' in best_checkpoint:
            print("\nVerifying checkpoint with saved training data (BN batch-stat eval; no BN state changes)...")
