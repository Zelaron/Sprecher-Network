...
class SimpleSpline(nn.Module):
    """
    A 1D spline with configurable interpolation type.

    - spline_type='linear' (default): original piecewise-linear behaviour
    - spline_type='cubic' : piecewise-cubic Hermite (C¹, non-trivial second
      derivative almost everywhere). For monotonic splines we use a
      monotone, shape-preserving scheme (Fritsch–Carlson style).
    ...
    def __init__(self, num_knots=30, in_range=(0, 1), out_range=(0, 1), monotonic=False,
                 train_codomain=False, codomain_params=None, spline_type="linear"):
        super().__init__()
        ...
        # Spline interpolation type: 'linear' (original) or 'cubic'
        self.spline_type = str(spline_type).lower()
        if self.spline_type not in ("linear", "cubic"):
            raise ValueError(
                f"Unsupported spline_type '{spline_type}'. "
                f"Use 'linear' or 'cubic'."
            )
    ...
    def update_domain(self, new_range, allow_resampling=True, force_resample=False):
        ...
        with torch.no_grad():
            ...
            if CONFIG.get('debug_checkpoint_loading', False):
                ...
                print(f"  - Spline type: {self.spline_type}")
            ...
            if not self.monotonic and (self.total_evaluations > 0 or force_resample) and allow_resampling:
                if CONFIG.get('debug_checkpoint_loading', False):
                    print(
                        "  - RESAMPLING WILL OCCUR (non-monotonic, "
                        f"evaluations={self.total_evaluations}, force={force_resample}, "
                        f"spline_type={self.spline_type}, resampling allowed)"
                    )
                ...
                def old_spline_eval(x_vals):
                    ...
                    if self.spline_type == "linear":
                        # Original piecewise-linear interpolation
                        interpolated = (1 - t) * old_coeffs[intervals] + t * old_coeffs[intervals + 1]
                        return interpolated
                    elif self.spline_type == "cubic":
                        # Cubic Hermite interpolation with generic slopes
                        slopes = self._compute_generic_slopes(old_coeffs)
                        i0 = intervals
                        i1 = intervals + 1
                        y0 = old_coeffs[i0]
                        y1 = old_coeffs[i1]
                        ...
                        raise ValueError(f"Unsupported spline_type '{self.spline_type}' in resampling.")

                # Define the new knot positions in the new domain
                new_knots_pos = torch.linspace(
                    new_min, new_max, self.num_knots, 
                ...
                if CONFIG.get('debug_checkpoint_loading', False) and not self.monotonic:
                    print(
                        "  - NO RESAMPLING "
                        f"(evaluations={self.total_evaluations}, allow={allow_resampling})"
                    )
            ...
            self.knots.data = torch.linspace(
                self.in_min, self.in_max, self.num_knots, 
                device=self.knots.device, dtype=self.knots.dtype
            )
    ...
    def initialize_as_identity(self, domain_min, domain_max):
        ...
        with torch.no_grad():
            if self.monotonic:
                ...
                self.coeffs.data = torch.linspace(
                    domain_min, domain_max, self.num_knots, 
    ...
    # ------------------------------------------------------------------
    # Cubic Hermite helper methods
    # ------------------------------------------------------------------
    def _compute_slopes(self, coeffs):
        """
        Compute per-knot first derivatives for cubic interpolation.

        For monotonic splines, we use a Fritsch–Carlson style scheme to
        preserve monotonicity. For general splines, we use a simple
        ...
        if self.spline_type != "cubic":
            # Should not be called for linear splines, but keep safe.
            return torch.zeros_like(coeffs)

        if self.monotonic:
            return self._compute_monotone_slopes(coeffs)
        else:
            return self._compute_generic_slopes(coeffs)

    def _compute_generic_slopes(self, coeffs):
        """
        Generic cubic slopes: central differences with endpoint slopes
        equal to the nearest secant. This gives a smooth C¹ spline but
        is not shape-preserving.
        """
        n = coeffs.shape[0]
        slopes = torch.zeros_like(coeffs)
        if n == 1:
            return slopes

        h = self.knots[1:] - self.knots[:-1]           # (n-1,)
        delta = (coeffs[1:] - coeffs[:-1]) / (h + 1e-8)
        ...
        # Endpoints: use one-sided secants
        slopes[0] = delta[0]
        slopes[-1] = delta[-1]
        # Interior points: average neighbouring secants
        slopes[1:-1] = 0.5 * (delta[:-1] + delta[1:])
        return slopes

    def _compute_monotone_slopes(self, coeffs):
        """
        Shape-preserving monotone cubic slopes (Fritsch–Carlson style).
        Assumes coeffs are non-decreasing in x (monotone φ case).
        """
        n = coeffs.shape[0]
        slopes = torch.zeros_like(coeffs)
        if n == 1:
        ...
        if n == 2:
            # Degenerate: single interval, use the secant everywhere
            slopes[0] = delta[0]
            slopes[1] = delta[0]
            return slopes
        ...
        for i in range(1, n - 1):
            delta_im1 = delta[i - 1]
            delta_i = delta[i]
            if (delta_im1 * delta_i) <= 0:
                # Opposite signs or one zero → flat slope
                slopes[i] = torch.zeros_like(delta_i)
            else:
                # Weighted harmonic mean of neighbouring secants
                w1 = 2.0 * h[i] + h[i - 1]
                w2 = h[i] + 2.0 * h[i - 1]
                denom = (w1 / (delta_im1 + 1e-12)) + (w2 / (delta_i + 1e-12))
                slopes[i] = (w1 + w2) / (denom + 1e-12)

        # Endpoint slopes (left)
        h0 = h[0]
        h1 = h[1]
        delta0 = delta[0]
        delta1 = delta[1]
        m0 = ((2.0 * h0 + h1) * delta0 - h0 * delta1) / (h0 + h1 + 1e-12)
        cond1 = (m0 * delta0) <= 0
        cond2 = (delta0 * delta1) <= 0
        cond3 = torch.abs(m0) > 3.0 * torch.abs(delta0)
        m0 = torch.where(cond1, torch.zeros_like(m0), m0)
        m0 = torch.where(~cond1 & cond2 & cond3, 3.0 * delta0, m0)
        slopes[0] = m0

        # Endpoint slopes (right)
        hn1 = h[-1]
        hn2 = h[-2]
        deltan1 = delta[-1]
        deltan2 = delta[-2]
        mn = ((2.0 * hn1 + hn2) * deltan1 - hn1 * deltan2) / (hn1 + hn2 + 1e-12)
        cond1 = (mn * deltan1) <= 0
        cond2 = (deltan1 * deltan2) <= 0
        cond3 = torch.abs(mn) > 3.0 * torch.abs(deltan1)
        mn = torch.where(cond1, torch.zeros_like(mn), mn)
        mn = torch.where(~cond1 & cond2 & cond3, 3.0 * deltan1, mn)
        slopes[-1] = mn

        return slopes

    def _cubic_interpolate(self, t, intervals, knot_spacing, coeffs, slopes):
        """
        Cubic Hermite interpolation on each interval.

        Args:
            t: local parameter in [0,1], same shape as x
            intervals: integer indices of left knots, same shape as x
            knot_spacing: h per evaluation point
            coeffs: 1D tensor of knot values (y_i)
            slopes: 1D tensor of per-knot derivatives (dy/dx at x_i)
        """
        i0 = intervals
        i1 = intervals + 1
    ...
    def get_actual_output_range(self):
        ...
        with torch.no_grad():
            ...
            if self.monotonic:
                ...
                # For Φ splines, the piecewise function achieves its extrema at knots
                # for linear splines. For cubic splines with codomain scaling, the
                # true range is bounded by the codomain parameters when trainable.
                if self.train_codomain and self.codomain_params is not None:
                    ...
                    # No codomain transformation: min/max are coefficient extrema
    ...
    def forward(self, x):
        ...
        # Handle out-of-domain inputs by extending the spline linearly (or flat, for φ)
        ...
        # Interpolation & boundary slopes (used for linear extension for Φ)
        if self.spline_type == "linear":
            # Original piecewise-linear interpolation
            interpolated = (1 - t) * coeffs[intervals] + t * coeffs[intervals + 1]
            boundary_left_slope = (coeffs[1] - coeffs[0]) / (self.knots[1] - self.knots[0] + 1e-8)
            boundary_right_slope = (coeffs[-1] - coeffs[-2]) / (self.knots[-1] - self.knots[-2] + 1e-8)
        elif self.spline_type == "cubic":
            # Cubic Hermite interpolation
            slopes = self._compute_slopes(coeffs)
            interpolated = self._cubic_interpolate(t, intervals, knot_spacing, coeffs, slopes)
            boundary_left_slope = slopes[0]
            boundary_right_slope = slopes[-1]
        else:
            raise ValueError(f"Unsupported spline_type '{self.spline_type}' in forward().")
        
        # For monotonic splines (φ), handle out-of-domain with constant extension (slope 0)
        if self.monotonic:
            ...
            # For general splines (Φ), extend linearly based on boundary slopes
            if self.train_codomain and self.codomain_params is not None:
                ...
                left_slope_scaled = boundary_left_slope * scale_factor
                right_slope_scaled = boundary_right_slope * scale_factor
                ...
                # Fixed codomain case - extend linearly with boundary slopes
                left_slope = boundary_left_slope
                right_slope = boundary_right_slope
    ...
    def set_domain_state(self, state):
        ...
        if CONFIG.get('debug_checkpoint_loading', False):
            ...
            print(f"  - Spline type in state: {state.get('spline_type', 'linear')}")

        # Restore spline type if present (fallback to existing attribute otherwise)
        if 'spline_type' in state:
            self.spline_type = state['spline_type']
...
class SprecherLayerBlock(nn.Module):
    ...
    def __init__(self, d_in, d_out, layer_num=0, is_final=False, phi_knots=100, Phi_knots=100,
        ...
        # Spline types (global config, but stored per block through SimpleSpline)
        phi_spline_type = CONFIG.get('phi_spline_type', 'linear')
        Phi_spline_type = CONFIG.get('Phi_spline_type', 'linear')
        ...
        self.phi = SimpleSpline(
            ...
            out_range=(0, 1),       # Fixed codomain [0,1]
            monotonic=True,
            train_codomain=False,
            codomain_params=None,
            spline_type=phi_spline_type
        )
    ...
    def initialize_codomain_from_domain(self, domain_min, domain_max):
        ...
        with torch.no_grad():
            ...
            self.phi_codomain_params.cc.data = torch.tensor(
                domain_center, 
                device=self.phi_codomain_params.cc.device,
                dtype=self.phi_codomain_params.cc.dtype
            )
            self.phi_codomain_params.cr.data = torch.tensor(
                domain_radius,
    ...
    def compute_output_range_theoretical(self):
        ...
        with torch.no_grad():
            ...
            # Per-q Φ output ranges
            if self.Phi.train_codomain and self.phi_codomain_params is not None and self.Phi.spline_type == "cubic":
                # For cubic Φ with trainable codomain, the true output is bounded
                # globally by [cc-cr, cc+cr]. Use that as a safe (possibly loose)
                # bound instead of sampling interior points, to avoid missing
                # interior cubic extrema.
                cc = self.phi_codomain_params.cc.item()
                cr = self.phi_codomain_params.cr.item()
                y_min_per_q = torch.full_like(s_min_per_q, cc - cr)
                y_max_per_q = torch.full_like(s_max_per_q, cc + cr)
            else:
                # Original sampling-based bounds: endpoints + any interior knots
