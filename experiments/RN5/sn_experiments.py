...
def parse_args():
    ...
    parser.add_argument(
        "--pinn_mode",
        action="store_true",
        help="Enable PINN-friendly configuration (e.g., spline choices / BN behavior in other modules).",
    )
...
def main():
    ...
    # PINN mode flag (used by other modules / config logic)
    CONFIG["pinn_mode"] = bool(getattr(args, "pinn_mode", False))
    ...
    # --- CRITICAL CHANGE: ensure evaluation & plotting use BN-safe eval semantics ---
    model.eval()
    has_bn = has_batchnorm(model)
    print(
        "\nSwitched model to EVAL mode for verification and plotting "
        "(Dropout disabled; BatchNorm handled via BN-safe context)."
    )
    print(f"Model training flag now: {model.training}")

    # Verify the snapshot works correctly with multiple computations (BN-safe EVAL)
    print("\nVerifying plotting snapshot consistency (BN-safe eval semantics):")
    ...
    with torch.no_grad():
        for i in range(5):
            if has_bn:
                # BN layers use batch statistics but do not update running stats
    ...
    print(f"\nReference BN-safe eval loss (first pass): {saved_eval_loss:.4e}")
    ...
    print(f"Max |Δ| from reference loss: {max_diff:.4e}")

    if max_diff < 1e-8:
        print("[OK] Perfect BN-safe EVAL-mode consistency achieved! The snapshot is completely isolated.")
