...
import numpy as np
import matplotlib
from contextlib import contextmanager  # <-- for safe eval/no-grad contexts
...
def _eval_no_grad(model):
    """
    Temporarily put `model` into eval() and disable grads for the enclosed block,
    then restore the model's previous training/eval mode on exit.

    This ensures that validation/metrics/plotting forwards never mutate model state
    (e.g., BatchNorm running stats), while leaving training behavior unchanged.
    """
...
def profile_memory_usage(model, x_sample):
    """Profile memory usage of forward pass (safe eval: no-grad, eval-mode)."""
    import torch.cuda
...
def main():
    ...
    # Verify the snapshot works correctly with multiple computations (SAFE EVAL)
    ...
    # DEBUG: Testing with same points as checkpoint loading (SAFE EVAL)
    ...
    # Export parameters if requested (no forward pass required)
    if CONFIG.get('export_params'):
    ...
    try:
        ...
        # Re-run plotting logic with the safe backend (SAFE EVAL)
        os.makedirs("plots", exist_ok=True)
