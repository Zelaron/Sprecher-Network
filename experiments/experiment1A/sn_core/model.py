...
@contextmanager
def _eval_no_grad(model: nn.Module):
    """
    Temporarily switch `model` to eval() and disable grads, then restore the
    previous train/eval mode on exit. Use this for any *evaluation* path inside
    this module to guarantee BatchNorm (and similar) buffers are not mutated.
    """
...
class SimpleSpline(nn.Module):
    ...
    def forward(self, x):
        ...
        # Track domain violations if debugging — but *only* during training to avoid
        # mutating module state in eval/metrics/verification forwards.
        if CONFIG.get('track_domain_violations', False) and self.training:
...
def test_domain_tightness(model, dataset, n_samples=10000):
    ...
    IMPORTANT: This function now runs the model under `eval()` and `torch.no_grad()`,
    and restores the previous training/eval mode afterward. This guarantees that
    BatchNorm running stats (and similar buffers) are **not** mutated during this
    verification pass.
    """
    ...
    def hook_fn(name):
        def hook(module, input, output):
            if hasattr(module, 'in_min'):  # It's a spline
                ...
                # Safety check moved here: just reporting — no mutating of model state
    ...
    # Run forward passes STRICTLY in eval+no_grad, then restore mode
