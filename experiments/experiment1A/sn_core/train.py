...
def eval_mode(model: nn.Module):
    """
    Temporarily switch `model` to eval() and run with torch.no_grad(), then
    restore its previous training/eval state afterward.

    Use this around *all* metric/validation/verification/test forward passes.
    """
    was_training = model.training
    try:
        model.eval()
...
def train_network(dataset, architecture, total_epochs=4000, print_every=400, 
    ...
    if best_checkpoint is not None and not no_load_best:
        ...
        # Get a sample output before loading checkpoint (now strictly eval; no BN mutation)
        ...
        if 'model_params' in best_checkpoint:
            ...
            print("\n[CHECKPOINT DEBUG] Testing fresh model BEFORE loading state_dict (eval, frozen):")
            with eval_mode(fresh_model):
        ...
        # Verify outputs after loading — strictly under eval (no BN mutations)
        if has_bn and CONFIG.get('debug_checkpoint_loading', False):
        ...
        with eval_mode(model):
            _ = model(x_train[:5]).cpu().numpy()
        print(f"Sample outputs AFTER loading checkpoint: [eval/no-grad verification]")
        
        if has_bn and CONFIG.get('debug_checkpoint_loading', False):
            print("\n[CHECKPOINT DEBUG] Verifying BN stats didn't change during eval forward pass...")
            for name, module in model.named_modules():
        ...
        # For BN models, verification is now done in eval mode to avoid mutating state.
        if 'x_train' in best_checkpoint:
            print("\nVerifying checkpoint with saved training data (eval mode; BN frozen by eval).")
            saved_x = best_checkpoint['x_train'].to(device)
            saved_y = best_checkpoint['y_train'].to(device)
            saved_output = best_checkpoint['output']  # training-mode snapshot; not compared strictly
            
            with eval_mode(model):
                current_output = model(saved_x)
                current_loss = torch.mean((current_output - saved_y) ** 2).item()
            
            print(f"Saved checkpoint (train-mode) loss: {best_checkpoint['loss']:.4e}")
            print(f"Eval-mode loss on saved data:       {current_loss:.4e}")
            print("[INFO] For models with BatchNorm, train vs eval losses may differ; "
                  "we no longer enforce exact equality to avoid false alarms.")
        ...
        if best_checkpoint.get('has_batchnorm', False):
            ...
            if bn_recalc_on_load:
                ...
                with eval_mode(model):
                    _ = model(x_train[:5]).cpu().numpy()
                print(f"Sample outputs AFTER BN recalc: [eval/no-grad verification]")
            else:
