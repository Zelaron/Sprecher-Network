...
class SprecherMultiLayerNetwork(nn.Module):
    ...
    Args:
        ...
        norm_skip_blocks: Which blocks to skip: 'none', 'first', 'last', 'edges', 'all'
    ...
    def __init__(self, input_dim, architecture, final_dim=1, phi_knots=100, Phi_knots=100,
                 norm_type='none', norm_position='after', norm_skip_blocks='first', norm_skip_first=None,
                 initialize_domains=True, domain_ranges=None):
        super().__init__()
        ...
        if norm_skip_first is not None:
            import warnings
            warnings.warn("norm_skip_first is deprecated. Use norm_skip_blocks instead.", DeprecationWarning)
            norm_skip_blocks = 'first' if norm_skip_first else 'none'
        
        self.norm_skip_blocks = norm_skip_blocks
        ...
        if norm_type != 'none':
            ...
            for i, layer in enumerate(self.layers):
                # Determine if this layer should skip normalization
                skip_norm = False
                if norm_skip_blocks == 'all':
                    skip_norm = True
                elif norm_skip_blocks == 'first' and i == 0:
                    skip_norm = True
                elif norm_skip_blocks == 'last' and i == num_layers - 1:
                    skip_norm = True
                elif norm_skip_blocks == 'edges' and (i == 0 or i == num_layers - 1):
                    skip_norm = True
                # 'none' means don't skip any blocks
