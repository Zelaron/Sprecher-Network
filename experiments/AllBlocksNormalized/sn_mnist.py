...
def parse_args():
    ...
    parser.add_argument("--norm_skip_blocks", type=str, default=None,
                        choices=["none", "first", "last", "edges", "all"],
                        help="Which blocks to skip normalization: none, first, last, edges, all (default: from CONFIG)")
    # Backward compatibility
    parser.add_argument("--norm_skip_first", action="store_true", default=None,
                        help="DEPRECATED: Use --norm_skip_blocks instead")
...
class MNISTSprecherNet(nn.Module):
    ...
    def __init__(self, architecture, phi_knots=100, Phi_knots=100,
                 norm_type="none", norm_position="after", norm_skip_blocks='first'):
        super().__init__()
        self.sprecher_net = SprecherMultiLayerNetwork(
            ...
            norm_skip_blocks=norm_skip_blocks
        )
...
def print_configuration(args, architecture, phi_knots, Phi_knots, epochs, batch_size, lr):
    ...
    if args.no_norm:
        ...
        if args.norm_skip_first is not None:
            print("Warning: --norm_skip_first is deprecated. Use --norm_skip_blocks instead.")
            norm_skip_blocks_display = 'first' if args.norm_skip_first else 'none'
        else:
            norm_skip_blocks_display = args.norm_skip_blocks if args.norm_skip_blocks else CONFIG.get('norm_skip_blocks', 'first')
        print(f"Normalization: {args.norm_type} (position: {args.norm_position}, skip_blocks: {norm_skip_blocks_display})")
    else:
        # Handle backward compatibility
        if args.norm_skip_first is not None:
            norm_skip_blocks_display = 'first' if args.norm_skip_first else 'none'
            ...
            norm_skip_blocks_display = args.norm_skip_blocks if args.norm_skip_blocks else CONFIG.get('norm_skip_blocks', 'first')
        print(f"Normalization: {CONFIG.get('norm_type', 'batch')} (position: {args.norm_position}, skip_blocks: {norm_skip_blocks_display})")
...
def test_mnist(args):
    ...
    # Handle norm_skip_blocks with backward compatibility
    if args.norm_skip_first is not None:
        norm_skip_blocks = 'first' if args.norm_skip_first else 'none'
    else:
        norm_skip_blocks = args.norm_skip_blocks if args.norm_skip_blocks else CONFIG.get('norm_skip_blocks', 'first')
