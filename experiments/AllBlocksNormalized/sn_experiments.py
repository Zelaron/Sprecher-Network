...
def main():
    ...
    if effective_norm_type != "none":
        ...
        # Handle backward compatibility for norm_skip_first
        ...
        print(f"Normalization: {effective_norm_type} (position: {norm_position}, skip_blocks: {norm_skip_blocks})")
    else:
    ...
    if effective_norm_type != "none":
        ...
        # Handle backward compatibility for norm_skip_first
        if args.norm_skip_first is not None:
            final_norm_skip_blocks = 'first' if args.norm_skip_first else 'none'
        else:
            final_norm_skip_blocks = args.norm_skip_blocks if args.norm_skip_blocks else CONFIG.get('norm_skip_blocks', 'first')
    else:
        final_norm_position = args.norm_position  # Keep for backward compatibility
        final_norm_skip_blocks = 'none'  # No normalization means skip all
    ...
    plotting_snapshot, losses = train_network(
        ...
        norm_skip_blocks=final_norm_skip_blocks,
