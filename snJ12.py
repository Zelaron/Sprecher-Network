...
architecture = [20, 20, 20, 20, 20]

# Desired dimension of the final output.
# For a scalar target (Sprecher style) set FINAL_OUTPUT_DIM=1.
# For a vector-valued function (e.g. f(x) = {x, 3x - x^2 + 5}) set FINAL_OUTPUT_DIM accordingly.
FINAL_OUTPUT_DIM = 2
...
def target_function(x):
    ...
    # return torch.sin(11*x[:, [0]]) + torch.sin(15*x[:, [1]])
    # return torch.exp(torch.sin(11*x[:, [0]])) + 3*x[:, [1]] + 4*torch.sin(8*x[:, [1]])
    # return (1/4)*x[:, [1]] + (1/5)*x[:, [1]]**2 - x[:, [0]]**3 + (1/5)*torch.sin(7*x[:, [0]])

    # 2D vector function:
    #return torch.cat([
    #    (torch.exp(torch.sin(torch.pi * x[:, [0]]) + x[:, [1]]**2) - 1)/7,
    #    50*torch.sin(x[:, [0]]) + 70*torch.sin(x[:, [1]]),
    #    2*x[:, [0]] + 3*x[:, [1]] + 2,
    #    (1/20)*torch.sin(x[:, [0]]) + x[:, [1]]
    #], dim=1)

    # Another 2D vector function:
    return torch.cat([
        (torch.exp(torch.sin(torch.pi * x[:, [0]]) + x[:, [1]]**2) - 1) / 7,
        # torch.sin(11*x[:, [0]]) + torch.sin(15*x[:, [1]]),
...
def generate_data(n_samples=32):
    ...
    If the target_function returns a 1D tensor (shape: (n,) ),
    we unsqueeze it to have shape (n,1) so that scalar and vector targets
...
def plot_results(model, layers):
    ...
    In the bottom row, if the input is 1D then a 2D line plot is used; if 2D,
    a 3D surface plot is shown.
    
    For multi-output (vector-valued) target functions, each output dimension is plotted
    separately. For 1D input, each subplot shows the d-th component of the target and the network.
    For 2D input with multiple outputs, each subplot shows both the target and network surfaces.
    """
    if len(layers) > 0:
        input_dim = layers[0].d_in
    else:
    ...
    if input_dim == 1:
        ...
        if final_dim == 1:
            ...
            # For multi-output, plot each output dimension separately
        ...
        if final_dim == 1:
            ...
            # For multiple outputs, plot each dimension's target and network output together
            ...
            with torch.no_grad():
                Z_pred_all = model(points.to(next(model.parameters()).device))  # shape (n*n, final_dim)
            for d in range(final_dim):
                ...
                # Plot target and network surfaces with some transparency for comparison
