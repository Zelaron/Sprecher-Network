...
\begin{itemize}
    \item \textbf{Monotonic spline $\phi(\cdot)$:} An increasing piecewise-linear function, typically with codomain $[0,1]$. This function is shared across all input-output connections within the block and its coefficients are learnable. Strict monotonicity (strictly increasing) is enforced during training (see Section \ref{sec:implementation}).
...
One advantage of the structured Sprecher architecture is the ability to compute exact theoretical bounds for all intermediate values. This enables principled initialization and can aid in debugging and analysis.

\begin{lemma}[Domain propagation through Sprecher blocks]\label{lem:domain_prop}
Consider a Sprecher block with input range $[a,b]^{d_{in}}$, shift parameter $\eta$, weights $\lambda_i$, and output dimension $d_{out}$. Then:
\begin{enumerate}
\item The domain required for $\phi$ is:
$$\begin{cases}
...
\item If $\Phi$ has trainable codomain parameters $(c_c, c_r)$ defining codomain $[c_c - c_r, c_c + c_r]$, then the output range is determined by the actual range of $\Phi$ given its domain and parameters.
\end{enumerate}
\end{lemma}
...
This lemma enables a forward propagation algorithm for computing all spline domains throughout the network:

\begin{algorithm}
\caption{Forward domain propagation}
\label{alg:domain_prop} % A label for cross-referencing
\begin{algorithmic}[1] % The [1] adds line numbers
    \State Initialize: Set input range to $[0,1]^{d_0}$
    \For{each block $\ell = 1, \ldots, L$ (or $L+1$ for vector outputs)}
        \State Compute $\phi^{(\ell)}$ domain using Lemma \ref{lem:domain_prop} part (1)
        \State Compute $\Phi^{(\ell)}$ domain using Lemma \ref{lem:domain_prop} part (2)
        \State Compute output range by evaluating $\Phi^{(\ell)}$'s range on its domain
        \If{residual connections exist}
            \State Expand range by $[w \cdot a_{in}, w \cdot b_{in}]$ for scalar residual weight $w$
            \State Expand by worst-case amplification for projection matrix
        \EndIf
        \State Use output range as input range for next block
    \EndFor
\end{algorithmic}
\end{algorithm}

One particularly elegant application of this domain computation is the initialization of $\Phi^{(\ell)}$ splines. By computing each $\Phi^{(\ell)}$'s theoretical domain before training begins, we can initialize both its domain and codomain parameters to match this computed range, making each $\Phi^{(\ell)}$ start as an approximate identity function on its natural operating range. This provides a theoretically motivated initialization that avoids arbitrary choices and can improve optimization dynamics.

\begin{remark}[Domain-aware initialization]
The ability to compute exact domains enables the following initialization strategy:
\begin{enumerate}
\item Initialize all parameters ($\lambda^{(\ell)}, \eta^{(\ell)}$) with small random values
\item Run forward domain propagation to compute each spline's theoretical domain
\item For each $\Phi^{(\ell)}$:
    \begin{itemize}
    \item Set domain knots uniformly across the computed domain
    \item If using trainable codomain, initialize $(c_c, c_r)$ so the codomain matches the domain
    \item Initialize coefficients to create an approximately linear function from domain minimum to maximum
    \end{itemize}
\item For each $\phi^{(\ell)}$:
    \begin{itemize}
    \item Set domain knots uniformly across the computed domain
    \item Initialize to an approximately linear increasing function mapping the domain to $[0,1]$
    \end{itemize}
\end{enumerate}
This ensures all splines begin in reasonable configurations based on the network structure.
\end{remark}
