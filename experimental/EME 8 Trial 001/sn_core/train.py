...
def train_network(dataset, architecture, total_epochs=4000, print_every=400, 
    ...
    if best_checkpoint is not None and not no_load_best:
        ...
        # Restore domain states if needed
        if domain_ranges:
            print("Domains already initialized from saved ranges, skipping restoration")
            # Still need to restore evaluation counters
            if 'domain_states' in best_checkpoint:
                print("Restoring evaluation counters from domain states...")
                for i, layer in enumerate(model.layers):
                    phi_state = best_checkpoint['domain_states'].get(f'layer_{i}_phi')
                    Phi_state = best_checkpoint['domain_states'].get(f'layer_{i}_Phi')
                    if phi_state:
                        layer.phi.total_evaluations = phi_state.get('total_evaluations', 0)
                        layer.phi.domain_violations = phi_state.get('domain_violations', 0)
                    if Phi_state:
                        layer.Phi.total_evaluations = Phi_state.get('total_evaluations', 0)
                        layer.Phi.domain_violations = Phi_state.get('domain_violations', 0)
        elif 'domain_states' in best_checkpoint:
    ...
    # Return training data along with model for plotting consistency
    return model, losses, model.layers, x_train, y_train