...
def train_network(dataset, architecture, total_epochs=4000, print_every=400, 
    ...
    if best_checkpoint is not None and not no_load_best:
        ...
        # DEBUG: Check BatchNorm stats right after loading
        if best_checkpoint['has_batchnorm']:
            print("\nDEBUG: BatchNorm stats RIGHT AFTER loading checkpoint:")
        ...
        if CONFIG.get('use_theoretical_domains', True):
            print("\nDEBUG: About to update domains...")
            model.update_all_domains()
            print("DEBUG: Domain update complete")
            
            # Check BatchNorm stats after domain update
            if best_checkpoint['has_batchnorm']:
                print("\nDEBUG: BatchNorm stats AFTER domain update:")
                for name, module in model.named_modules():
                    if isinstance(module, nn.BatchNorm1d):
                        print(f"  {name}: mean={module.running_mean.data[:3].cpu().numpy()}, var={module.running_var.data[:3].cpu().numpy()}")
        ...
        if best_checkpoint['has_batchnorm']:
            ...
            if bn_recalc_on_load:
                ...
                # Don't set to eval before recalc - let recalc_bn_stats handle modes
                recalculate_bn_stats(model, x_train, num_passes=10)  # More passes
    ...
    # DEBUG: Check model state before setting to eval
    print("\nDEBUG: About to set model to eval mode...")
    if has_bn:
        for name, module in model.named_modules():
            if isinstance(module, nn.BatchNorm1d):
                print(f"  BN {name} before eval: mean={module.running_mean.data[:3].cpu().numpy()}")
                break
    
    # Set model to eval mode
    model.eval()
    
    # DEBUG: Check model state after setting to eval
    if has_bn:
        for name, module in model.named_modules():
            if isinstance(module, nn.BatchNorm1d):
                print(f"  BN {name} after eval: mean={module.running_mean.data[:3].cpu().numpy()}")
                break
    
    # DEBUG: Check model output before and after final domain update
