...
def parse_args():
    ...
    parser.add_argument("--batchnorm_momentum", type=float, default=0.1,
                      help="BatchNorm momentum (default: 0.1)")
    parser.add_argument("--no_batchnorm_affine", action="store_true",
                      help="Disable affine parameters in BatchNorm (not recommended)")
...
def main():
    ...
    # Determine if using batch normalization
    use_batchnorm = (args.normalization == "batch")
    batchnorm_affine = not args.no_batchnorm_affine  # Default to True!
    
    # Disable theoretical domains if using normalization
    if use_batchnorm:
        print("Note: Disabling theoretical domain checking due to normalization")
        CONFIG['use_theoretical_domains'] = False
    ...
    if use_batchnorm:
        print(f"  BatchNorm momentum: {args.batchnorm_momentum}")
        print(f"  BatchNorm affine: {batchnorm_affine}")
    ...
    # Train network with normalization settings
