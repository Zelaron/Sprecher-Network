...
def train_network(dataset, architecture, total_epochs=4000, print_every=400, 
    ...
    Args:
        ...
        batchnorm_affine: Whether to use affine parameters in BatchNorm
    ...
    # Warn about small batch sizes with BatchNorm
    if use_batchnorm and n_samples < 32:
        print(f"\nWARNING: Using BatchNorm with small batch size ({n_samples}). "
              f"This may lead to unstable statistics and poor performance.")
        print("Consider using LayerNorm or increasing the dataset size.")
    ...
    total_params = lambda_params + eta_params + spline_params + residual_params + output_params + codomain_params + batchnorm_params
    ...
    if use_batchnorm and batchnorm_params > 0:
        print(f"  - BatchNorm parameters: {batchnorm_params}")
    ...
    # For BatchNorm, we might want to track more info
    best_bn_stats = None
    ...
    for epoch in pbar:
        ...
        elif use_batchnorm and epoch % 1000 == 0:
            # Occasionally update domains with BatchNorm awareness
            model.update_all_domains_with_batchnorm()
        ...
        # Update best loss and save complete model state
        if loss.item() < best_loss:
            best_loss = loss.item()
            best_epoch = epoch
            
            # Save the complete state including BatchNorm statistics
            best_state = model.state_dict()
            
            # If using BatchNorm, also save some diagnostic info
            if use_batchnorm and model.batch_norms is not None:
                best_bn_stats = []
                for i, bn in enumerate(model.batch_norms):
                    if bn is not None:
                        best_bn_stats.append({
                            'layer': i,
                            'running_mean': bn.running_mean.clone().cpu(),
                            'running_var': bn.running_var.clone().cpu(),
                            'num_batches_tracked': bn.num_batches_tracked.clone().cpu() if hasattr(bn, 'num_batches_tracked') else None
                        })
    ...
    if best_state is not None:
        print(f"\nLoading best model from epoch {best_epoch} with loss: {best_loss:.4e}")
        
        # CRITICAL: Set to eval mode BEFORE loading state dict
        ...
        # Print BatchNorm statistics info if available
        if use_batchnorm and best_bn_stats:
            print("\nBatchNorm statistics from best model:")
            for stat in best_bn_stats:
                print(f"  Layer {stat['layer']}: "
                      f"mean range=[{stat['running_mean'].min():.3f}, {stat['running_mean'].max():.3f}], "
                      f"var range=[{stat['running_var'].min():.3f}, {stat['running_var'].max():.3f}]")
                if stat['num_batches_tracked'] is not None:
                    print(f"    Num batches tracked: {stat['num_batches_tracked'].item()}")
    
    # Ensure model is in eval mode for final domain updates and evaluation
    model.eval()
