...
class SprecherMultiLayerNetwork(nn.Module):
    ...
    def __init__(self, input_dim, architecture, final_dim=1, phi_knots=100, Phi_knots=100,
                 use_batchnorm=False, batchnorm_momentum=0.1, batchnorm_affine=True):
        super().__init__()
        ...
        if not architecture: # No hidden layers
            ...
            # No BatchNorm for single layer network
            if use_batchnorm:
                batch_norms.append(None)
        else:
            L = len(architecture)
            ...
            for i in range(L):
                ...
                # Add BatchNorm before this layer (except for the first layer)
                if use_batchnorm:
                    if i == 0:
                        batch_norms.append(None)  # No BatchNorm before first layer
                    else:
        ...
        self.batch_norms = nn.ModuleList(batch_norms) if use_batchnorm else None
        ...
        # Note: With BatchNorm, theoretical bounds are no longer valid
        if CONFIG.get('use_theoretical_domains', True) and not use_batchnorm:
            self.update_all_domains()
        elif use_batchnorm:
            # Still initialize domains, but with awareness that BatchNorm will change things
            self.update_all_domains_with_batchnorm()
    ...
    def update_all_domains_with_batchnorm(self):
        """Update domains with awareness of BatchNorm effects."""
        # For the first layer, input is still in [0,1]
        current_range = TheoreticalRange(0.0, 1.0)
        
        for i, layer in enumerate(self.layers):
            if self.batch_norms and i > 0 and i < len(self.batch_norms) and self.batch_norms[i] is not None:
                # After BatchNorm, assume roughly [-3, 3] range (3-sigma)
                # This is approximate since actual range depends on batch statistics
            ...
            if CONFIG.get('debug_domains', False):
                print(f"Layer {layer.layer_num} (with BN={self.batch_norms is not None and i > 0}): "
                      f"input_range={layer.input_range}, output_range={layer.output_range}")
    
    def forward(self, x):
        x_in = x
        for i, layer in enumerate(self.layers):
            # Apply BatchNorm if enabled and available for this layer
            if self.batch_norms is not None and i < len(self.batch_norms) and self.batch_norms[i] is not None:
                x_in = self.batch_norms[i](x_in)
            
            # For residual connections, pass the normalized input
    ...
    def verify_batchnorm_state(self):
        """Verify BatchNorm layers are properly initialized."""
        if not self.use_batchnorm or self.batch_norms is None:
            return True
        
        issues = []
        for i, bn in enumerate(self.batch_norms):
            if bn is not None:
                # Check if running stats are initialized
                if torch.all(bn.running_mean == 0) and torch.all(bn.running_var == 1):
                    issues.append(f"Layer {i}: BatchNorm appears uninitialized")
                
                # Check for NaN or Inf
                if torch.any(torch.isnan(bn.running_mean)) or torch.any(torch.isnan(bn.running_var)):
                    issues.append(f"Layer {i}: BatchNorm has NaN values")
                
                if torch.any(torch.isinf(bn.running_mean)) or torch.any(torch.isinf(bn.running_var)):
                    issues.append(f"Layer {i}: BatchNorm has Inf values")
        
        if issues:
            print("BatchNorm verification issues found:")
            for issue in issues:
                print(f"  - {issue}")
            return False
        return True
