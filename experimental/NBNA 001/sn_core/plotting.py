...
def plot_network_structure_ax(ax, layers, input_dim, final_dim=1, batch_norms=None):
    ...
    for block_index, block in enumerate(layers):
        ...
        # Add BatchNorm indicator if present
        if batch_norms is not None and block_index > 0 and block_index < len(batch_norms) and batch_norms[block_index] is not None:
            # Draw a small "BN" label
            bn_x = 0.5 * (layer_x[block_index] + new_x)
            ax.text(bn_x, 0.04, "BN", ha='center', fontsize=7, color='purple', 
                   bbox=dict(boxstyle="round,pad=0.1", facecolor='lavender', alpha=0.5))
...
def plot_results(model, layers, dataset=None, save_path=None, 
    ...
    """
    # CRITICAL: Ensure model is in eval mode for consistent behavior
    was_training = model.training
    model.eval()
    ...
    try:
        ...
        if plot_network and (plot_splines or not plot_function):
            ...
            if input_dim <= 10:
                ...
                # Pass batch_norms if model has them
                batch_norms = getattr(model, 'batch_norms', None)
                plot_network_structure_ax(ax_net, layers, input_dim, final_dim, batch_norms)
                col_offset = 1
