...
class DomainAwareNorm(nn.Module):
    """
    Normalizes inputs while respecting theoretical domain bounds.
    Instead of forcing zero mean/unit variance, it scales inputs to a 
    consistent range that preserves domain properties.
    """
    def __init__(self, num_features, target_range=(-1, 1), eps=1e-5):
        super().__init__()
        self.num_features = num_features
        self.target_min, self.target_max = target_range
        self.eps = eps
        
        # Learnable scale and shift within the target range
    ...
    def forward(self, x):
        if self.training:
            ...
            if self.num_batches_tracked == 0:
                self.running_min = batch_min.squeeze(0)
                self.running_max = batch_max.squeeze(0)
            else:
                momentum = 0.1
                self.running_min = (1 - momentum) * self.running_min + momentum * batch_min.squeeze(0)
                self.running_max = (1 - momentum) * self.running_max + momentum * batch_max.squeeze(0)
            
            self.num_batches_tracked += 1
            
            # Use batch statistics for normalization
            x_min, x_max = batch_min, batch_max
        else:
            # Use running statistics
            x_min = self.running_min.unsqueeze(0)
            x_max = self.running_max.unsqueeze(0)
        
        # Normalize to [0, 1] first
        x_range = x_max - x_min + self.eps
        x_norm = (x - x_min) / x_range
        
        # Scale to target range with learnable parameters
        range_size = self.target_max - self.target_min
        x_scaled = x_norm * range_size + self.target_min
        
        # Apply learnable scale and shift
        return x_scaled * self.scale + self.shift
...
class SprecherMultiLayerNetwork(nn.Module):
    ...
    def __init__(self, input_dim, architecture, final_dim=1, phi_knots=100, Phi_knots=100,
        ...
        if normalization_type != NormalizationType.NONE and len(self.layers) > 1:
            self.norm_layers = nn.ModuleList()
            
            for i in range(1, len(self.layers)):
                prev_layer = self.layers[i-1]
                num_features = prev_layer.d_out
                
                if normalization_type == NormalizationType.BATCH_NORM:
                    norm = nn.BatchNorm1d(num_features, momentum=0.1, eps=1e-5)
                elif normalization_type == NormalizationType.LAYER_NORM:
                    norm = nn.LayerNorm(num_features, eps=1e-5)
                elif normalization_type == NormalizationType.DOMAIN_AWARE:
                    # Use theoretical output range of previous layer for initialization
                    norm = DomainAwareNorm(num_features, target_range=(-1, 1))
                else:
    ...
    def update_all_domains(self):
        ...
        for i, layer in enumerate(self.layers):
            # If normalization is applied before this layer, adjust the range
            if self.norm_layers is not None and i > 0:
                norm = self.norm_layers[i-1]
                
                if isinstance(norm, nn.BatchNorm1d):
                    # BatchNorm roughly outputs in [-3, 3] range (3-sigma)
                    current_range = TheoreticalRange(-3.0, 3.0)
                elif isinstance(norm, nn.LayerNorm):
                    # LayerNorm also roughly [-3, 3]
                    current_range = TheoreticalRange(-3.0, 3.0)
                elif isinstance(norm, DomainAwareNorm):
                    # Domain-aware norm outputs in its target range
                    current_range = TheoreticalRange(norm.target_min, norm.target_max)
            
            # Update φ domain based on potentially normalized input
    ...
    def forward(self, x):
        ...
        for i, layer in enumerate(self.layers):
            # Apply normalization BEFORE the layer (except for first layer)
            if self.norm_layers is not None and i > 0:
                x_in = self.norm_layers[i-1](x_in)
