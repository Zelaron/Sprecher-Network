...
class RangePreservingNorm(nn.Module):
    """
    Range-preserving normalization for Sprecher Networks.
    
    Key properties:
    - Exact output bounds computable from input bounds
    - No possibility of domain violations
    - Linear parameter scaling O(width)
    - Preserves mathematical elegance of Sprecher construction
    """
    def __init__(self, num_features):
        super().__init__()
        self.num_features = num_features
        
        # Single learnable scale parameter per feature
        # Constrained to [0.1, 1.0] via sigmoid to ensure stability
        self.scale = nn.Parameter(torch.ones(num_features))
        
        # Track expected input range for exact computations
        self.register_buffer('input_min', torch.zeros(1))
        self.register_buffer('input_max', torch.ones(1))
        
    def update_input_range(self, input_min, input_max):
        """Update expected input range."""
        self.input_min = torch.tensor(float(input_min))
        self.input_max = torch.tensor(float(input_max))
    
    def get_exact_output_bounds(self, input_min, input_max):
        """
        Compute EXACT output bounds - the key to preventing violations.
        
        The normalization maps [input_min, input_max] to [0, scale],
        where scale is bounded in [0.1, 1.0].
        """
        with torch.no_grad():
            # Bounded scale ensures output is always in [0, max_scale]
            bounded_scale = 0.1 + 0.9 * torch.sigmoid(self.scale)
            max_scale = bounded_scale.max().item()
            
        return 0.0, max_scale
    
    def forward(self, x):
        # Normalize to [0, 1] based on theoretical input range
        x_norm = (x - self.input_min) / (self.input_max - self.input_min + 1e-8)
        
        # Apply bounded scale (always in [0.1, 1.0])
        bounded_scale = 0.1 + 0.9 * torch.sigmoid(self.scale)
        
        # Output is guaranteed to be in [0, bounded_scale]
        return bounded_scale * x_norm
...
class SprecherLayerBlock(nn.Module):
    ...
    def update_phi_domain_theoretical(self, input_range, safety_epsilon=1e-6):
        """Update φ domain based on theoretical bounds with safety margin."""
        ...
        # Add safety margin to prevent floating-point edge cases
        phi_min -= safety_epsilon
        phi_max += safety_epsilon
        
        self.phi.update_domain((phi_min, phi_max))
    
    def update_Phi_domain_theoretical(self, safety_epsilon=1e-6):
        """Update Φ domain based on theoretical bounds with safety margin."""
        ...
        with torch.no_grad():
            ...
            # Add safety margin to prevent floating-point edge cases
            phi_domain_min -= safety_epsilon
            phi_domain_max += safety_epsilon
...
class SprecherMultiLayerNetwork(nn.Module):
    ...
    def __init__(self, input_dim, architecture, final_dim=1, phi_knots=100, 
                 Phi_knots=100, batchnorm=False, norm_type=None):
        """
        ...
        elif self.norm_type == 'range_preserving':
            # Range-preserving normalization
            self.batch_layers = None
            self.norm_layers = nn.ModuleList()
            
            # Create RangePreservingNorm for each layer output (except final)
            for i, layer in enumerate(self.layers[:-1]):  # Skip last layer
                self.norm_layers.append(
                    RangePreservingNorm(layer.d_out)
                )
        else:
            # No normalization
            self.batch_layers = None
    ...
    def update_all_domains(self):
        ...
        # Safety epsilon to prevent floating-point edge cases
        SAFETY_EPSILON = 1e-6
        ...
        for i, layer in enumerate(self.layers):
            ...
            # Update φ domain based on input range with safety margin
            layer.update_phi_domain_theoretical(current_range, safety_epsilon=SAFETY_EPSILON)
            
            # Update Φ domain based on current lambdas with safety margin
            layer.update_Phi_domain_theoretical(safety_epsilon=SAFETY_EPSILON)
            
            # Compute this layer's output range for next layer
            layer_output_range = layer.compute_output_range_theoretical()
            
            # Apply normalization effect if using exact LayerNorm
            if self.norm_type == 'exact_layer' and self.norm_layers is not None:
            ...
            # Apply normalization effect if using range-preserving normalization
            elif self.norm_type == 'range_preserving' and self.norm_layers is not None:
                if i < len(self.norm_layers):
                    # Update expected input range
                    self.norm_layers[i].update_input_range(
                        layer_output_range.min, layer_output_range.max
                    )
                    # Get exact bounds after normalization
                    norm_min, norm_max = self.norm_layers[i].get_exact_output_bounds(
            ...
            if CONFIG.get('debug_domains', False):
                ...
                print(f"  Φ domain: [{layer.Phi.in_min:.3f}, {layer.Phi.in_max:.3f}]")
                if self.norm_type in ['exact_layer', 'range_preserving'] and i < len(self.norm_layers):
    ...
    def forward(self, x):
        ...
        for i, layer in enumerate(self.layers):
            ...
            # Apply post-block normalization (skip last layer)
            if self.norm_type == 'exact_layer' and self.norm_layers is not None:
                if i < len(self.norm_layers):
        ...
        # Return directly without output scaling
        return x_in
