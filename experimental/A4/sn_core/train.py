...
def train_network(dataset, architecture, total_epochs=4000, print_every=400, 
    ...
    # Compute target statistics for better initialization (still computed for optional use)
    y_mean = y_train.mean(dim=0).mean().item()
    y_std = y_train.std(dim=0).mean().item()
    target_stats = {'mean': y_mean, 'std': y_std}
    
    # Create model with normalization support
    # Pass target_stats to model (it can use them for alternative initialization if desired)
    model = SprecherMultiLayerNetwork(
        ...
        batchnorm=batchnorm,
        norm_type=norm_type,
        target_stats=target_stats  # Pass for optional alternative initialization
    ).to(device)
    ...
    # Core parameters (no more output params)
    ...
    if model.norm_type != 'none':
        ...
        elif model.norm_type == 'affine' and model.norm_layers is not None:
            for i, norm_layer in enumerate(model.norm_layers):
                print(f"Layer {i+1} affine normalization:")
                print(f"  weight: {norm_layer.weight.data.cpu().numpy()}")
                print(f"  bias: {norm_layer.bias.data.cpu().numpy()}")
                print(f"  Expected input range: [{norm_layer.expected_input_min.item():.3f}, "
                      f"{norm_layer.expected_input_max.item():.3f}]")
                out_min, out_max = norm_layer.get_exact_output_bounds(
                    norm_layer.expected_input_min.item(),
