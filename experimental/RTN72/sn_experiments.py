...
def parse_args():
    ...
    parser.add_argument("--norm_type", type=str, default="none",
                      choices=['none', 'exact_layer', 'giovanni_batch', 'range_preserving'],
                      help="Normalization type (default: none)")
    parser.add_argument("--batchnorm", action="store_true",
                      help="Enable Giovanni's batch normalization (legacy)")
...
def main():
    ...
    # Legacy support: if --batchnorm is used, override norm_type
    if args.batchnorm:
        norm_type = 'giovanni_batch'
        print("Using Giovanni's batch normalization (--batchnorm flag)")
    else:
    ...
    model, losses, layers = train_network(
        ...
        Phi_knots=args.Phi_knots,
        seed=args.seed,
        batchnorm=(norm_type == 'giovanni_batch'),  # Legacy compatibility
        norm_type=norm_type
    )

    # Put model in evaluation mode. This is relevant for normalization
    model.eval()
    ...
    try:
        ...
        # Add normalization info to filename if used
        if norm_type != 'none':
            norm_str = f"-{norm_type}"
        else:
            norm_str = ""
        
        filename = f"{prefix}-{args.dataset}-{arch_str}{norm_str}-{args.epochs}-epochs-outdim{dataset.output_dim}.png"
        ...
        # Plot loss curve
        loss_filename = f"loss-{args.dataset}-{arch_str}{norm_str}-{args.epochs}-epochs.png"
