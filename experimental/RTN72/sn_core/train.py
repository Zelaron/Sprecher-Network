...
def train_network(dataset, architecture, total_epochs=4000, print_every=400, 
                  ...
                  batchnorm=False, norm_type=None):
    """
    ...
    Args:
        ...
        norm_type: Normalization type ('none', 'exact_layer', 'giovanni_batch', 'range_preserving')
    ...
    # Create model with normalization support
    ...
    for name, param in model.named_parameters():
        ...
        elif 'batch_layers' in name or 'norm_layers' in name:
            # Normalization parameters
            norm_params += param.numel()
    
    # Core parameters (no output parameters anymore!)
    total_params = lambda_params + eta_params + spline_params + residual_params + codomain_params + norm_params
    ...
    if norm_params > 0:
        print(f"  - Normalization parameters ({model.norm_type}): {norm_params}")
    ...
    if CONFIG['use_advanced_scheduler']:
        ...
        params = [
            ...
            {"params": [p for n, p in model.named_parameters() if "norm_layers" in n or "batch_layers" in n], 
             "lr": 0.001, "lr_scale": 0.7},  # Normalization parameters
            {"params": [p for n, p in model.named_parameters() if "phi_codomain_params" not in n and "norm_layers" not in n and "batch_layers" not in n], 
    ...
    for epoch in pbar:
        ...
        # Add normalization type to progress bar
        if model.norm_type != 'none':
            pbar_dict['norm'] = model.norm_type[:4]  # First 4 chars
    ...
    # If using normalization, save buffer states before updating domains
    norm_buffer_states = {}
    if model.norm_type != 'none' and model.norm_layers is not None:
        for i, norm_layer in enumerate(model.norm_layers):
            if model.norm_type == 'range_preserving':
                norm_buffer_states[i] = {
                    'input_min': norm_layer.input_min.clone(),
                    'input_max': norm_layer.input_max.clone()
                }
            elif model.norm_type == 'exact_layer':
                norm_buffer_states[i] = {
                    'expected_input_min': norm_layer.expected_input_min.clone(),
                    'expected_input_max': norm_layer.expected_input_max.clone()
                }
    ...
    # Restore normalization buffer states
    if norm_buffer_states:
        for i, state in norm_buffer_states.items():
            if model.norm_type == 'range_preserving':
                model.norm_layers[i].input_min.data = state['input_min']
                model.norm_layers[i].input_max.data = state['input_max']
            elif model.norm_type == 'exact_layer':
                model.norm_layers[i].expected_input_min.data = state['expected_input_min']
                model.norm_layers[i].expected_input_max.data = state['expected_input_max']
    ...
    # Print normalization information if applicable
    if model.norm_type != 'none':
        print(f"\nNormalization type: {model.norm_type}")
        if model.norm_type == 'exact_layer' and model.norm_layers is not None:
            for i, norm_layer in enumerate(model.norm_layers):
                print(f"Layer {i+1} normalization:")
                print(f"  gamma: {norm_layer.gamma.data.cpu().numpy()}")
                print(f"  beta: {norm_layer.beta.data.cpu().numpy()}")
                print(f"  Expected input range: [{norm_layer.expected_input_min.item():.3f}, "
                      f"{norm_layer.expected_input_max.item():.3f}]")
                out_min, out_max = norm_layer.get_exact_output_bounds(
                    norm_layer.expected_input_min.item(),
                    norm_layer.expected_input_max.item()
                )
                print(f"  Exact output bounds: [{out_min:.3f}, {out_max:.3f}]")
        elif model.norm_type == 'range_preserving' and model.norm_layers is not None:
            for i, norm_layer in enumerate(model.norm_layers):
                print(f"Layer {i+1} normalization:")
                print(f"  scale params: {norm_layer.scale.data.cpu().numpy()}")
                bounded_scale = 0.1 + 0.9 * torch.sigmoid(norm_layer.scale)
                print(f"  bounded scale: {bounded_scale.data.cpu().numpy()}")
                print(f"  Expected input range: [{norm_layer.input_min.item():.3f}, "
                      f"{norm_layer.input_max.item():.3f}]")
                out_min, out_max = norm_layer.get_exact_output_bounds(
                    norm_layer.input_min.item(),
                    norm_layer.input_max.item()
                )
                print(f"  Exact output bounds: [{out_min:.3f}, {out_max:.3f}]")
    
    print(f"\nFinal loss: {best_loss:.4e}")
