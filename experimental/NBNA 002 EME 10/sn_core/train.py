...
def save_spline_domains(model):
    """Save domain information for all splines in the model."""
    domain_info = {}
    for i, layer in enumerate(model.layers):
        domain_info[f'layer_{i}_phi_min'] = layer.phi.in_min
        domain_info[f'layer_{i}_phi_max'] = layer.phi.in_max
        domain_info[f'layer_{i}_Phi_min'] = layer.Phi.in_min
        domain_info[f'layer_{i}_Phi_max'] = layer.Phi.in_max
    return domain_info


def restore_spline_domains(model, domain_info):
    """Restore domain information for all splines in the model."""
    for i, layer in enumerate(model.layers):
        # Restore phi domain
        phi_min = domain_info.get(f'layer_{i}_phi_min')
        phi_max = domain_info.get(f'layer_{i}_phi_max')
        if phi_min is not None and phi_max is not None:
            layer.phi.in_min = phi_min
            layer.phi.in_max = phi_max
            # Update knots to match the restored domain
            layer.phi.knots.data = torch.linspace(phi_min, phi_max, layer.phi.num_knots, 
                                                 device=layer.phi.knots.device, 
                                                 dtype=layer.phi.knots.dtype)
...
def train_network(dataset, architecture, total_epochs=4000, print_every=400, 
    ...
    for epoch in pbar:
        ...
        # Update best loss and save checkpoint with domain information
        if loss.item() < best_loss:
            best_loss = loss.item()
            # Save spline domains along with model state
            domain_info = save_spline_domains(model)
            best_checkpoint = {
                'epoch': epoch,
                ...
                'domain_info': domain_info  # Add domain information
            }
    ...
    if best_checkpoint is not None and not no_load_best:
        ...
        # Restore domain information if available
        if 'domain_info' in best_checkpoint:
            restore_spline_domains(model, best_checkpoint['domain_info'])
            print("Spline domains restored from checkpoint!")
        else:
            print("WARNING: No domain information in checkpoint (old checkpoint format)")
        ...
        # Skip domain updates entirely since we restored the exact domains
        print("\nSkipping domain updates - using restored domains from checkpoint")
        
        # DEBUG: Check BatchNorm stats after loading
        if best_checkpoint['has_batchnorm']:
            print("\nDEBUG: BatchNorm stats AFTER loading checkpoint:")
