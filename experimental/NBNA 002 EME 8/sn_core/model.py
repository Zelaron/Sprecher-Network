...
class SimpleSpline(nn.Module):
    ...
    def update_domain(self, new_range, allow_resampling=True):
        """
        ...
        with torch.no_grad():
            ...
            if not self.monotonic and self.total_evaluations > 0 and allow_resampling:
...
class SprecherLayerBlock(nn.Module):
    ...
    def update_phi_domain_theoretical(self, input_range, allow_resampling=True):
        ...
        self.phi.update_domain((phi_min, phi_max), allow_resampling=allow_resampling)
    
    def update_Phi_domain_theoretical(self, allow_resampling=True):
        """Update Φ domain based on theoretical bounds."""
        ...
        with torch.no_grad():
            ...
            self.Phi.update_domain((phi_domain_min, phi_domain_max), allow_resampling=allow_resampling)
...
class SprecherMultiLayerNetwork(nn.Module):
    ...
    def update_all_domains(self, allow_resampling=True):
        """
        ...
        for layer in self.layers:
            ...
            layer.update_phi_domain_theoretical(current_range, allow_resampling=allow_resampling)
            
            # Update Φ domain based on current lambdas
            layer.update_Phi_domain_theoretical(allow_resampling=allow_resampling)
